...
# Without httpfs patches, fall back to the same env xlwings uses (no
# httpfs in the wheel). Useful for the bootstrap phase to verify the
# end-to-end pipeline before layering httpfs on.
if [ "${RECIPE_PATCHES_OPTIONAL:-0}" = "1" ]; then
  echo "=== bootstrap mode: building WITHOUT httpfs ==="
  EXTRA_CMAKE_ARGS=""
else
  EXTRA_CMAKE_ARGS="-DBUILD_HTTPFS_EXTENSION=ON"
fi
