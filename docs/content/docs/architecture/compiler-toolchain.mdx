...
The commands a guest runs through [process execution](/agentos/docs/processes), the shell
...
runtime, see the [WASM VM](/agentos/docs/architecture/posix-syscalls) page. This page is the build-side
counterpart: it documents the toolchain that emits binaries carrying both
[the host-import layer and the WASI shim](/agentos/docs/architecture/posix-syscalls).

Browser-targeted modules embedded inside V8 use a different integration path.
They call explicit JavaScript host functions and can suspend across promises
through [Async WebAssembly & JSPI](/agentos/docs/architecture/async-webassembly),
so they do not need this POSIX sysroot unless their upstream code expects it.
...
- **A patched `wasi-libc`.** The libc is the WASI standard library, modified so
  ...
  same patched libc the [Layer 2 shim](/agentos/docs/architecture/posix-syscalls) adapts at runtime; the
...
- **The `wasi-ext` bindings.** These declare the extra WebAssembly import
  ...
  [Layer 1: custom host import modules](/agentos/docs/architecture/posix-syscalls) for the runtime half.
...
the kernel and gated by the VM's [permission policy](/agentos/docs/permissions) at
...
- Keep the build and runtime contracts aligned: the patched `wasi-libc` and the
  ...
  the [WASM VM](/agentos/docs/architecture/posix-syscalls) runtime expects to satisfy.
