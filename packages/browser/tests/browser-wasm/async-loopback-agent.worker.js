...
// tests/browser-wasm/async-loopback-agent.worker.ts
var endpoint = null;
var buffer = "";
var decoder = new TextDecoder();
var encoder = new TextEncoder();
var PORT = 39556;
function syscall(operation, arg) {
  const raw = endpoint.syscall(encodeSyscall(operation, [arg]));
  const response = JSON.parse(decoder.decode(raw));
...
function runLoopback(message) {
  ...
  const received = typeof read.data === "string" ? decoder.decode(decodeBase642(read.data)) : "";
