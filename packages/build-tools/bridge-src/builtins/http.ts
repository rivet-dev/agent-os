...
var IncomingMessage = class {
	...
	constructor(response) {
		...
		const bodyEncoding =
			...
			(Array.isArray(bodyEncodingHeader)
				? bodyEncodingHeader[0]
				: bodyEncodingHeader);
		if (
			bodyEncoding === "base64" &&
			response?.body &&
	...
	pipe(dest) {
		...
		if (
			...
			(typeof buf === "string" ? buf.length : buf.length) > 0
		) {
			dest.write(buf);
		}
		...
		return {
			async next() {
				...
				if (!dataEmitted && !self._bodyConsumed) {
					...
					if (typeof Buffer !== "undefined") {
						buf = self._isBinary
							? Buffer.from(self._body || "", "binary")
							: Buffer.from(self._body || "");
					} else {
						buf = self._body || "";
					}
...
var ClientRequest = class {
	...
	async _dispatchWithSocket(socket) {
		...
		try {
			...
			const bridgeBackedSocket =
				socket instanceof NetSocket ||
				(typeof socket?._socketId === "string" &&
					socket._socketId.length > 0) ||
				(typeof socket?._socketId === "number" && socket._socketId > 0);
			...
			if (
				...
				isRawSocketRequest(requestMethod, normalizedHeaders) ||
				this._options.socketPath ||
				this._agent?.keepAlive === true
			) {
				await this._dispatchRawSocketRequest(
					socket,
					requestMethod,
					normalizedHeaders,
				);
	...
	async _dispatchUndiciRequest(socket, requestMethod) {
		await waitForSocketReadyForProtocol(
			socket,
			this._options.protocol || "http:",
		);
		...
		const headerPairs = buildRawHttpHeaderPairs(
			this._headers,
		...
		const res = new IncomingMessage({
			...
			trailers:
				response?.trailers && typeof response.trailers === "object"
					? response.trailers
					: {},
		...
		res.once("end", () => {
			process.nextTick(() => {
				this._finalizeSocket(
					socket,
					this._agent?.keepAlive === true && !this.aborted,
				);
			});
		});
		if (this._callback) {
	...
	async _dispatchRawSocketRequest(socket, requestMethod, normalizedHeaders) {
		...
		if (
			...
			!normalizedHeaders["transfer-encoding"]
		) {
			headerPairs.push(["Content-Length", String(bodyBuffer.length)]);
		}
		...
		const timeoutMs =
			typeof this._options.timeout === "number" && this._options.timeout > 0
				? this._options.timeout
				: 3e4;
		const responsePromise = waitForRawHttpResponse(
			socket,
			requestMethod,
			timeoutMs,
		);
	...
	_execute() {
		...
		if (typeof createConnection === "function") {
			...
			const maybeSocket = createConnection(
				{ ...this._options },
				(_err, socket) => {
					finish(socket);
				},
	...
	off(event, listener) {
		if (this._listeners[event]) {
			const idx = this._listeners[event].findIndex(
				(registered) =>
					registered === listener || registered.listener === listener,
			);
			if (idx !== -1) this._listeners[event].splice(idx, 1);
		}
		return this;
	...
	getRawHeaderNames() {
		return Object.keys(this._headers).map(
			(key) => this._rawHeaderNames.get(key) || key,
		);
	}
	hasHeader(name) {
		if (typeof name !== "string") {
		...
		return Object.hasOwn(this._headers, name.toLowerCase());
	}
	removeHeader(name) {
		if (typeof name !== "string") {
			throw createTypeErrorWithCode(
	...
	write(data, _encoding, callback) {
		...
		const addedBytes =
			typeof Buffer !== "undefined" ? Buffer.byteLength(data) : data.length;
		if (this._bodyBytes + addedBytes > MAX_HTTP_BODY_BYTES) {
			throw new Error(
				"ERR_HTTP_BODY_TOO_LARGE: request body exceeds " +
					MAX_HTTP_BODY_BYTES +
					" byte limit",
			);
		}
	...
	destroy(err) {
		...
		const destroyError =
			err ??
			(!this.aborted && !responseStarted ? createConnResetError() : void 0);
	...
	setTimeout(timeout, callback) {
		...
		if (!Number.isFinite(timeout) || timeout < 0) {
			throw new TypeError(
				`The "timeout" argument must be of type number. Received ${String(timeout)}`,
			);
		}
	...
	_validateTimeoutOption() {
		...
		if (typeof timeout !== "number") {
			...
			const error = new TypeError(
				`The "timeout" argument must be of type number. Received ${received}`,
			);
	...
	_bindAbortSignal() {
		...
		if (typeof signal.addEventListener === "function") {
			signal.addEventListener("abort", this._signalAbortHandler, {
				once: true,
			});
			return;
		}
		const signalWithOnAbort = signal;
		signalWithOnAbort.__agentOsPrevOnAbort__ =
			signalWithOnAbort.onabort ?? null;
	...
	_unbindAbortSignal() {
		...
		if (signalWithOnAbort.onabort === this._signalAbortHandler) {
			signalWithOnAbort.onabort =
				signalWithOnAbort.__agentOsPrevOnAbort__ ?? null;
...
var DirectTunnelSocket = class {
	...
	_pushData(buffer) {
		...
		this.emit(
			"data",
			this._encoding ? buffer.toString(this._encoding) : buffer,
		);
	}
	_pushEnd() {
		if (this.destroyed) {
...
var Agent = class _Agent {
	...
	constructor(options) {
		...
		this._validateSocketCountOption(
			"maxTotalSockets",
	...
	createConnection(options, cb) {
		const createConnection =
			typeof options.createConnection === "function"
				...
				: typeof this.options.createConnection === "function"
					? this.options.createConnection
					: null;
		if (createConnection) {
			return createConnection(options, cb ?? (() => void 0));
		}
	...
	_createSocketForRequest(name, request, options) {
		...
		try {
			const maybeSocket = this.createSocket(
				request,
				connectionOptions,
				(err, socket) => {
					finish(err, socket);
...
var HTTP_TOKEN_EXTRA_CHARS = /* @__PURE__ */ new Set([
	"!",
	"#",
	"$",
	"%",
...
function appendNormalizedHeader(target, key, value) {
	...
	target[key] =
		existing === void 0 ? value : `${joinHeaderValue(existing)}, ${value}`;
}
...
function buildHostHeader(options) {
	...
	const defaultPort =
		options.protocol === "https:" || Number(options.port) === 443 ? 443 : 80;
...
function normalizeRequestHeaders(headers) {
	...
	Object.entries(headers).forEach(([key, value]) => {
		...
		if (Array.isArray(value)) {
			value.forEach((entry) =>
				appendNormalizedHeader(normalized, normalizedKey, String(entry)),
			);
			return;
		}
...
function hasUpgradeRequestHeaders(headers) {
	const connectionHeader = joinHeaderValue(
		headers.connection || "",
	).toLowerCase();
...
function parseRawHttpResponse(buffer) {
	...
	for (const line of lines) {
		...
		if (
			...
			headers[previousHeaderName] =
				joinHeaderValue(headers[previousHeaderName]) + ` ${continuation}`;
			continue;
		}
...
function waitForRawHttpResponseHead(socket, timeoutMs) {
	return new Promise((resolve, reject) => {
		...
		const onEnd = () => {
			finish(
				createConnResetError(
					"socket ended before receiving HTTP response head",
				),
			);
		};
		const onClose = () => {
			finish(
				createConnResetError(
					"socket closed before receiving HTTP response head",
				),
			);
		};
		const timer = setTimeout(() => {
			finish(
				new Error(
					`Timed out waiting for HTTP response head after ${timeoutMs}ms`,
				),
			);
		}, timeoutMs);
	...
	return new Promise((resolve, reject) => {
		...
		const configureBodyHandling = () => {
			...
			if (transferEncoding !== void 0) {
				const tokens = splitTransferEncodingTokens(
					joinHeaderValue(transferEncoding),
				);
				const chunkedCount = tokens.filter(
					(entry) => entry === "chunked",
				).length;
				const hasChunked = chunkedCount > 0;
				const chunkedIsFinal =
					hasChunked && tokens[tokens.length - 1] === "chunked";
				if (
					!hasChunked ||
					chunkedCount !== 1 ||
					!chunkedIsFinal ||
		...
		const onEnd = () => {
			...
			finish(
				createConnResetError(
					"socket ended before receiving complete HTTP response body",
				),
			);
		};
		const onClose = () => {
			...
			finish(
				createConnResetError(
					"socket closed before receiving complete HTTP response body",
				),
			);
		};
		const timer = setTimeout(() => {
			finish(
				new Error(`Timed out waiting for HTTP response after ${timeoutMs}ms`),
			);
		}, timeoutMs);
		socket.on("data", onData);
		socket.once("error", onError);
...
function parseChunkedBody(bodyBuffer, maxBodyBytes = MAX_HTTP_BODY_BYTES) {
	...
	while (true) {
		...
		if (chunkSize === 0) {
			...
			if (
				bodyBuffer[trailersStart] === 13 &&
				bodyBuffer[trailersStart + 1] === 10
			) {
				return {
					complete: true,
			...
			const trailerBlock = bodyBuffer
				.subarray(trailersStart, trailersEnd)
				.toString("latin1");
...
function parseLoopbackRequestBuffer(buffer, server) {
	...
	const headerBlock = buffer
		.subarray(requestStart, headerEnd)
		.toString("latin1");
	...
	const requestMatch = /^([A-Z]+)\s+(\S+)\s+HTTP\/(1)\.(0|1)$/.exec(
		requestLine,
	);
	if (!requestMatch) {
		return {
	...
	const requestCloseHeader = joinHeaderValue(
		headers.connection || "",
	).toLowerCase();
	...
	if (
		hasUpgradeRequestHeaders(headers) &&
	...
	if (transferEncoding !== void 0) {
		...
		if (
			...
			parsedContentLength > MAX_HTTP_BODY_BYTES
		) {
			return {
				kind: "bad-request",
				closeConnection: true,
	...
	return {
		...
		request: {
			...
			rawHeaders,
			bodyBase64:
				requestBody.length > 0 ? requestBody.toString("base64") : void 0,
		},
	};
}
...
function serializeLoopbackResponse(response, request, requestWantsClose) {
	...
	const { headers, rawNameMap, order } = serializeRawHeaderPairs(
		response.rawHeaders,
		response.headers,
	);
	const trailerInfo = serializeRawHeaderPairs(
	...
	if (closeConnection) {
		...
		headers.connection === void 0 &&
		request.headers.connection !== void 0
	) {
	...
	for (const informational of response.informational ?? []) {
		const infoHeaders = finalizeRawHeaderPairs(
			serializeRawHeaderPairs(informational.rawHeaders, informational.headers)
				.headers,
			serializeRawHeaderPairs(informational.rawHeaders, informational.headers)
				.rawNameMap,
			serializeRawHeaderPairs(informational.rawHeaders, informational.headers)
		...
		const headerLines2 = infoHeaders
			.map(
...
`,
	...
	const headerLines = finalHeaders
		.map(
			([name, value]) => `${name}: ${value}\r
`,
		)
	...
	if (bodyAllowed) {
		if (isChunked) {
			if (bodyBuffer.length > 0) {
				serializedChunks.push(
					Buffer.from(bodyBuffer.length.toString(16) + "\r\n", "latin1"),
				);
...
`,
	...
	return {
		payload:
			serializedChunks.length === 1
				? serializedChunks[0]
				: Buffer.concat(serializedChunks),
		closeConnection,
	};
}
...
var ServerIncomingMessage = class {
	...
	read() {
		if (!this._bodyReady || this._bodyDelivered) {
			return null;
		}
		this._bodyDelivered = true;
		this._readableState.length = 0;
		this._scheduleBodyDelivery();
		return this.rawBody ?? Buffer.alloc(0);
	}
	pipe(dest) {
	...
	_markBodyReady() {
		...
		this._readableState.length = this.rawBody?.length ?? 0;
		if (!this.rawBody || this.rawBody.length === 0) {
			this._bodyDelivered = true;
		}
		this._scheduleBodyDelivery();
	}
	_scheduleBodyDelivery() {
		if (
			!this._bodyReady ||
			this._endDelivered ||
			this._bodyDeliveryScheduled ||
			this.destroyed
		) {
			return;
		}
		const hasDataListener = (this._listeners.data?.length ?? 0) > 0;
		const hasEndListener = (this._listeners.end?.length ?? 0) > 0;
		if (!this._bodyDelivered && !hasDataListener) {
			return;
		}
		if (this._bodyDelivered && !hasEndListener) {
			return;
		}
		this._bodyDeliveryScheduled = true;
		queueMicrotask(() => {
			this._bodyDeliveryScheduled = false;
			this._flushBody();
		});
	}
	_flushBody() {
		if (!this._bodyReady || this._endDelivered || this.destroyed) {
			return;
		}
		if (!this._bodyDelivered) {
			if ((this._listeners.data?.length ?? 0) === 0) {
				return;
			}
			this._bodyDelivered = true;
			this._readableState.length = 0;
		...
		if ((this._listeners.end?.length ?? 0) === 0) {
			return;
		}
		this._endDelivered = true;
		this.readable = false;
		this.complete = true;
		this._readableState.ended = true;
		this.emit("end");
...
var ServerResponseBridge = class {
	...
	writeHead(statusCode, headers) {
		if (statusCode >= 100 && statusCode < 200 && statusCode !== 101) {
			...
			const normalizedHeaders = Array.from(
				informationalHeaders.entries(),
			).flatMap(([key, value]) => {
				const serialized = serializeHeaderValue(value);
				return Array.isArray(serialized)
					? serialized.map((entry) => [key, entry])
					: [[key, serialized]];
			});
			const rawHeaders = Array.from(informationalHeaders.entries()).flatMap(
				([key, value]) => {
	...
	_appendChunk(chunk, encoding, streamed) {
		...
		const buf =
			typeof chunk === "string"
				? Buffer.from(chunk, typeof encoding === "string" ? encoding : void 0)
				: chunk;
		if (this._chunksBytes + buf.byteLength > MAX_HTTP_BODY_BYTES) {
			throw new Error(
				"ERR_HTTP_BODY_TOO_LARGE: response body exceeds " +
					MAX_HTTP_BODY_BYTES +
					" byte limit",
			);
		}
		...
		if (this._streamSocket && this._streamHeadSent && !this.writableFinished) {
			...
			if (this._streamUsesChunked && !this._streamSocket.destroyed) {
				...
				for (const [key, value] of this._trailers) {
					...
					const serialized = serializeHeaderValue(value);
					for (const entry of Array.isArray(serialized)
						? serialized
						: [serialized]) {
	...
	_streamEndBody(body, encoding) {
		...
		const built = serializeLoopbackResponse(
			headerResponse,
			this._streamRequest,
			true,
		);
	...
	_streamWriteHead() {
		...
		if (
			this._streamUsesChunked &&
			payload.length >= 5 &&
			payload.subarray(payload.length - 5).toString("latin1") === "0\r\n\r\n"
		) {
	...
	getRawHeaderNames() {
		return Array.from(this._headers.keys()).map(
			(key) => this._rawHeaderNames.get(key) || key,
		);
	}
	getHeaders() {
	...
	serialize() {
		...
		const serializedHeaders = Array.from(this._headers.entries()).flatMap(
			([key, value]) => {
				const serialized = serializeHeaderValue(value);
		...
		const rawHeaders = Array.from(this._headers.entries()).flatMap(
			([key, value]) => {
		...
		const serializedTrailers = Array.from(this._trailers.entries()).flatMap(
			([key, value]) => {
				const serialized = serializeHeaderValue(value);
				return Array.isArray(serialized)
					? serialized.map((entry) => [key, entry])
		...
		const rawTrailers = Array.from(this._trailers.entries()).flatMap(
			([key, value]) => {
				const rawName = this._rawTrailerNames.get(key) || key;
				const serialized = serializeHeaderValue(value);
				return Array.isArray(serialized)
		...
		return {
			...
			rawHeaders,
			informational:
				this._informational.length > 0 ? [...this._informational] : void 0,
	...
	_flushPendingRawInformational() {
		...
		while (separatorIndex !== -1) {
			...
			this._pendingRawInfoBuffer = this._pendingRawInfoBuffer.slice(
				separatorIndex + 4,
			);
			const [statusLine, ...headerLines] = rawFrame.split("\r\n");
			const statusMatch = /^HTTP\/1\.[01]\s+(\d{3})(?:\s+(.*))?$/.exec(
				statusLine,
			);
			if (!statusMatch) {
...
var Server = class {
	...
	_endRequestDispatch() {
		this._activeRequestDispatches = Math.max(
			0,
			this._activeRequestDispatches - 1,
		);
...
async function dispatchHttp2CompatibilityRequest(serverId, requestId) {
	...
	if (
		...
		typeof _networkHttp2ServerRespondRaw === "undefined"
	) {
		return;
	}
...
async function dispatchLoopbackServerRequest(serverOrId, requestInput) {
	...
	const request =
		typeof requestInput === "string" ? JSON.parse(requestInput) : requestInput;
	const incoming = new ServerIncomingMessage(request);
	const outgoing = new ServerResponseBridge();
	incoming.socket = outgoing.socket;
	incoming.connection = outgoing.socket;
...
function dispatchSocketRequest(
	...
	const head =
		typeof Buffer !== "undefined"
			? Buffer.from(headBase64, "base64")
			: new Uint8Array(0);
	const hostHeader = incoming.headers["host"];
	const socket = new UpgradeSocket(socketId, {
		host:
			(Array.isArray(hostHeader) ? hostHeader[0] : hostHeader)?.split(":")[0] ||
			"127.0.0.1",
	});
...
ServerResponseCallable.prototype = Object.create(
	ServerResponseBridge.prototype,
	{
		constructor: {
			value: ServerResponseCallable,
			writable: true,
			configurable: true,
		},
	},
...
function createHttpModule(protocol) {
	...
	const moduleAgent = new Agent({
		keepAlive: false,
		createConnection(options, cb) {
			return createHttpRequestSocket(
	...
	return {
		request(options, optionsOrCallback, maybeCallback) {
			...
			return new ClientRequest(
				withModuleDefaultAgent(ensureProtocol(opts)),
				callback,
			);
		},
		...
		createServer(_optionsOrListener, maybeListener) {
			const listener =
				typeof _optionsOrListener === "function"
					...
					: maybeListener;
			const serverOptions =
				typeof _optionsOrListener === "function" ? null : _optionsOrListener;
...
function onHttpServerRequest(eventType, payload) {
	...
	if (
		!payload ||
		payload.serverId === void 0 ||
		payload.requestId === void 0 ||
...
export {
	Agent,
	appendNormalizedHeader,
	attachHttpServerSocket,
	buildHostHeader,
	buildRawHttpHeaderPairs,
	buildUndiciOrigin,
	ClientRequest,
	checkInvalidHeaderChar,
	checkIsHttpToken,
	cloneStoredHeaderValue,
	createAbortError2,
	createBadRequestResponseBuffer,
	createConnResetError,
	createErrorWithCode,
	createHttpModule,
	createHttpRequestSocket,
	createInvalidArgTypeError2,
	createTypeErrorWithCode,
	createUnsupportedHttpSocketWriteError,
	DirectTunnelSocket,
	debugBridgeNetwork,
	dispatchConnectRequest,
	dispatchHttp2CompatibilityRequest,
	dispatchLoopbackServerRequest,
	dispatchServerRequest,
	dispatchSocketBackedServerRequest,
	dispatchSocketRequest,
	dispatchUpgradeRequest,
	FakeSocket,
	finalizeRawHeaderPairs,
	flattenHeaderPairs,
	formatReceivedType,
	getUndiciClientForSocket,
	HTTP_METHODS,
	HTTP_STATUS_TEXT,
	HTTP_TOKEN_EXTRA_CHARS,
	hasResponseBody,
	hasUpgradeRequestHeaders,
	http,
	https,
	INVALID_REQUEST_PATH_REGEXP,
	IncomingMessage,
	isFlatHeaderList,
	isLoopbackRequestHost,
	isRawSocketRequest,
	isSocketReadyForProtocol,
	joinHeaderValue,
	nextServerId,
	...
	readUndiciReadableBody,
	Server,
	ServerCallable,
	ServerIncomingMessage,
	ServerResponseBridge,
	ServerResponseCallable,
	serializeHeaderValue,
	serializeLoopbackResponse,
	serializeRawHeaderPairs,
	serializeRawHttpRequest,
	serverInstances,
	socketReadyEventNameForProtocol,
	splitTransferEncodingTokens,
	UpgradeSocket,
	upgradeSocketInstances,
	validateHeaderName,
	validateHeaderValue,
	validateRequestMethod,
	validateRequestPath,
	waitForRawHttpResponse,
	waitForRawHttpResponseHead,
