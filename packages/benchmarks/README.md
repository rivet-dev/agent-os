...
The committed local baseline is `packages/benchmarks/results/baseline-local.json`. It is generated from the full latency matrix and records:
...
```bash
pnpm install --frozen-lockfile --filter "@rivet-dev/agentos-benchmarks..."
cargo build --release -p agentos-sidecar
cargo build --release -p agentos-benchmark-baseline
cargo build --release -p agentos-benchmark-baseline --target wasm32-wasip1
...
AGENTOS_SIDECAR_BIN=/abs/path/to/agentos-sidecar
```
