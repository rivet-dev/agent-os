...
const SIDECAR_BUILD_INPUTS = [
	...
	path.join(REPO_ROOT, "crates/driver-tokio"),
	...
	path.join(REPO_ROOT, "crates/resource-accounting"),
	path.join(REPO_ROOT, "crates/rivetkit-ars-client"),
	...
	path.join(REPO_ROOT, "crates/vfs-storage"),
...
export class NodeFileSystem implements VirtualFileSystem {
	...
	async chown(
		...
		const normalized = this.normalizeTarget(targetPath);
		if (options?.followSymlinks === false) {
			await fs.lchown(normalized, uid, gid);
		} else {
			await fs.chown(normalized, uid, gid);
		}
	}
...
function normalizeFsPermissionScope(
	scope: FsPermissions | undefined,
): PermissionsPolicy["fs"] {
...
function normalizePatternPermissionScope(
	scope:
		| NetworkPermissions
...
function normalizePermissionsPolicy(
...
class DeferredFileSystem implements VirtualFileSystem {
	...
	chown(
		...
		return this.filesystem().chown(path, uid, gid, options);
	}
	utimes(path: string, atime: number, mtime: number): Promise<void> {
...
class NativeKernel implements Kernel {
	...
	private async initialize(): Promise<void> {
		...
		if (this.loopbackExemptPorts.length > 0) {
			createVmEnv.AGENTOS_LOOPBACK_EXEMPT_PORTS = JSON.stringify(
