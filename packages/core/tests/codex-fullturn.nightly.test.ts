...
import { wasmBackendTestTimeout } from "@rivet-dev/agentos-test-harness";
...
const CODEX_EXEC_TIMEOUT_MS = wasmBackendTestTimeout(45_000, 120_000);
const CODEX_TEST_TIMEOUT_MS = wasmBackendTestTimeout(70_000, 150_000);
...
describe.skipIf(!hasCodexExecArtifact)(
	...
	() => {
		test(
			"codex-exec --session-turn completes a model turn end-to-end",
			async () => {
				...
				expect(
					stdout,
					`codex-exec did not emit its start event; exitCode=${exitCode}; stderr=${stderr}; requests=${requests.length}`,
		...
		test(
			"emits only the final unphased assistant snapshot",
			async () => {
		...
		test(
			"runs a shell tool call with on-request approval and reports tool_call updates",
			async () => {
		...
		test(
			"shell tool runs a REAL subprocess with an observable filesystem side effect",
			async () => {
				...
				const sourcePath = `${workspace}/codex-source.txt`;
				const outPath = `${workspace}/codex-side-effect.txt`;
				...
				try {
					...
					let written: string;
					try {
						written = new TextDecoder().decode(await vm.readFile(outPath));
					} catch (error) {
						throw new Error(
							`Codex tool subprocess did not create ${outPath}; exitCode=${r.exitCode}; stderr=${r.stderr}; stdout=${r.stdout}`,
							{ cause: error },
						);
					}
		...
		test(
			"replays adapter-supplied history on a resumed multi-turn session",
			async () => {
