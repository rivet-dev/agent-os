...
describe("sidecar process client", () => {
	...
	test("dispatches BARE sidecar_request frames to the registered handler", async () => {
		...
		writeFileSync(
			...
			[
				...
				"const schema = { name: 'agentos-sidecar', version: 8 };",
	...
	test("NativeKernel refreshes zombieTimerCount from the sidecar proxy", async () => {
		const zombieTimerCount = vi
			.spyOn(SidecarProcess.prototype, "getZombieTimerCount")
	...
	test("NativeKernel exposes symlinked node_modules passthrough directories", async () => {
		...
		const kernel = createKernel({
			...
			user: {
				...
				euid: process.geteuid?.() ?? process.getuid?.() ?? 1000,
				egid: process.getegid?.() ?? process.getgid?.() ?? 1000,
			},
		});
	...
	test("connectTerminal forwards host stdin and output on the sidecar path", async () => {
		const kernel = createKernel({
			filesystem: createInMemoryFileSystem(),
			permissions: ALLOW_ALL_VM_PERMISSIONS,
		});
