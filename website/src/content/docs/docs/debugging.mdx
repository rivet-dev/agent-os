...
It's a VM-level option covering every session's agent process; if omitted, chunks are written to the host `process.stderr` by default. See [Agent logs](#agent-logs-onagentstderr).
