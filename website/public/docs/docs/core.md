...
| Agent-to-agent | Custom | Built into [Rivet Actors](/docs/agent-to-agent) |
| Authentication | Set up yourself | [Docs](/docs/authentication) |
...
- This keeps per-VM memory in the tens of MB and warm creation in single-digit
  ms (see [Performance](/docs/performance)).
...
- The [`agentOS()` actor](/docs/quickstart) accepts the same options and layers
  persistence, sleep/wake, and preview URLs on top.

See [Mounts](#mounts) and [Software](/docs/software).
...
- Action timeouts and automatic sleep/wake are [`agentOS()` actor](/docs/quickstart)
  features, not the direct VM API.
- A direct VM stays alive until `dispose()`. See
  [Persistence & Sleep](/docs/persistence).