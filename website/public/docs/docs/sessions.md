...
- The [inspector](/docs/inspector)'s Transcript tab renders the same session stream live, with a composer for prompting sessions by hand.
...
- See the [agent guide](/docs/agents/pi) for adapter specifics.

## Agent Plugins

[Agent Plugins](https://agent-plugins.org/) are portable bundles of skills and
optional MCP servers described by a root `plugin.json`. agentOS leaves plugin
loading to the session's agent harness: install the bundle in that client's VM
location before opening the session. This lets the same bundle work with other
[compatible clients](https://agent-plugins.org/compatible-clients) without an
agentOS-specific plugin format.

This example installs and enables a small Agent Plugin for Codex, then invokes
its namespaced skill:
