...
use agentos_driver_tokio::readiness::{
...
use agentos_driver_tokio::{DriverHandle, VmExecutorPermit};
#[cfg(not(test))]
use agentos_resource_accounting::queue_tracker::warn_limit_exhausted;
use agentos_resource_accounting::queue_tracker::{register_queue, QueueGauge, TrackedLimit};
use agentos_vm_host_interface::{bridge_contract, BridgeCallConvention};
...
impl SessionReadiness {
    ...
    ) -> Result<Vec<agentos_driver_tokio::readiness::SignalObservation>, String> {
        self.broker
    ...
    fn refill_until(
        ...
        loop {
            ...
            if let Some((evicted_key, worker)) = evicted {
                ...
                eprintln!(
                    "agentos-executor-v8-runtime: warm worker evicted key={} heap={}",
            ...
            workers.push(worker);
            eprintln!(
                "agentos-executor-v8-runtime: warm worker refilled key={} heap={} pool_size={}",
                warm_key_prefix(&key),
                key.heap_limit_mb,
                workers.len()
            );
...
fn spawn_warm_worker(
    ...
    let join_handle = match thread::Builder::new()
        ...
        Ok(handle) => handle,
        Err(error) => {
            eprintln!("agentos-executor-v8-runtime: warm worker spawn failed: {error}");
            return None;
        }
    };
    ...
    match ready_rx.recv() {
        ...
        Ok(Err(error)) => {
            eprintln!(
                "agentos-executor-v8-runtime: warm worker refill failed key={} heap={}: {error}",
                warm_key_prefix(&key),
                key.heap_limit_mb
            );
...
struct SessionEntry {
    ...
    ready_broker: Arc<SessionReadiness>,
    #[cfg(test)]
    session_resources: Arc<agentos_driver_tokio::accounting::ResourceLedger>,
}
...
impl SessionManager {
    ...
    fn claim_warm_worker(
        ...
        let Some(worker) = self.warm_pool.claim(&key) else {
            record_warm_worker_miss();
            eprintln!(
                "agentos-executor-v8-runtime: warm worker pool-empty key={} heap={}",
                warm_key_prefix(&key),
                key.heap_limit_mb
            );
            return Err(assignment);
        ...
        match worker.assignment_tx.send(assignment) {
            Ok(()) => {
                ...
                eprintln!(
                    "agentos-executor-v8-runtime: warm worker claimed key={} heap={}",
                    warm_key_prefix(&key),
                    key.heap_limit_mb
                );
    ...
    pub fn session_resources(
        &self,
        session_id: &str,
    ) -> Option<Arc<agentos_driver_tokio::accounting::ResourceLedger>> {
        self.sessions
            .get(session_id)
...
fn session_thread(
    ...
    let execution_task_owner =
        output_generation.map(|generation| agentos_driver_tokio::TaskOwner::Vm { generation });
    #[cfg(test)]
    let _ = (
        heap_limit_mb,
        cpu_time_limit_ms,
                        ...
                        if v8_isolate.is_none() {
                            ...
                            let snapshot_blob = match snapshot_cache.get_or_create_with_userland(
                                ...
                                Err(message) => {
                                    ...
                                    eprintln!(
                                        "agentos-executor-v8-runtime: snapshot creation failed, \
                            ...
                            let mut iso = match snapshot_blob {
                                Some(blob) => {
                                    from_snapshot = true;
                                    eprintln!(
                                        "agentos-executor-v8-runtime: restored session isolate from_snapshot=true"
                                    );
                                    let phase_start = Instant::now();
...
mod tests {
    ...
    fn configured_executor_and_command_bounds_drive_session_manager() {
        ...
        let mut config = agentos_driver_tokio::DriverConfig {
    ...
    fn bridge_contract_function_partitions_cover_contract() {
        ...
        assert_eq!(
            ...
            "sync bridge function partition drifted from crates/vm-host-interface/vm-host-interface.json"
        );
        assert_eq!(
            registered_async, expected_async,
            "async bridge function partition drifted from crates/vm-host-interface/vm-host-interface.json"
        );
        assert!(
    ...
    fn admitted_session_command_precedes_later_signal_wake() {
        ...
        assert_eq!(
            ...
            vec![agentos_driver_tokio::readiness::SignalObservation {
                signal: 15,
                delivery_token: 29,
            }]
        );
