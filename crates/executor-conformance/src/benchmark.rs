...
impl JavascriptBenchmarkReport {
    ...
    pub fn render_markdown_with_comparison(
        ...
        let _ = writeln!(&mut markdown, "# agentos Node Import Benchmark");
        ...
        let _ = writeln!(
            &mut markdown,
            "- Reproduce: `cargo run -p agentos-executor-conformance --bin node-import-bench -- --iterations {} --warmup-iterations {}`",
            self.config.iterations, self.config.warmup_iterations
        );
        let _ = writeln!(&mut markdown);
    ...
    fn json_artifact<'a>(
        ...
        JavascriptBenchmarkArtifact {
            ...
            command: format!(
                "cargo run -p agentos-executor-conformance --bin node-import-bench -- --iterations {} --warmup-iterations {}",
                self.config.iterations, self.config.warmup_iterations
            ),
            config: &self.config,
            host: &self.host,
...
impl BenchmarkWorkspace {
    fn create(repo_root: &Path) -> Result<Self, JavascriptBenchmarkError> {
        let root = repo_root.join(format!(
            ".tmp-agentos-executor-conformance-bench-{}-{}",
            std::process::id(),
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap_or_default()
