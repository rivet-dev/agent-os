...
use super::engine::{WasmtimeEngineHandle, WasmtimeEngineProfile, WasmtimeEngineRegistry};
use super::limits;
use super::linker;
...
impl WasmtimeExecution {
    pub fn spawn(
        ...
        if threaded && !cfg!(feature = "threads") {
            return Err(WasmExecutionError::Host(
                HostServiceError::new(
                    "ERR_AGENTOS_EXECUTOR_NOT_COMPILED",
                    "the Wasmtime threads executor was not compiled into this binary",
                )
                .with_details(serde_json::json!({
                    "executor": "wasm-wasmtime-threads",
                    "feature": "threads",
                })),
            ));
        }
