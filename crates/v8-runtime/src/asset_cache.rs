...
pub const NODE_IMPORT_CACHE_DEBUG_ENV: &str = "AGENTOS_NODE_IMPORT_CACHE_DEBUG";
pub const NODE_IMPORT_CACHE_METRICS_PREFIX: &str = "__AGENTOS_NODE_IMPORT_CACHE_METRICS__:";
pub const NODE_IMPORT_CACHE_ASSET_ROOT_ENV: &str = "AGENTOS_NODE_IMPORT_CACHE_ASSET_ROOT";
...
#[derive(Debug)]
pub struct NodeImportCacheCleanup {
    root_dir: PathBuf,
}
...
impl NodeImportCache {
    ...
    pub fn cleanup_guard(&self) -> Arc<NodeImportCacheCleanup> {
        Arc::clone(&self.cleanup)
    }

    #[cfg_attr(not(test), allow(dead_code))]
    pub fn python_runner_path(&self) -> &Path {
        &self.python_runner_path
    }

    #[cfg(test)]
    pub fn timing_bootstrap_path(&self) -> &Path {
        &self.timing_bootstrap_path
    }

    pub fn wasm_runner_path(&self) -> &Path {
    ...
    pub fn pyodide_dist_path(&self) -> &Path {
        &self.pyodide_dist_path
    }

    pub fn prewarm_marker_dir(&self) -> &Path {
        &self.prewarm_marker_dir
    }

    pub fn shared_compile_cache_dir(&self) -> PathBuf {
        self.root_dir.join("compile-cache")
    }

    pub fn ensure_materialized_with_runtime(
        &self,
    ...
    pub fn ensure_materialized_with_timeout_and_runtime(
        &self,
        runtime: &RuntimeContext,
        timeout: Duration,
    ) -> Result<(), io::Error> {
    ...
    pub async fn ensure_materialized_with_timeout_and_runtime_async(
        &self,
        runtime: &RuntimeContext,
        timeout: Duration,
    ) -> Result<(), io::Error> {
    ...
    #[cfg(test)]
    pub fn ensure_materialized(&self) -> Result<(), io::Error> {
        let runtime = crate::test_runtime_context();
        self.ensure_materialized_with_runtime(&runtime)
    }

    #[cfg(test)]
    pub fn ensure_materialized_with_timeout(&self, timeout: Duration) -> Result<(), io::Error> {
        let runtime = crate::test_runtime_context();
