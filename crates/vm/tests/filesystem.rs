...
mod host_dir {
    ...
    mod tests {
        ...
        use agentos_vm_kernel::fd_table::O_RDWR;
        ...
        ) -> agentos_vm_kernel::kernel::KernelProcessHandle {
            kernel
                .spawn_process(
                    "sh",
                    Vec::new(),
        ...
        fn filesystem_host_dir_metadata_ops_reject_symlink_escape_targets() {
            let host_dir = temp_dir("agentos-vm-filesystem-host-dir");
            let outside_dir = temp_dir("agentos-vm-filesystem-host-dir-outside");
        ...
        fn filesystem_host_dir_write_file_with_mode_honors_requested_permissions() {
            let host_dir = temp_dir("agentos-vm-filesystem-host-dir-write-mode");
        ...
        fn filesystem_host_dir_recursive_mkdir_with_mode_honors_requested_permissions() {
            let host_dir = temp_dir("agentos-vm-filesystem-host-dir-mkdir-mode");
        ...
        fn filesystem_host_dir_mkdir_existing_mount_root_returns_eexist() {
            let host_dir = temp_dir("agentos-vm-filesystem-host-dir-mkdir-root");
        ...
        fn filesystem_host_dir_stat_preserves_nanosecond_timestamp_precision() {
            let host_dir = temp_dir("agentos-vm-filesystem-host-dir-stat");
        ...
        fn filesystem_host_dir_utimes_spec_honors_omit_and_now_controls() {
            let host_dir = temp_dir("agentos-vm-filesystem-host-dir-utimes-spec");
            let tracked_file = host_dir.join("tracked.txt");
            fs::write(&tracked_file, b"tracked").expect("seed tracked file");
        ...
        fn filesystem_host_dir_lutimes_updates_symlink_without_touching_target() {
            let host_dir = temp_dir("agentos-vm-filesystem-host-dir-lutimes");
        ...
        fn kernel_futimes_updates_host_dir_mount_with_nanosecond_precision() {
            let host_dir = temp_dir("agentos-vm-filesystem-host-dir-futimes");
            let tracked_file = host_dir.join("tracked.txt");
            fs::write(&tracked_file, b"tracked").expect("seed tracked file");
...
mod kernel_authority {
    ...
    fn create_test_sidecar() -> agentos_vm::VmManager<RecordingBridge> {
    ...
    ) -> agentos_vm::wire::GuestFilesystemResultResponse {
    ...
    fn guest_lstat(
        ...
        request_id: i64,
        path: &str,
    ) -> agentos_vm::wire::GuestFilesystemStat {
    ...
    #[test]
    fn kernel_rename_moves_a_broken_symlink_without_recreating_its_source() {
        let mut sidecar = create_test_sidecar();
        let (connection_id, session_id) = authenticate_and_open_session(&mut sidecar);
        let cwd = temp_dir("filesystem-kernel-broken-symlink-rename-cwd");
        let (vm_id, _) = create_vm_wire(
            &mut sidecar,
            3,
            &connection_id,
    ...
    fn filesystem_cross_mount_rename_reports_exdev_to_js_and_falls_back_in_shell() {
        ...
        let host_dir = temp_dir("agentos-vm-cross-mount-rename-js");
