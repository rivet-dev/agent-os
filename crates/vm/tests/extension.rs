...
use agentos_vm::{Extension, ExtensionContext, ExtensionFuture, ExtensionResponse, VmError};
use agentos_vm_host_interface::{HostPersistence, LoadFilesystemStateRequest};
use support::{
    ...
    fn handle_request<'a>(
        ...
        Box::pin(async move {
            ...
            let payload = String::from_utf8(payload).map_err(|error| {
                VmError::InvalidState(format!("invalid extension test entrypoint: {error}"))
            })?;
            let mut payload_lines = payload.lines();
            let entrypoint = payload_lines
                .next()
                .ok_or_else(|| {
                    VmError::InvalidState(String::from("missing extension process entrypoint"))
                })?
                .to_string();
            let lifecycle_entrypoint = payload_lines
                .next()
                .ok_or_else(|| {
                    VmError::InvalidState(String::from("missing extension lifecycle entrypoint"))
                })?
                .to_string();
            ...
            let stdout = String::from_utf8(stdout).map_err(|error| {
                VmError::InvalidState(format!("invalid extension process stdout: {error}"))
            })?;
