...
/// Resolve the workspace root from `CARGO_MANIFEST_DIR` (which points at `crates/vm`).
fn workspace_root() -> PathBuf {
    let manifest = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    manifest
        .parent()
        .and_then(Path::parent)
        .expect("crates/vm has a workspace root two levels up")
        .to_path_buf()
}
...
fn limit_constants_are_classified() {
        ...
        if !inventory_keys.contains(&key) {
            failures.push(format!(
                "unclassified limit constant {} in {}: wire it through a typed configuration field and mark it \
                 ...
                 crates/vm/tests/fixtures/limits-inventory.json with a one-line rationale",
                c.name, c.path
            ));
        }
    }
    ...
    // through. Process-wide reactor and transport limits intentionally live in DriverConfig,
