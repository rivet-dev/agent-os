...
use agentos_rivetkit_ars_client::protocol as wire;
...
mod core {
    pub use agentos_vm::core::*;
}

// The included plugin only needs the mount context's runtime handle. Keep the
// integration test independent from the full sidecar service graph.
mod bridge {
    pub struct MountPluginContext<B> {
        pub runtime_context: agentos_driver_tokio::DriverHandle,
        ...
        large_metadata
            .create(
                ...
                CreateInodeAttrs::file(
                    ...
                    0,
                    0,
                    agentos_vfs_core::engine::types::Storage::Inline(vec![
                        7;
...
async fn migrations_are_independent_strict_and_atomic_over_actor_sqlite_uds() {
    ...
    assert!(matches!(
        ...
        Err(vm_sqlite::VmSqliteError::Actor(
            agentos_rivetkit_ars_client::ActorUdsError::Sql { .. }
        ))
    ));
