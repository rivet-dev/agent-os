...
use crate::{VmError, VmManagerHost};

use crate::core::permissions::filesystem_permission_capability;
use agentos_vfs_core::engine::engines::{ChunkedFs, ChunkedFsOptions};
use agentos_vfs_core::engine::mem::{InMemoryMetadataStore, MemoryBlockStore};
use agentos_vfs_storage::MountedEngineFileSystem;
use agentos_vm_host_interface::FilesystemAccess;
use agentos_vm_kernel::mount_plugin::{
...
use agentos_vm_kernel::vfs::{
    VfsError, VfsResult, VirtualDirEntry, VirtualFileSystem, VirtualStat,
};
#[cfg(test)]
...
impl<B> HostFilesystem<B> {
    ...
    fn vfs_error(error: VmError) -> VfsError {
    ...
    fn link_state_error() -> VfsError {
        VfsError::io("sidecar host filesystem link state lock poisoned")
    }
    ...
    fn read_dir(&mut self, path: &str) -> VfsResult<Vec<String>> {
        ...
        for linked_path in links.path_to_ino.keys() {
            ...
            if entries.iter().all(|entry| entry.name != name) {
                entries.push(agentos_vm_host_interface::DirectoryEntry {
                    name,
                    kind: FileKind::File,
                });
            }
...
) -> Result<FileSystemPluginRegistry<MountPluginContext<B>>, VmError>
where
    B: VmManagerHost + Send + 'static,
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
