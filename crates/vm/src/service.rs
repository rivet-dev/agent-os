...
pub(crate) const MAX_PENDING_SIDECAR_RESPONSES: usize =
    agentos_sidecar_protocol::config::DEFAULT_MAX_PENDING_RESPONSES;
#[cfg(test)]
#[allow(dead_code)]
pub(crate) const MAX_OUTBOUND_SIDECAR_REQUESTS: usize =
    agentos_sidecar_protocol::config::DEFAULT_MAX_OUTBOUND_REQUESTS;
#[cfg(test)]
#[allow(dead_code)]
pub(crate) const MAX_COMPLETED_SIDECAR_RESPONSES: usize =
    agentos_sidecar_protocol::config::DEFAULT_MAX_COMPLETED_RESPONSES;
pub(crate) fn process_event_queue_overflow_error(limit: usize) -> VmError {
    VmError::host("ERR_AGENTOS_PROCESS_EVENT_LIMIT", format!("process event queue exceeded {limit} pending events; raise runtime.protocol.maxProcessEvents"
    ))
}

fn sidecar_response_pending_overflow_error(limit: usize) -> VmError {
    VmError::host("ERR_AGENTOS_PENDING_RESPONSE_LIMIT", format!("sidecar response tracker exceeded {limit} pending responses; raise runtime.protocol.maxPendingResponses"
    ))
}

fn outbound_sidecar_request_queue_overflow_error(limit: usize) -> VmError {
    VmError::host("ERR_AGENTOS_OUTBOUND_REQUEST_LIMIT", format!("outbound sidecar request queue exceeded {limit} pending requests; raise runtime.protocol.maxOutboundRequests"
    ))
}
...
fn parse_legacy_javascript_child_process_spawn_options(
    ...
    raw_options: &str,
) -> Result<LegacyJavascriptChildProcessSpawnOptions, VmError> {
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    pub(crate) fn with_mut<T>(
        ...
        let mut bridge = self
            .inner
            .lock()
            .map_err(|_| VmError::Bridge(String::from("sidecar bridge lock poisoned")))?;
    ...
    pub(crate) fn queue_set_vm_permissions_result(
        ...
        let mut outcomes = self.set_vm_permissions_outcomes.lock().map_err(|_| {
            VmError::Bridge(String::from(
                "sidecar test set_vm_permissions outcome lock poisoned",
            ))
        })?;
    ...
    pub(crate) fn emit_lifecycle(&self, vm_id: &str, state: LifecycleState) -> Result<(), VmError> {
        #[cfg(test)]
        {
            let outcome = self
                ...
                .lock()
                .map_err(|_| {
                    VmError::Bridge(String::from("sidecar test lifecycle outcome lock poisoned"))
                })?
                .pop_front();
    ...
    pub(crate) fn require_resolved_network_access(
        ...
        let permissions = self
            .permissions
            .lock()
            .map_err(|_| VmError::Bridge(String::from("sidecar permission policy lock poisoned")))?
            .get(vm_id)
            .cloned();
    ...
    pub(crate) fn set_vm_permissions(
        ...
        let mut stored = self.permissions.lock().map_err(|_| {
            VmError::Bridge(String::from("sidecar permission policy lock poisoned"))
        })?;
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    ...
    pub fn with_driver_and_executors(
        ...
        config.protocol.validate().map_err(VmError::InvalidState)?;
        ...
        let protocol_limits = config.protocol.clone();
    ...
    pub(crate) fn check_pending_process_event_capacity(
        ...
        let global_limit = self.config.protocol.max_process_events;
        if self.pending_process_events.len() >= global_limit {
            return Err(process_event_queue_overflow_error(global_limit));
        }
        let defaults = crate::core::limits::ProcessLimits::default();
        let limits = self
            .vms
            .get(&envelope.vm_id)
    ...
    pub async fn dispatch(&mut self, request: RequestFrame) -> Result<DispatchResult, VmError> {
        ...
        if !matches!(&route, RequestRoute::DisposeVm(_)) {
            if let OwnershipScope::VmOwnership(ownership) = &request.ownership {
                ...
                {
                    let error = VmError::host("ERR_AGENTOS_VM_TASK_FAILED", format!("vm_id={} class={:?} owner={} reason={:?}; dispose and recreate this VM generation",
    ...
    pub fn queue_sidecar_request(
        ...
        let outbound_limit = self.config.protocol.max_outbound_requests;
        if self.outbound_sidecar_requests.len() >= outbound_limit {
            return Err(outbound_sidecar_request_queue_overflow_error(
                outbound_limit,
            ));
        }
        let pending_limit = self.config.protocol.max_pending_responses;
    ...
    pub fn accept_sidecar_response(
        ...
        let completed_limit = self.config.protocol.max_completed_responses;
    ...
    fn vm_acp_limits<'a>(
        ...
        Box::pin(async move {
            ...
            self.vms
                ...
                .ok_or_else(|| VmError::InvalidState(format!("VM not found: {vm_id}")))
        })
    }
...
pub(crate) fn vfs_error(error: VfsError) -> VmError {
    VmError::host(error.code(), error.message())
}
