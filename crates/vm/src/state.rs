...
use agentos_driver_tokio::fairness::FairWorkTurn;
use agentos_driver_tokio::DriverHandle;
use agentos_resource_accounting::queue_tracker::{self, QueueGauge, TrackedLimit};
use agentos_vm_config as vm_config;
use agentos_vm_config::PermissionsPolicy;
use agentos_vm_host_interface::{FilesystemSnapshot, VmHostTypes};
use agentos_vm_kernel::fd_table::TransferredFd;
use agentos_vm_kernel::kernel::{KernelProcessHandle, KernelVm};
use agentos_vm_kernel::mount_table::MountTable;
use agentos_vm_kernel::root_fs::RootFilesystemMode;
use agentos_vm_kernel::socket_table::SocketId;
...
pub(crate) type BridgeError<B> = <B as VmHostTypes>::Error;
...
pub(crate) struct SocketDescriptionLease {
    lease: Mutex<Option<Arc<agentos_driver_tokio::capability::CapabilityLease>>>,
}

impl SocketDescriptionLease {
    pub(crate) fn retain(&self, lease: Arc<agentos_driver_tokio::capability::CapabilityLease>) {
...
impl SocketFairnessRetirement {
    pub(crate) fn new(identity: Arc<OnceLock<(u64, u64)>>, runtime: DriverHandle) -> Arc<Self> {
...
pub(crate) const EXECUTION_DRIVER_NAME: &str = "agentos-vm-execution";
...
impl Default for VmManagerConfig {
    fn default() -> Self {
        Self {
            instance_id: String::from("agentos-vm"),
...
impl From<crate::executor::backend::HostServiceError> for VmError {
    fn from(error: crate::executor::backend::HostServiceError) -> Self {
        Self::Host(error)
    }
}

impl From<crate::core::SidecarCoreError> for VmError {
    fn from(error: crate::core::SidecarCoreError) -> Self {
        match error.code() {
...
pub(crate) fn guest_limit_message(limit: &agentos_driver_tokio::accounting::LimitError) -> String {
...
impl From<agentos_driver_tokio::accounting::LimitError> for VmError {
    fn from(error: agentos_driver_tokio::accounting::LimitError) -> Self {
        Self::ResourceLimit(error)
    }
}

impl From<agentos_driver_tokio::capability::CapabilityError> for VmError {
    fn from(error: agentos_driver_tokio::capability::CapabilityError) -> Self {
        match error {
            agentos_driver_tokio::capability::CapabilityError::Limit(limit) => {
...
impl From<agentos_driver_tokio::TaskSpawnError> for VmError {
    fn from(error: agentos_driver_tokio::TaskSpawnError) -> Self {
        match error {
            agentos_driver_tokio::TaskSpawnError::ResourceLimit(limit) => {
                Self::ResourceLimit(limit)
            }
            agentos_driver_tokio::TaskSpawnError::AdmissionClosed { scope } => Self::Execution(
...
impl From<agentos_driver_tokio::BlockingJobError> for VmError {
    fn from(error: agentos_driver_tokio::BlockingJobError) -> Self {
        match error {
            agentos_driver_tokio::BlockingJobError::ResourceLimit(limit) => {
                Self::ResourceLimit(limit)
            }
            other => Self::Execution(other.to_string()),
        }
...
pub trait EventSinkTransport: Send + Sync {
    fn emit_event(&self, event: crate::wire::EventFrame) -> Result<(), VmError>;
}
...
impl SharedEventSink {
    ...
    /// `VmManager` with no stdout loop) so the caller can fall back to the
...
pub(crate) struct VmState {
    ...
    /// Child of the one process ledger owned by DriverHandle.
    pub(crate) resources: Arc<agentos_driver_tokio::accounting::ResourceLedger>,
...
pub(crate) struct VmFetchStreamState {
    ...
    pub(crate) _capability: agentos_driver_tokio::capability::CapabilityLease,
...
pub(crate) struct ActiveProcess {
    ...
    /// receiver.
    pub(crate) runtime_control: agentos_vm_kernel::process_runtime::RuntimeControlReceiver,
    ...
    pub(crate) pending_runtime_exit: Option<agentos_vm_kernel::process_runtime::ProcessExit>,
    ...
    pub(crate) capability_leases:
        BTreeMap<NativeCapabilityKey, Arc<agentos_driver_tokio::capability::CapabilityLease>>,
    ...
    /// each VM starts a fresh PID namespace, while the sidecar process
...
pub(crate) struct Http2SharedState {
    ...
    pub(crate) capability_leases:
        BTreeMap<NativeCapabilityKey, agentos_driver_tokio::capability::CapabilityLease>,
...
pub(crate) enum Http2SessionCommand {
    Request {
        headers_json: String,
        options_json: String,
        pending_capability: agentos_driver_tokio::capability::PendingCapability,
...
pub(crate) enum TcpSocketEvent {
    Data {
        bytes: Vec<u8>,
        reservation: agentos_driver_tokio::accounting::SharedReservation,
        /// Protocol-specific ownership that remains live until the payload is
        /// transferred out of the transport/event layer.
        source_reservations: Vec<agentos_driver_tokio::accounting::SharedReservation>,
    },
    End,
    Close {
        had_error: bool,
...
pub(crate) struct SocketEventPusher {
    ...
    pub(crate) capability_id: agentos_driver_tokio::capability::CapabilityId,
    pub(crate) capability_generation: agentos_driver_tokio::capability::CapabilityGeneration,
    live: Arc<AtomicBool>,
...
impl SocketEventPusher {
    ...
    pub(crate) fn publish_readiness(
        &self,
        flags: agentos_driver_tokio::readiness::ReadyFlags,
...
impl SocketReadinessRegistration {
    ...
    pub(crate) fn register(
        ...
        owner_notify: Arc<Notify>,
        replay_flags: agentos_driver_tokio::readiness::ReadyFlags,
    ) {
    ...
    pub(crate) fn set_application_read_interest(&self, enabled: bool) -> Result<bool, VmError> {
        let identity = {
...
pub(crate) enum NativeTlsCommand {
    ...
    Shutdown {
        _command_reservation: agentos_driver_tokio::accounting::SharedReservation,
...
pub(crate) struct PlainSocketWritePayload {
    pub(crate) bytes: Vec<u8>,
    pub(crate) _command_reservation: agentos_driver_tokio::accounting::SharedReservation,
    pub(crate) _bytes_reservation: agentos_driver_tokio::accounting::SharedReservation,
    pub(crate) _buffered_reservation: agentos_driver_tokio::accounting::SharedReservation,
}
...
pub(crate) struct TlsWritePayload {
    pub(crate) bytes: Vec<u8>,
    pub(crate) _command_reservation: agentos_driver_tokio::accounting::SharedReservation,
    pub(crate) _command_bytes_reservation: agentos_driver_tokio::accounting::SharedReservation,
    pub(crate) _buffered_reservation: agentos_driver_tokio::accounting::SharedReservation,
    pub(crate) _tls_reservation: agentos_driver_tokio::accounting::SharedReservation,
}
...
pub(crate) struct LoopbackTlsTransportPairState {
    ...
    pub(crate) lower_to_higher_reservations:
        VecDeque<agentos_driver_tokio::accounting::Reservation>,
...
impl UdpFamily {
    pub(crate) fn from_socket_type(value: &str) -> Result<Self, VmError> {
        match value {
            "udp4" => Ok(Self::Ipv4),
            "udp6" => Ok(Self::Ipv6),
...
pub(crate) enum DatagramEvent {
    Message {
        ...
        _byte_reservation: agentos_driver_tokio::accounting::SharedReservation,
        _datagram_reservation: agentos_driver_tokio::accounting::SharedReservation,
        _udp_byte_reservation: agentos_driver_tokio::accounting::SharedReservation,
        _udp_datagram_reservation: agentos_driver_tokio::accounting::SharedReservation,
    },
    Error {
        code: Option<String>,
        message: String,
...
impl From<DeferredRpcError> for VmError {
...
mod async_completion_tests {
    use super::*;
    use agentos_driver_tokio::accounting::ResourceLimit;

    fn completion_runtime(maximum: usize, generation: u64) -> (DriverHandle, Arc<ResourceLedger>) {
    ...
    fn completion_runtime_with_limits(
        ...
        byte_maximum: usize,
        generation: u64,
    ) -> (DriverHandle, Arc<ResourceLedger>) {
        let process = agentos_driver_tokio::TokioDriver::process(
...
mod socket_readiness_registry_tests {
    ...
    fn retired_socket_subscription_drops_cloned_late_end_wake() {
        ...
        assert!(!late_transport_target
            .publish_readiness(agentos_driver_tokio::readiness::ReadyFlags::END)
    ...
    fn socket_subscription_without_executor_wake_notifies_posix_poll_owner() {
        ...
        assert!(target
            .publish_readiness(agentos_driver_tokio::readiness::ReadyFlags::READABLE)
    ...
    fn native_alias_registration_is_raii_and_read_interest_is_aggregate_or() {
        let process_runtime = agentos_driver_tokio::TokioDriver::process(
            &agentos_driver_tokio::DriverConfig::default(),
        )
        ...
        let host =
            V8RuntimeHost::spawn(&process_runtime.handle()).expect("spawn subscriber test V8 host");
        let session = ExecutionWakeHandle::new(
            crate::executor::backend::ExecutionWakeIdentity {
                generation: 1,
                pid: 1,
