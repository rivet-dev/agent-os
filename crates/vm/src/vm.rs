...
use crate::core::permissions::{allow_all_policy, deny_all_policy};
use crate::core::{
...
use agentos_vm_kernel::mount_table::{MountOptions, MountTable, MountedFileSystem};
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    ...
    pub(crate) async fn configure_vm(
        ...
        let snapshot_runtime_context = self.runtime_context.as_ref().cloned().ok_or_else(|| {
            VmError::host(
                "ERR_AGENTOS_RUNTIME_UNAVAILABLE",
                String::from("snapshot pre-warm requires DriverHandle"),
            )
        })?;
        let vm = self.vms.get_mut(&vm_id).expect("owned VM should exist");
    ...
    pub(crate) async fn provided_commands(
        ...
        let vm = self
            .vms
            .get(&vm_id)
            .ok_or_else(|| VmError::host("ESTALE", "VM disappeared during command lookup"))?;
        let packages = vm
            .provided_commands
            .iter()
