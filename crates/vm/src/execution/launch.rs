...
fn resolve_execute_request(
    ...
    let runtime = payload.runtime.clone().ok_or_else(|| {
        VmError::InvalidState(String::from("execute requires either command or runtime"))
    })?;
...
fn resolve_command_execution(
    ...
    .ok_or_else(|| {
        VmError::InvalidState(format!("command not found on sidecar path: {command}"))
    })?;
        ...
        match rest.first().map(String::as_str) {
            Some("-c") => {
                entrypoint = rest.get(1).cloned().ok_or_else(|| {
                    VmError::InvalidState(String::from("argument expected for the -c option"))
                })?;
            ...
            Some("-m") => {
                let name = rest.get(1).cloned().ok_or_else(|| {
                    VmError::InvalidState(String::from("argument expected for the -m option"))
                })?;
                module = Some(name);
...
fn parse_javascript_shebang(
...
) -> Result<Option<(String, Vec<String>)>, VmError> {
    const MAX_SHEBANG_LINE_BYTES: usize = 256;
    ...
    let text = std::str::from_utf8(line)
        .map_err(|_| VmError::host("ENOEXEC", format!("invalid shebang line: {script_path}")))?;
...
fn parse_linux_shebang(header: &[u8], path: &str) -> Result<Option<LinuxShebang>, VmError> {
    if !header.starts_with(b"#!") {
        return Ok(None);
    }
    ...
    let line_end = line
        ...
        .map(|index| index + 1)
        .ok_or_else(|| VmError::host("ENOEXEC", format!("invalid shebang line: {path}")))?;
    ...
    let interpreter = std::str::from_utf8(&interpreter_tail[..interpreter_end])
        .map_err(|_| VmError::host("ENOEXEC", format!("invalid shebang line: {path}")))?;
    if interpreter.is_empty() {
        return Err(VmError::host(
            "ENOEXEC",
...
fn resolve_posix_spawn_path_candidate(
    ...
    command: &str,
    search_path: &str,
) -> Result<SpawnPathCandidate, VmError> {
    if command.is_empty() {
        return Err(VmError::host("ENOENT", "posix_spawnp command is empty"));
    }
...
fn resolve_live_registered_command_entrypoint(
    ...
    roots.sort();
    for root in roots {
        let candidate = normalize_path(&format!("/__agentos/commands/{root}/{command}"));
...
mod live_kernel_command_resolution_tests {
    ...
    use agentos_vm_kernel::vfs::{MemoryFileSystem, VirtualFileSystem};
...
pub(super) fn prepare_guest_runtime_env(
    ...
    env.insert(
        ...
        serde_json::to_string(&path_mappings).map_err(|error| {
            VmError::InvalidState(format!("failed to encode guest path mappings: {error}"))
        })?,
    );
    ...
    env.insert(
        ...
        .map_err(|error| VmError::InvalidState(format!("failed to encode read paths: {error}")))?,
    );
    env.insert(
        ...
        .map_err(|error| VmError::InvalidState(format!("failed to encode write paths: {error}")))?,
    );
    env.insert(
        String::from("AGENTOS_ALLOWED_NODE_BUILTINS"),
        serde_json::to_string(&allowed_node_builtins).map_err(|error| {
            VmError::InvalidState(format!("failed to encode allowed builtins: {error}"))
        })?,
    );
    ...
    if !loopback_exempt_ports.is_empty() {
        env.insert(
            ...
            serde_json::to_string(&loopback_exempt_ports).map_err(|error| {
                VmError::InvalidState(format!("failed to encode loopback exemptions: {error}"))
            })?,
        );
    }
...
mod runtime_guest_path_mapping_tests {
    ...
    fn host_node_modules_root_prefers_workspace_root_over_pnpm_package_node_modules() {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        let temp = std::env::temp_dir().join(format!("agentos-vm-node-modules-{unique}"));
    ...
    fn host_node_modules_root_preserves_symlinked_workspace_node_modules_path() {
        let unique = SystemTime::now()
            ...
            .expect("clock should be monotonic")
            .as_nanos();
        let temp = std::env::temp_dir().join(format!("agentos-vm-node-modules-symlink-{unique}"));
        let workspace_node_modules = temp.join("node_modules");
...
mod kernel_poll_sync_rpc_tests {
    ...
    use agentos_vm_kernel::permissions::Permissions;
    use agentos_vm_kernel::poll::{POLLHUP, POLLIN};
...
pub(super) fn stage_kernel_wasm_launch_asset(
    ...
    fs::write(&asset_path, image.bytes)
        .map_err(|error| VmError::Io(format!("failed to stage runtime launch image: {error}")))?;
...
fn nearest_guest_package_json_module_mode(
    ...
    requester_pid: u32,
    guest_path: &str,
) -> Result<Option<GuestPackageModuleMode>, VmError> {
...
fn host_launch_io_error(operation: &str, path: &Path, error: std::io::Error) -> VmError {
...
fn open_host_launch_source(
    path: PathBuf,
    maximum_bytes: u64,
) -> Result<OpenHostLaunchSource, VmError> {
...
fn host_launch_read_reservation(opened: &OpenHostLaunchSource) -> Result<usize, VmError> {
...
fn materialize_guest_launch_asset(
    ...
    if stat.is_symbolic_link {
        ...
        .map_err(kernel_error)?;
        std::os::unix::fs::symlink(&target, &asset_path)
            .map_err(|error| VmError::Io(format!("failed to stage launch symlink: {error}")))?;
        return Ok(());
    }

    if stat.is_directory {
        fs::create_dir_all(&asset_path).map_err(|error| {
            VmError::Io(format!("failed to create launch-asset directory: {error}"))
        })?;
    ...
    if let Some(parent) = asset_path.parent() {
        fs::create_dir_all(parent).map_err(|error| {
            VmError::Io(format!("failed to create launch-asset parent: {error}"))
        })?;
    }
    let owned_bytes;
    ...
    fs::write(&asset_path, bytes)
        .map_err(|error| VmError::Io(format!("failed to stage guest launch asset: {error}")))?;
    fs::set_permissions(&asset_path, fs::Permissions::from_mode(stat.mode & 0o7777)).map_err(
        |error| {
            VmError::Io(format!(
...
fn remove_existing_launch_asset(asset_path: &Path) -> Result<(), VmError> {
    match fs::symlink_metadata(asset_path) {
        ...
        Err(error) => Err(VmError::Io(format!(
...
fn rollback_failed_top_level_process_start(
    kernel: &mut SidecarKernel,
    kernel_handle: &agentos_vm_kernel::kernel::KernelProcessHandle,
