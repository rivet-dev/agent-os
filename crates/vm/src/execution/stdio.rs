...
fn socket_write_deadline_error(limit: Duration) -> VmError {
...
pub(crate) fn parse_kernel_stdin_read_args(
    request: &HostRpcRequest,
) -> Result<(usize, Option<u64>), VmError> {
    let max_bytes =
...
pub(super) fn service_javascript_kernel_tty_set_size_sync_rpc(
    ...
    let cols = u16::try_from(cols)
        .map_err(|_| VmError::Host(HostServiceError::new("EINVAL", "TTY columns exceed u16")))?;
    let rows = u16::try_from(rows)
        .map_err(|_| VmError::Host(HostServiceError::new("EINVAL", "TTY rows exceed u16")))?;
    kernel
...
pub(super) fn service_javascript_kernel_tcsetattr_sync_rpc(
    ...
    if cc.len() != 7 {
        return Err(VmError::Host(HostServiceError::new(
            "EINVAL",
            format!(
    ...
    kernel
        .tcsetattr(
            ...
            process.kernel_pid,
            fd,
            agentos_vm_kernel::pty::PartialTermios {
                ...
                cc: Some(agentos_vm_kernel::pty::PartialTermiosControlChars {
                    vintr: Some(parsed[0]),
                    vquit: Some(parsed[1]),
                    vsusp: Some(parsed[2]),
                    veof: Some(parsed[3]),
...
pub(crate) fn parse_kernel_poll_args(
    request: &HostRpcRequest,
) -> Result<(Vec<KernelPollFdRequest>, i32), VmError> {
    ...
    let timeout_ms = if request.args.get(1).is_some_and(Value::is_null) {
        ...
        i32::try_from(timeout_ms).map_err(|_| {
            VmError::InvalidState(String::from("__kernel_poll timeout ms must fit within i32"))
        })?
    };
...
{
    pub(crate) fn wake_ready_deferred_fd_reads(vm: &mut VmState) -> Result<(), VmError> {
    ...
    pub(crate) fn wake_ready_deferred_fd_writes(vm: &mut VmState) -> Result<(), VmError> {
        let kernel = &mut vm.kernel;
        for process in vm.active_processes.values_mut() {
    ...
    fn sidecar_owned_stdin_writer_is_nonblocking() {
        ...
        let flags = kernel
            .fd_fcntl(
                ...
                process.pid(),
                writer_fd,
                agentos_vm_kernel::fd_table::F_GETFL,
                0,
            )
            .expect("read stdin writer flags");

        assert_ne!(flags & agentos_vm_kernel::fd_table::O_NONBLOCK, 0);
    }
