...
pub(in crate::execution) fn decode_abstract_unix_name(hex: &str) -> Result<Vec<u8>, VmError> {
...
fn guest_unix_address_for_host_key(
    registry: &GuestUnixAddressRegistry,
    host_address_key: &str,
) -> Result<Option<GuestUnixAddress>, VmError> {
...
pub(in crate::execution) fn register_guest_unix_connection(
    registry: &GuestUnixAddressRegistry,
    target_binding_id: &str,
) -> Result<Arc<GuestUnixConnectionState>, VmError> {
    let mut registry = registry
        .lock()
        .map_err(|_| VmError::InvalidState(String::from("Unix address registry poisoned")))?;
...
pub(in crate::execution) fn rollback_guest_unix_path_binding(
    ...
    match (registry_error, marker_error) {
        ...
        (Some(registry_error), Some(marker_error)) => Err(VmError::Execution(format!(
...
impl ActiveUnixSocket {
    ...
    pub(in crate::execution) fn poll(
        ...
        if let Some(event) = self
            ...
            .lock()
            .map_err(|_| {
                VmError::InvalidState(String::from("Unix pending read event lock poisoned"))
            })?
            .take()
        {
        ...
        match self
            ...
            .lock()
            .map_err(|_| {
                VmError::InvalidState(String::from("Unix socket event channel lock poisoned"))
            })?
            .try_recv()
        {
    ...
    pub(in crate::execution) fn write_all(&self, contents: &[u8]) -> Result<usize, VmError> {
        let mut stream = self
            .stream
            .lock()
            .map_err(|_| VmError::InvalidState(String::from("Unix socket lock poisoned")))?;
    ...
    pub(in crate::execution) fn shutdown_write(&self) -> Result<(), VmError> {
        let stream = self
            .stream
            .lock()
            .map_err(|_| VmError::InvalidState(String::from("Unix socket lock poisoned")))?;
    ...
    pub(in crate::execution) fn close(&self) -> Result<(), VmError> {
        let stream = self
            .stream
            .lock()
            .map_err(|_| VmError::InvalidState(String::from("Unix socket lock poisoned")))?;
...
impl ActiveUnixListener {
    ...
    pub(in crate::execution) fn bind_abstract_unlistened(
        ...
        _runtime_context: agentos_driver_tokio::DriverHandle,
    ) -> Result<Self, VmError> {
        Err(abstract_unix_unsupported())
    }
    ...
    pub(in crate::execution) fn poll(
        &mut self,
        wait: Duration,
    ) -> Result<Option<UnixListenerEvent>, VmError> {
        if let Some(event) = self
            .pending_event
            .lock()
            .map_err(|_| VmError::host("EIO", "Unix listener pending event lock poisoned"))?
            .take()
        {
            return Ok(Some(event));
        }
        ...
        match self
            ...
            .lock()
            .map_err(|_| {
                VmError::InvalidState(String::from("Unix listener event channel lock poisoned"))
            })?
            .try_recv()
        {
            Ok(event) => Ok(Some(event)),
...
mod transferred_unix_alias_transport_tests {
    ...
    fn exercise_surviving_unix_alias(close_sender: bool) {
        ...
        let event = runtime.tokio_handle().block_on(async {
...
fn push_listener_event(event_pusher: &Arc<SocketReadinessSubscribers>) {
    for target in event_pusher.targets() {
        if let Err(error) =
            target.publish_readiness(agentos_driver_tokio::readiness::ReadyFlags::ACCEPT)
        {
...
pub(in crate::execution) fn push_socket_event(
    ...
    let flags = match event {
        "data" => agentos_driver_tokio::readiness::ReadyFlags::READABLE,
        "end" => agentos_driver_tokio::readiness::ReadyFlags::END,
        "error" => agentos_driver_tokio::readiness::ReadyFlags::ERROR,
        "close" => agentos_driver_tokio::readiness::ReadyFlags::CLOSE,
        _ => {
            NET_TCP_TRACE_COUNTERS
