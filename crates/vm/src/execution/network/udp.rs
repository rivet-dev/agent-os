...
impl ActiveUdpPollHandle {
    ...
    pub(in crate::execution) async fn acquire_fair_turn(&self) -> Result<FairWorkTurn, VmError> {
    ...
    pub(in crate::execution) async fn poll_native_once(
        ...
        .await
        {
            Ok(Ok(result)) => result.map_err(VmError::from),
            Ok(Err(_)) => Err(VmError::host(
                "EPIPE",
...
pub(crate) fn reserve_udp_receive_buffer(
    ...
    capacity: usize,
) -> Result<(Vec<u8>, Reservation, Reservation, Reservation, Reservation), VmError> {
...
fn udp_send_service_response(result: ActiveUdpSendResult) -> HostServiceResponse {
    match result {
        ...
        ActiveUdpSendResult::Deferred { receiver } => HostServiceResponse::Deferred {
            receiver,
            timeout: None,
            task_class: agentos_driver_tokio::TaskClass::Udp,
        },
    }
}
...
fn udp_command_admission_error(
    ...
    match error {
        ...
        tokio::sync::mpsc::error::TrySendError::Closed(_) => {
            VmError::host("EBADF", "native UDP owner task is closed")
        }
    }
}

fn reserve_udp_command(resources: &ResourceLedger) -> Result<SharedReservation, VmError> {
    resources
...
fn reserve_udp_send_payload(
    resources: &ResourceLedger,
    contents: &[u8],
) -> Result<NativeUdpSendPayload, VmError> {
...
fn udp_deferred_error(error: VmError) -> crate::state::DeferredRpcError {
...
async fn run_native_udp_send_fair_step<F>(
    ...
    payload_len: usize,
    operation: F,
) -> Result<std::io::Result<usize>, VmError>
where
...
fn spawn_native_udp_owner(
    ...
    socket: UdpSocket,
    registration: NativeUdpOwnerRegistration,
) -> Result<TokioSender<NativeUdpCommand>, VmError> {
    ...
    runtime
        .spawn(agentos_driver_tokio::TaskClass::Udp, async move {
...
impl ActiveUdpSocket {
    ...
    async fn acquire_fair_turn(&self) -> Result<FairWorkTurn, VmError> {
        acquire_native_udp_fair_turn(
            &self.runtime_context,
            self.reactor_limits,
            &self.fairness_identity,
    ...
    pub(in crate::execution) fn bind(
        ...
        if self.native_commands.is_some() || self.guest_local_addr.is_some() {
            return Err(VmError::host(
                "EINVAL",
                String::from("agentos dgram socket is already bound"),
            ));
        }
    ...
    fn ensure_native_owner(&mut self) -> Result<&TokioSender<NativeUdpCommand>, VmError> {
        ...
        self.native_commands
            .as_ref()
            .ok_or_else(|| VmError::host("EBADF", "native UDP owner is unavailable"))
    }

    pub(in crate::execution) fn send_to<B>(
        &mut self,
        request: ActiveUdpSendToRequest<'_, B>,
    ) -> Result<ActiveUdpSendResult, VmError>
    where
    ...
    ) -> Result<ActiveUdpValueResult, VmError> {
    ...
    pub(in crate::execution) fn remote_address(&mut self) -> Result<ActiveUdpValueResult, VmError> {
        if let Some(remote_addr) = self.kernel_connected_remote_addr {
            return Ok(ActiveUdpValueResult::Immediate(json!({
                "address": remote_addr.ip().to_string(),
                "port": remote_addr.port(),
    ...
    fn get_buffer_size(&mut self, which: &str) -> Result<ActiveUdpValueResult, VmError> {
        if self.native_commands.is_none() {
...
fn dgram_option_field<'a>(payload: &'a Value, field: &str) -> Result<&'a Value, VmError> {
    payload
        .get(field)
        .ok_or_else(|| VmError::InvalidState(format!("dgram.setOption payload requires {field}")))
}

fn dgram_option_ip(payload: &Value, field: &str) -> Result<IpAddr, VmError> {
    let value = dgram_option_field(payload, field)?
        .as_str()
        .ok_or_else(|| VmError::InvalidState(format!("{field} must be an IP address")))?;
    value
        .parse()
        .map_err(|_| VmError::host("EINVAL", format!("invalid UDP {field} address {value}")))
}

fn dgram_option_interface(payload: &Value) -> Result<Option<String>, VmError> {
    match payload.get("interface") {
        None | Some(Value::Null) => Ok(None),
        Some(Value::String(value)) => Ok(Some(value.clone())),
        Some(_) => Err(VmError::InvalidState(String::from(
            "dgram.setOption interface must be a string",
        ))),
    }
}

fn parse_native_udp_option(name: &str, payload: &Value) -> Result<NativeUdpSocketOption, VmError> {
    match name {
        "broadcast" => Ok(NativeUdpSocketOption::Broadcast(
            dgram_option_field(payload, "enabled")?
                .as_bool()
                .ok_or_else(|| {
                    VmError::InvalidState(String::from("dgram.setOption enabled must be boolean"))
                })?,
        )),
        "ttl" => Ok(NativeUdpSocketOption::Ttl(
            ...
            .map_err(|_| VmError::host("EINVAL", "UDP TTL overflow"))?,
        )),
        "multicastTtl" => Ok(NativeUdpSocketOption::MulticastTtl(
            ...
            .map_err(|_| VmError::host("EINVAL", "UDP multicast TTL overflow"))?,
        )),
        ...
        "membership" => Ok(NativeUdpSocketOption::Membership {
            ...
            join: dgram_option_field(payload, "join")?
                .as_bool()
                .ok_or_else(|| {
                    VmError::InvalidState(String::from("dgram.setOption join must be boolean"))
                })?,
        }),
    ...
    match operation {
        ...
        DgramOperation::Connect {
            ...
            Ok(udp_value_service_response(
                result,
                agentos_driver_tokio::TaskClass::Udp,
            ))
        }
        ...
        DgramOperation::Address { socket_id } => {
            ...
            let local_addr = socket
                .local_addr()
                .ok_or_else(|| VmError::host("EBADF", "bad file descriptor"))?;
...
mod native_udp_owner_tests {
    ...
    fn receive_admission_pauses_before_recv_and_resumes_with_one_coalesced_wake() {
        ...
        let resources = Arc::new(ResourceLedger::child(
            ...
            Arc::clone(process_runtime.handle().resources()),
        ));
        ...
        let limits = reactor_io_limits(&crate::limits::VmLimits::default());
        let commands = {
            let _runtime_guard = runtime.tokio_handle().enter();
