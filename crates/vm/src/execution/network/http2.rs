...
fn reserve_http2_inbound_chunk(
    ...
    bytes: usize,
) -> Result<Vec<Reservation>, VmError> {
...
fn http2_snapshot_json(snapshot: &Http2SessionSnapshot) -> Result<String, VmError> {
    serde_json::to_string(snapshot)
        .map_err(|error| VmError::host("ERR_AGENTOS_NODE_SYNC_RPC", format!("{error}")))
}

fn http2_event_value(event: &Http2BridgeEvent) -> Result<Value, VmError> {
...
fn push_http2_retain_wake(
    ...
    if let Err(error) = session.publish_readiness(
        ...
        crate::executor::backend::ExecutionReadyFlags::READABLE,
    ) {
...
fn defer_http2_poll(
    ...
    process
        .runtime_context
        .spawn(agentos_driver_tokio::TaskClass::Http2, async move {
    ...
    Ok(HostServiceResponse::Deferred {
        receiver,
        timeout: None,
        task_class: agentos_driver_tokio::TaskClass::Http2,
    })
}
...
fn parse_http2_headers_json(
    ...
    serde_json::from_str::<BTreeMap<String, Value>>(headers_json)
        .map_err(|error| VmError::InvalidState(format!("{label} must be valid JSON: {error}")))
}
...
fn apply_http2_header_values(
    ...
    let header_name = HeaderName::from_bytes(name.as_bytes()).map_err(|error| {
        VmError::InvalidState(format!("invalid HTTP/2 header name {name:?}: {error}"))
    })?;
    match value {
        Value::Array(values) => {
            for value in values {
        ...
        Value::String(text) => {
            let value = HeaderValue::from_str(text).map_err(|error| {
                VmError::InvalidState(format!("invalid HTTP/2 header value for {name}: {error}"))
            })?;
...
fn build_http2_request(headers_json: &str) -> Result<Request<()>, VmError> {
    ...
    let mut builder = Request::builder()
        .method(Method::from_bytes(method.as_bytes()).map_err(|error| {
            VmError::InvalidState(format!("invalid HTTP/2 method {method:?}: {error}"))
        })?)
        .uri(path.parse::<Uri>().map_err(|error| {
            VmError::InvalidState(format!("invalid HTTP/2 path {path:?}: {error}"))
        })?);
    {
    ...
    builder
        .body(())
        .map_err(|error| VmError::InvalidState(format!("invalid HTTP/2 request: {error}")))
}

fn build_http2_response(headers_json: &str) -> Result<Response<()>, VmError> {
    ...
    builder
        .body(())
        .map_err(|error| VmError::InvalidState(format!("invalid HTTP/2 response headers: {error}")))
}
    ...
    for (name, value) in headers {
        ...
        let value = Value::String(
            value
                .to_str()
                .map_err(|error| {
                    VmError::Execution(format!("invalid HTTP/2 header value: {error}"))
                })?
                .to_owned(),
        );
...
fn serialize_http2_request_headers(request: &Request<h2::RecvStream>) -> Result<String, VmError> {
...
fn reserve_http2_stream(session: &ActiveHttp2Session) -> Result<Vec<Reservation>, VmError> {
...
async fn reserve_http2_stream_when_available(
    ...
    let session = shared
        ...
        .get(&session_id)
        .cloned()
        .ok_or_else(|| VmError::InvalidState(format!("unknown HTTP/2 session {session_id}")))?;
    let per_connection = session
        .stream_resources
...
fn spawn_http2_client_session(
    ...
    if let Err(error) = runtime.spawn(agentos_driver_tokio::TaskClass::Http2, async move {
                ...
                tokio::select! {
                    ...
                    admission = async {
                        ...
                        let capability = capabilities
                            ...
                            .await
                            .map_err(|error| VmError::Execution(error.to_string()))?;
                        Ok::<_, VmError>((capability, reservations))
...
fn spawn_http2_server_accept_loop(
    ...
    if let Err(error) = runtime.spawn(agentos_driver_tokio::TaskClass::Listener, async move {
...
fn send_http2_command(
    ...
    header_bytes: usize,
    data_bytes: usize,
    command: impl FnOnce(Http2ResponseSender) -> Result<Http2SessionCommand, VmError>,
) -> Result<HostServiceResponse, VmError> {
    ...
    session
        ...
        .map_err(|error| match error {
            ...
            tokio::sync::mpsc::error::TrySendError::Closed(_) => {
                VmError::InvalidState(String::from("HTTP/2 session command channel closed"))
            }
        })?;
    Ok(HostServiceResponse::Deferred {
...
) -> Result<Http2ServerListenOptions, VmError> {
    let payload_json =
...
fn parse_http2_connect_payload(
    request: &HostRpcRequest,
) -> Result<Http2SessionConnectOptions, VmError> {
    let payload_json =
...
fn http2_session_for_id(
    process: &ActiveProcess,
    session_id: u64,
) -> Result<ActiveHttp2Session, VmError> {
    let shared = process
        .http2
        .shared
        .lock()
...
fn http2_stream_for_id(
    process: &ActiveProcess,
    stream_id: u64,
) -> Result<ActiveHttp2Stream, VmError> {
    let shared = process
        .http2
        .shared
        .lock()
    ...
    shared
        ...
        .get(&stream_id)
        .cloned()
        .ok_or_else(|| VmError::InvalidState(format!("unknown HTTP/2 stream {stream_id}")))
}
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    ...
    let response = match request.method.as_str() {
        "net.http2_server_listen" => {
            ...
            )?;
            {
                let mut state = process.http2.shared.lock().map_err(|_| {
                    VmError::InvalidState(String::from("HTTP/2 state lock poisoned"))
                })?;
            ...
            spawn_http2_server_accept_loop(
                ...
                listener.listener.take().ok_or_else(|| {
                    VmError::InvalidState(String::from("HTTP/2 listener missing host TCP socket"))
                })?,
                close_notify,
                capabilities.clone(),
            );
        ...
        "net.http2_server_close" => {
            ...
            .ok_or_else(|| VmError::InvalidState(format!("unknown HTTP/2 server {server_id}")))?;
        ...
        "net.http2_session_connect" => {
            ...
            let url = Url::parse(&authority).map_err(|error| {
                VmError::InvalidState(format!("invalid HTTP/2 authority {authority:?}: {error}"))
            })?;
            ...
            let opaque_data = request
                ...
                .map(|value| {
                    base64::engine::general_purpose::STANDARD
                        .decode(value)
                        .map_err(|error| {
                            VmError::InvalidState(format!("invalid GOAWAY payload: {error}"))
                        })
                })
                .transpose()?;
            let session = http2_session_for_id(process, session_id)?;
        ...
        "net.http2_stream_write" => {
            ...
            return send_http2_command(&session, command_bytes, 0, decoded_bytes, |respond_to| {
                let chunk = base64::engine::general_purpose::STANDARD
                    .decode(chunk_base64)
                    .map_err(|error| {
                        VmError::InvalidState(format!("invalid HTTP/2 stream payload: {error}"))
                    })?;
...
mod http2_reactor_tests {
    ...
    use agentos_vm_kernel::resource_accounting::ResourceLimits;
            ...
            runtime.tokio_handle().block_on(async {
                assert!(matches!(
                    runtime
                        ...
                        .acquire(vm_generation, capability_id, FairBudget::new(1, 128))
                        .await,
                    Err(agentos_driver_tokio::fairness::FairnessError::CapabilityRetired {
