...
fn http_limit(limit_name: &'static str, limit: usize, observed: usize) -> VmError {
    VmError::Host(HostServiceError::limit(
...
mod tests {
    ...
    fn typed_http_limit_names_the_public_configuration_path() {
        let VmError::Host(error) = http_limit("limits.http.maxFetchResponseBytes", 8, 9) else {
