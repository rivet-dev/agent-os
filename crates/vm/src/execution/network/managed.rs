    ...
    match operation {
        ...
        crate::executor::host::NetworkOperation::ManagedWaitConnect { socket_id } => {
            ...
            serde_json::to_string(&info)
                ...
                .map_err(|error| VmError::host("EIO", format!("encode socket info: {error}")))
        }
...
fn managed_socket_poll(
    ...
    match state.terminal.as_ref() {
        ...
        .into()),
        Some(SocketReadTerminal::Error { code, message }) => Err(VmError::host(
...
fn prime_managed_socket_read_state(
    ...
    socket_id: &str,
) -> Result<Arc<Mutex<SocketReadState>>, VmError> {
...
fn managed_socket_write(
    ...
    if let Some(socket) = context.process.tcp_sockets.get(socket_id) {
        let receiver = if socket.tls_mode.load(Ordering::SeqCst) {
            Some((
                ...
                agentos_driver_tokio::TaskClass::Tls,
            ))
        } else if socket.kernel_socket_id.is_none() {
            Some((
                socket.begin_plain_write(bytes)?,
                agentos_driver_tokio::TaskClass::Socket,
            ))
        } else {
            None
        };
        if let Some((receiver, task_class)) = receiver {
            return Ok(HostServiceResponse::Deferred {
                receiver,
                timeout: (task_class == agentos_driver_tokio::TaskClass::Tls)
    ...
    let socket = context
        ...
        .unix_sockets
        .get(socket_id)
        .ok_or_else(|| VmError::host("EBADF", format!("unknown net socket {socket_id}")))?;
...
fn managed_tls_upgrade(
    ...
    let options: TlsBridgeOptions = serde_json::from_str(options_json)
        .map_err(|error| VmError::host("EINVAL", format!("invalid TLS options: {error}")))?;
    if context
        .process
        .capability_leases
    ...
    let socket = context
        ...
        .tcp_sockets
        .get(socket_id)
        .ok_or_else(|| VmError::host("EBADF", format!("unknown TCP socket {socket_id}")))?;
...
fn managed_close_listener(
    ...
    let listener = context
        ...
        .unix_listeners
        .remove(listener_id)
        .ok_or_else(|| VmError::host("EBADF", format!("unknown net listener {listener_id}")))?;
                ...
                let mut socket = if let Some(stream) = stream {
                    ...
                    ActiveTcpSocket::from_kernel(
                        kernel_socket_id.ok_or_else(|| {
                            VmError::host("EIO", "kernel TCP accept missing socket id")
                        })?,
                        Some(listener_id.to_owned()),
                        guest_local_addr,
                        guest_remote_addr,
