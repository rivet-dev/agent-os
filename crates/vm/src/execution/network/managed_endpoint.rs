    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    ...
    let socket = context
        ...
        .unix_sockets
        .get(socket_id)
        .ok_or_else(|| VmError::host("EBADF", format!("unknown Unix socket {socket_id}")))?;
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    ...
    if let Some(address) = endpoint.unix {
        ...
        let listener_id = bound
            ...
            .ok_or_else(|| VmError::host("EIO", "Unix bind omitted serverId"))?
            .to_owned();
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    ...
    let (local_path, local_abstract_path_hex) = {
        let listener = context
            ...
            .get_mut(listener_id)
            .ok_or_else(|| {
                VmError::host("EBADF", format!("unknown Unix listener {listener_id}"))
            })?;
        listener.relisten(
            &context.socket_paths.unix_bound_addresses,
            backlog,
