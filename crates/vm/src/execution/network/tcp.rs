...
pub(in crate::execution) struct ActiveTcpConnectRequest<'a, B> {
    ...
    pub(in crate::execution) runtime_context: agentos_driver_tokio::DriverHandle,
...
impl ActiveTcpSocket {
    ...
    pub(in crate::execution) fn retain_description_lease(
        &self,
        lease: Arc<agentos_driver_tokio::capability::CapabilityLease>,
    ) {
    ...
    pub(in crate::execution) fn poll(
        ...
        _wait: Duration,
        trace_enabled: bool,
    ) -> Result<Option<TcpSocketEvent>, VmError> {
        if let Some(event) = self
            .pending_read_event
            .lock()
            .map_err(|_| {
                VmError::InvalidState(String::from("TCP pending read event lock poisoned"))
            })?
            .take()
        {
        ...
        if self.tls_mode.load(Ordering::SeqCst) {
            ...
            return match self
                ...
                .as_ref()
                .ok_or_else(|| {
                    VmError::InvalidState(String::from("TCP socket event channel missing"))
                })?
                .lock()
                .map_err(|_| {
                    VmError::InvalidState(String::from("TCP socket event channel lock poisoned"))
                })?
                .try_recv()
            {
        ...
        match self
            .events
            .as_ref()
            .ok_or_else(|| VmError::InvalidState(String::from("TCP socket event channel missing")))?
            .lock()
            .map_err(|_| {
                VmError::InvalidState(String::from("TCP socket event channel lock poisoned"))
            })?
    ...
    fn ensure_tcp_reader(&self) -> Result<(), VmError> {
        ...
        let read_stream = self
            .pending_read_stream
            .as_ref()
            .ok_or_else(|| VmError::InvalidState(String::from("TCP socket reader handle missing")))?
            .lock()
            .map_err(|_| VmError::InvalidState(String::from("TCP socket reader lock poisoned")))?
            .take();
        if let Some(read_stream) = read_stream {
            ...
            let spawn_result = spawn_tcp_socket_reader(
                ...
                self.event_sender
                    .as_ref()
                    .ok_or_else(|| {
                        VmError::InvalidState(String::from("TCP socket event sender missing"))
                    })?
                    .clone(),
    ...
    pub(in crate::execution) fn set_no_delay(&mut self, enable: bool) -> Result<(), VmError> {
        ...
        let stream = self
            ...
            .lock()
            .map_err(|_| VmError::InvalidState(String::from("TCP socket lock poisoned")))?;
    ...
    > {
        ...
        };
        {
            let mut state = self
                .tls_state
                .lock()
                .map_err(|_| VmError::InvalidState(String::from("TLS state lock poisoned")))?;
        ...
        if self.kernel_socket_id.is_none() {
            ...
            self.pending_read_stream
                .as_ref()
                .ok_or_else(|| {
                    VmError::InvalidState(String::from("TCP socket reader handle missing"))
                })?
                .lock()
                .map_err(|_| {
                    VmError::InvalidState(String::from("TCP socket reader lock poisoned"))
                })?
                .take();
            let stream = self
                .stream
            ...
            *self.native_tls_commands.lock().map_err(|_| {
                VmError::InvalidState(String::from("native TLS command lock poisoned"))
            })? = Some(commands);
            return Ok(handshake);
        }
        ...
        let peer_socket_id = kernel
            ...
            .ok_or_else(|| {
                VmError::host("ERR_AGENTOS_LOOPBACK_PEER_MISSING", format!("kernel-backed loopback socket {socket_id} has no connected peer for TLS upgrade"
                ))
            })?;
        ...
        self.runtime_context
            .spawn(agentos_driver_tokio::TaskClass::Tls, async move {
    ...
    ) -> Result<Option<TlsClientHello>, VmError> {
        if let Some(socket_id) = self.kernel_socket_id {
            ...
            .and_then(|state| state.client_hello.clone())
        {
            return encode_net_json_string(
                serde_json::to_value(client_hello).map_err(|error| {
                    VmError::InvalidState(format!("failed to serialize TLS client hello: {error}"))
                })?,
    ...
    pub(in crate::execution) fn tls_query(
        ...
        let has_transport = self
            .native_tls_commands
            .lock()
            .map_err(|_| VmError::InvalidState(String::from("native TLS command lock poisoned")))?
            .is_some();
...
impl ActiveTcpListener {
    ...
    pub(in crate::execution) fn poll(
        ...
        wait: Duration,
        trace_enabled: bool,
    ) -> Result<Option<TcpListenerEvent>, VmError> {
        if let Some(event) = self
            .pending_event
            .lock()
            .map_err(|_| VmError::host("EIO", "TCP listener pending event lock poisoned"))?
            .take()
        {
            return Ok(Some(event));
        }
        ...
        loop {
            match self
                .listener
                .as_ref()
                .ok_or_else(|| VmError::InvalidState(String::from("TCP listener socket missing")))?
                .accept()
            {
...
pub(crate) fn build_socket_path_context(vm: &VmState) -> Result<SocketPathContext, VmError> {
...
pub(in crate::execution) fn normalize_tcp_listen_host(
    host: Option<&str>,
) -> Result<(SocketFamily, &'static str, &'static str), VmError> {
...
pub(in crate::execution) fn normalize_udp_bind_host(
    host: Option<&str>,
    family: UdpFamily,
) -> Result<(&'static str, &'static str, SocketFamily), VmError> {
    match (family, host) {
        ...
        (UdpFamily::Ipv4, Some(other)) => Err(VmError::host(
            "EACCES",
            format!("udp4 sockets must bind to 127.0.0.1 or 0.0.0.0, got {other}"),
        )),
        (UdpFamily::Ipv6, Some(other)) => Err(VmError::host(
            "EACCES",
...
pub(in crate::execution) fn allocate_guest_listen_port(
    ...
    if requested_port != 0 {
        if !is_allowed(requested_port) {
            ...
            return Err(VmError::Execution(reason));
        }
...
pub(in crate::execution) fn parse_proc_net_entries(
    table_path: &str,
) -> Result<Vec<ProcNetEntry>, VmError> {
...
pub(in crate::execution) fn resolve_tcp_bind_addr(
    ...
    (host, port)
        ...
        .next()
        .ok_or_else(|| {
            VmError::Execution(format!("failed to resolve TCP bind address {host}:{port}"))
        })
}
...
pub(in crate::execution) fn plain_socket_command_admission_error(
    ...
    match error {
        ...
        tokio::sync::mpsc::error::TrySendError::Closed(_) => {
            VmError::host("EPIPE", "socket transport task is closed")
        }
    }
}
...
pub(in crate::execution) fn reserve_plain_socket_write_payload(
    resources: &Arc<ResourceLedger>,
    contents: &[u8],
) -> Result<PlainSocketWritePayload, VmError> {
...
pub(in crate::execution) fn reserve_plain_socket_command(
    resources: &Arc<ResourceLedger>,
) -> Result<SharedReservation, VmError> {
    resources
        .reserve(ResourceClass::HandleCommands, 1)
...
fn spawn_tcp_plain_socket_transport(
    ...
    fairness_identity: Arc<OnceLock<(u64, u64)>>,
    fairness_identity_committed: Arc<tokio::sync::Notify>,
) -> Result<TokioSender<NativePlainSocketCommand>, VmError> {
    ...
    runtime
        .spawn(agentos_driver_tokio::TaskClass::Socket, async move {
...
pub(in crate::execution) fn resolve_tcp_connect_addr<B>(
    ...
    family: Option<u8>,
    context: &SocketPathContext,
) -> Result<ResolvedTcpConnectAddr, VmError>
where
    B: VmManagerHost + Send + 'static,
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    ...
    let ip = allowed
        ...
        .ok_or_else(|| {
            VmError::Execution(format!("failed to resolve TCP address {host}:{port}"))
        })?;
...
pub(in crate::execution) fn resolve_dns_records<B>(
    ...
    policy: DnsLookupPolicy,
) -> Result<DnsRecordResolution, VmError>
where
    B: VmManagerHost + Send + 'static,
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
...
pub(in crate::execution) fn encode_net_json_string(
    ...
    serde_json::to_string(&value)
        .map(Value::String)
        .map_err(|error| {
            VmError::InvalidState(format!("failed to serialize {label} payload: {error}"))
        })
}
...
pub(in crate::execution) fn net_read_value(
    ...
    match event {
        ...
        Some(TcpSocketEvent::Error { code, message }) => {
...
pub(in crate::execution) fn sidecar_net_error(error: std::io::Error) -> VmError {
...
mod plain_socket_fairness_tests {
    ...
    fn transferred_description_retires_transport_identity_after_last_alias() {
        ...
        let runtime = process_runtime.handle();
        ...
        runtime.tokio_handle().block_on(async {
            ...
            assert!(matches!(
                error,
                agentos_driver_tokio::fairness::FairnessError::CapabilityRetired {
