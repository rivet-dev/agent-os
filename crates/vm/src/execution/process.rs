...
impl ActiveProcess {
    ...
    ) -> Result<agentos_vm_kernel::process_runtime::RuntimeControlReceiver, VmError> {
        kernel_handle
    ...
    fn apply_backend_shutdown_outcome(&mut self, outcome: ShutdownOutcome) -> Result<(), VmError> {
        use agentos_vm_kernel::process_runtime::ProcessExit;
    ...
    pub(crate) fn apply_runtime_controls(&mut self) -> Result<(), VmError> {
        use agentos_vm_kernel::process_runtime::{ProcessCancellationReason, ProcessTermination};
                            ...
                            for reply in interrupted_replies {
                                if let Err(error) = reply.fail(interrupted.clone()) {
                                    if settlement_error.is_none() {
                                        settlement_error = Some(VmError::from(error));
                                    }
                                }
                            }
        ...
        match result {
            Ok(()) => match self.runtime_control.acknowledge(controls) {
                ...
                Err(error) => {
                    ...
                    Err(VmError::host(error.code(), error.message()))
                }
            },
            Err(error) => {
                self.runtime_control.retry_pending();
    ...
    fn try_queue_pending_execution_event(
        &mut self,
        event: ActiveExecutionEvent,
    ) -> Result<(), (VmError, ActiveExecutionEvent)> {
    ...
    pub(super) fn try_queue_pending_polled_execution_event(
        &mut self,
        polled: PolledExecutionEvent,
    ) -> Result<(), (VmError, PolledExecutionEvent)> {
    ...
    pub(crate) fn with_process_event_limits(
        mut self,
        limits: &crate::core::limits::ProcessLimits,
    ) -> Self {
    ...
    fn track_capability(
        ...
        lease: agentos_driver_tokio::capability::CapabilityLease,
    ) -> Result<(), VmError> {
        match self.capability_leases.entry(key.clone()) {
            ...
            std::collections::btree_map::Entry::Occupied(_) => Err(VmError::host(
    ...
    pub(super) fn shared_capability_lease(
        &self,
        key: &NativeCapabilityKey,
    ) -> Option<Arc<agentos_driver_tokio::capability::CapabilityLease>> {
        self.capability_leases.get(key).map(Arc::clone)
    }

    pub(super) fn release_capability(&mut self, key: &NativeCapabilityKey) -> Result<(), VmError> {
    ...
    pub(super) fn capability_fairness_identity(
        ...
        agentos_driver_tokio::capability::CapabilityId,
        agentos_driver_tokio::capability::SessionGeneration,
    )> {
    ...
    fn stop_and_continue_wait_state_follow_runtime_control_acknowledgement() {
        ...
        assert_eq!(
            kernel
                ...
                .status,
            agentos_vm_kernel::process_table::ProcessStatus::Running
        );
        process
            .apply_runtime_controls()
        ...
        assert_eq!(
            kernel
                ...
                .expect("kernel process")
                .status,
            agentos_vm_kernel::process_table::ProcessStatus::Stopped
        );
...
fn poll_binding_process_event_leased(
    ...
    if let Some(reason) = execution
        ...
        .clone()
    {
        return Err(VmError::Host(reason));
    }
    Ok(None)
}
...
pub(super) fn poll_child_execution_after_exit(
    ...
    match child.try_poll_execution_event() {
        Ok(event) => Ok(event),
        Err(VmError::ExecutionEventChannelClosed { .. }) => Ok(None),
        Err(error) => Err(error),
    }
}
...
impl ActiveExecution {
    ...
    pub(crate) fn start_prepared(&mut self) -> Result<(), VmError> {
        ExecutionBackend::start_prepared(self).map_err(VmError::from)
    }
    ...
    pub(crate) fn execute_retained_language(
        ...
        match (self, language) {
            ...
            (Self::Python(execution), RetainedExecutionLanguage::Python) => execution
                ...
                .map_err(|error| VmError::Execution(error.to_string())),
            _ => Err(VmError::InvalidState(String::from(
    ...
    pub(crate) fn terminate(&mut self) -> Result<(), VmError> {
    ...
    pub(crate) fn begin_shutdown(
        &mut self,
        reason: ShutdownReason,
    ) -> Result<ShutdownOutcome, VmError> {
        ExecutionBackend::begin_shutdown(self, reason).map_err(VmError::from)
    }

    pub(crate) fn pause(&self) -> Result<(), VmError> {
        ExecutionBackend::set_paused(self, true).map_err(VmError::from)
    }

    pub(crate) fn resume(&self) -> Result<(), VmError> {
        ExecutionBackend::set_paused(self, false).map_err(VmError::from)
    }
    ...
    pub(crate) fn deliver_signal_checkpoint(
        ...
        flags: u32,
        thread_id: u32,
    ) -> Result<SignalCheckpointOutcome, VmError> {
...
fn execution_host_call(
    ...
    identity: ProcessRuntimeIdentity,
    max_reply_bytes: usize,
) -> Result<ExecutionHostCall, VmError> {
...
mod typed_direct_error_tests {
    ...
    fn direct_settlement_preserves_limit_details_and_typed_errno() {
        ...
        let limit = VmError::ResourceLimit(agentos_driver_tokio::accounting::LimitError {
            scope: String::from("vm=vm-typed-errors"),
            resource: agentos_driver_tokio::accounting::ResourceClass::BridgeResponseBytes,
            used: 60,
            requested: 8,
            limit: 64,
        ...
        settle_execution_host_call(&reply(Arc::clone(&target), 1), Err(VmError::from(deferred)))
        ...
        settle_execution_host_call(&reply(Arc::clone(&target), 2), Err(VmError::Host(denied)))
    ...
    let event = match event {
        ...
        PythonExecutionEvent::VfsRpcRequest(request) => {
            ...
            if let Err(error) = host.submit(call.operation, call.reply.clone(), admission) {
                call.reply.fail(error).map_err(VmError::from)?;
            }
            return Ok(None);
        }
...
) -> Result<Option<SocketStateEntry>, VmError> {
    let vm = vm.ok_or_else(|| VmError::InvalidState(String::from("unknown sidecar VM")))?;
...
fn snapshot_vm_processes_inner(
    vm: &VmState,
    process_table: &BTreeMap<u32, agentos_vm_kernel::process_table::ProcessInfo>,
...
pub(super) fn build_process_snapshot_entry(
    process_id: &str,
    process: &ActiveProcess,
    info: &agentos_vm_kernel::process_table::ProcessInfo,
...
fn socket_inodes_for_pid(pid: u32) -> Result<BTreeSet<u64>, VmError> {
...
pub(super) fn reserve_capability(
    ...
    kind: CapabilityKind,
) -> Result<PendingCapability, VmError> {
    registry.reserve(kind).map_err(VmError::from)
}
...
> {
    ...
    let lease = pending.commit(backend).map_err(VmError::from)?;
