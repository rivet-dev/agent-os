...
use agentos_vm_kernel::vfs::{VirtualTimeSpec, VirtualUtimeSpec};
...
fn decode_javascript_dgram_operation(request: &HostRpcRequest) -> Result<DgramOperation, VmError> {
    let string =
    ...
    match request.method.as_str() {
        "dgram.createSocket" => {
            let payload: DgramCreateSocketOptions =
                ...
                .map_err(|error| {
                    VmError::InvalidState(format!("invalid dgram.createSocket payload: {error}"))
                })?;
        ...
        "dgram.bind" => {
            let payload: DgramBindOptions =
                serde_json::from_value(request.args.get(1).cloned().ok_or_else(|| {
                    VmError::InvalidState(String::from("dgram.bind requires a request payload"))
                })?)
                .map_err(|error| {
                    VmError::InvalidState(format!("invalid dgram.bind payload: {error}"))
                })?;
        ...
        "dgram.send" => {
            let payload: DgramSendOptions =
                serde_json::from_value(request.args.get(2).cloned().ok_or_else(|| {
                    VmError::InvalidState(String::from("dgram.send requires a request payload"))
                })?)
                .map_err(|error| {
                    VmError::InvalidState(format!("invalid dgram.send payload: {error}"))
                })?;
        ...
        "dgram.connect" => {
            let payload: DgramConnectOptions =
                serde_json::from_value(request.args.get(1).cloned().ok_or_else(|| {
                    VmError::InvalidState(String::from("dgram.connect requires a request payload"))
                })?)
                .map_err(|error| {
                    VmError::InvalidState(format!("invalid dgram.connect payload: {error}"))
                })?;
        ...
        "dgram.setOption" => Ok(DgramOperation::SetOption {
            ...
            payload: request.args.get(2).cloned().ok_or_else(|| {
                VmError::InvalidState(String::from("dgram.setOption requires an option payload"))
            })?,
        }),
        "dgram.setBufferSize" => {
...
fn decode_javascript_dns_operation(request: &HostRpcRequest) -> Result<DnsOperation, VmError> {
    match request.method.as_str() {
        "dns.lookup" => {
            let payload: JavascriptDnsLookupRequest =
                serde_json::from_value(request.args.first().cloned().ok_or_else(|| {
                    VmError::InvalidState(String::from("dns.lookup requires a request payload"))
                })?)
                .map_err(|error| {
                    VmError::InvalidState(format!("invalid dns.lookup payload: {error}"))
                })?;
        ...
        "dns.resolve" | "dns.resolve4" | "dns.resolve6" | "dns.resolveRawRr" => {
            let payload: JavascriptDnsResolveRequest =
                serde_json::from_value(request.args.first().cloned().ok_or_else(|| {
                    VmError::InvalidState(String::from("dns.resolve requires a request payload"))
                })?)
                .map_err(|error| {
                    VmError::InvalidState(format!("invalid dns.resolve payload: {error}"))
                })?;
...
) -> Result<Option<HostRpcRequest>, VmError> {
...
pub(crate) fn javascript_sync_rpc_arg_str<'a>(
    ...
    args.get(index)
        .and_then(Value::as_str)
        .ok_or_else(|| VmError::InvalidState(format!("{label} must be a string argument")))
}
...
pub(crate) fn javascript_sync_rpc_arg_bool(
    ...
    args.get(index)
        ...
        .ok_or_else(|| VmError::InvalidState(format!("{label} must be a boolean argument")))
}
...
pub(crate) fn javascript_sync_rpc_option_u32(
    ...
    let numeric = value
        ...
        .ok_or_else(|| VmError::InvalidState(format!("{key} must be numeric")))?;

    u32::try_from(numeric)
        .map(Some)
        .map_err(|_| VmError::InvalidState(format!("{key} must fit within u32")))
}
...
pub(crate) fn javascript_sync_rpc_arg_u32(
    ...
    u32::try_from(value).map_err(|_| VmError::InvalidState(format!("{label} must fit within u32")))
}
...
pub(crate) fn javascript_sync_rpc_arg_i32(
    ...
    let Some(value) = args.get(index) else {
        return Err(VmError::InvalidState(format!("{label} is required")));
    };
    ...
    let numeric = value
        ...
        .ok_or_else(|| VmError::InvalidState(format!("{label} must be a numeric argument")))?;

    i32::try_from(numeric)
        .map_err(|_| VmError::InvalidState(format!("{label} must fit within i32")))
}
...
pub(crate) fn javascript_sync_rpc_arg_u32_optional(
    ...
    javascript_sync_rpc_arg_u64_optional(args, index, label)?
        .map(|value| {
            u32::try_from(value)
                .map_err(|_| VmError::InvalidState(format!("{label} must fit within u32")))
        })
        .transpose()
}
...
pub(crate) fn javascript_sync_rpc_arg_u64(
    ...
    if let Some(text) = value.as_str() {
        return text
            .parse::<u64>()
            .map_err(|_| VmError::InvalidState(format!("{label} must be a u64")));
    }
...
fn javascript_sync_rpc_arg_rlim(args: &[Value], index: usize, label: &str) -> Result<u64, VmError> {
    let Some(value) = args.get(index) else {
        return Err(VmError::InvalidState(format!("{label} is required")));
    };
    if let Some(text) = value.as_str() {
...
fn process_resource_limit_kind(
    resource: u32,
) -> Result<agentos_vm_kernel::kernel::ProcessResourceLimitKind, VmError> {
    use agentos_vm_kernel::kernel::ProcessResourceLimitKind;
    match resource {
...
pub(crate) fn javascript_sync_rpc_bytes_arg(
    ...
    decode_encoded_bytes_value(value)
        .map_err(|error| VmError::host("EINVAL", format!("{label} {error}")))
}
...
pub(in crate::execution) fn javascript_sync_rpc_base64_arg(
    ...
    decode_base64(value).map_err(|error| VmError::InvalidState(format!("{label} {error}")))
}
...
fn wasm_process_resolve_at_path(
    ...
    let stat = kernel
        ...
        .map_err(kernel_error)?;
    if stat.filetype != agentos_vm_kernel::fd_table::FILETYPE_DIRECTORY {
        return Err(VmError::host(
            "ENOTDIR",
...
fn wasm_process_path_stat_value(stat: agentos_vm_kernel::vfs::VirtualStat) -> Value {
    let filetype = if stat.is_directory {
        agentos_vm_kernel::fd_table::FILETYPE_DIRECTORY
    } else if stat.is_symbolic_link {
        agentos_vm_kernel::fd_table::FILETYPE_SYMBOLIC_LINK
    } else {
...
fn wasm_process_utime_spec(
    ...
    explicit: bool,
    now: bool,
) -> Result<VirtualUtimeSpec, VmError> {
    if now {
    ...
    let nanoseconds = nanoseconds
        .parse::<u64>()
        .map_err(|_| VmError::host("EINVAL", "timestamp must be u64 nanoseconds"))?;
    let seconds = i64::try_from(nanoseconds / 1_000_000_000)
        .map_err(|_| VmError::host("EINVAL", "timestamp exceeds i64 seconds"))?;
    VirtualTimeSpec::new(seconds, (nanoseconds % 1_000_000_000) as u32)
        .map(VirtualUtimeSpec::Set)
        .map_err(|error| VmError::host("EINVAL", error.to_string()))
}
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    ...
    if request.raw_bytes_args.contains_key(&usize::MAX) && request.method == "fs.readFileRangeSync"
    {
        ...
        .map_err(|_| {
            VmError::InvalidState("filesystem ranged read length must fit within usize".to_string())
        })?;
        let bytes = kernel
    ...
    let response = match request.method.as_str() {
        ...
        "process.signal_mask" => {
            ...
            let signals = request
                ...
                .ok_or_else(|| {
                    VmError::host("EINVAL", String::from("signal-mask set must be an array",
                    ))
                })?
                .iter()
                .map(|value| {
                    value
                        ...
                        .ok_or_else(|| {
                            VmError::host("EINVAL", String::from("signal-mask entries must be 32-bit integers",
                            ))
                        })
                })
            ...
            let how = match operation {
                ...
                _ => {
                    return Err(VmError::host("EINVAL", format!("invalid signal-mask operation {operation}"
                    )))
                }
            };
        ...
        "process.signal_mask_scope_begin" => {
            let signals = request
                ...
                .and_then(Value::as_array)
                .ok_or_else(|| {
                    VmError::host("EINVAL", String::from("temporary signal mask must be an array",
                    ))
                })?
                .iter()
                .map(|value| {
                    value
                        ...
                        .and_then(|signal| i32::try_from(signal).ok())
                        .ok_or_else(|| {
                            VmError::host("EINVAL", String::from("temporary signal-mask entries must be 32-bit integers",
                            ))
                        })
                })
                .collect::<Result<Vec<_>, _>>()?;
        ...
        "process.itimer_real" => {
            ...
            let values = match operation {
                ...
                other => {
                    return Err(VmError::host("EINVAL", format!("invalid ITIMER_REAL operation {other}"
                    )))
                }
            };
            Ok(json!({
        ...
        "process.waitpid_transition" => {
            ...
            if options & !(1 | 2 | 8) != 0 {
                return Err(VmError::host("EINVAL", format!("invalid waitpid option bits {:#x}",
                    options & !(1 | 2 | 8)
                )));
            }
            ...
            match transition {
                Some(event) => {
                    let status = match event.event {
                        agentos_vm_kernel::kernel::WaitPidEvent::Stopped => {
                            ((event.status as u32 & 0xff) << 8) | 0x7f
                        }
                        agentos_vm_kernel::kernel::WaitPidEvent::Continued => 0xffff,
                        agentos_vm_kernel::kernel::WaitPidEvent::Exited => {
                ...
                Some(transition) => {
                    let (event, raw_status, exit_code, signal, core_dumped) =
                        match (transition.event, transition.termination) {
                            (
                                agentos_vm_kernel::kernel::WaitPidEvent::Exited,
                                Some(agentos_vm_kernel::process_runtime::ProcessExit::Exited(code)),
                            ) => (
                                "exit",
                                ((code as u32 & 0xff) << 8),
                                code as u32 & 0xff,
                            ...
                            (
                                agentos_vm_kernel::kernel::WaitPidEvent::Exited,
                                Some(agentos_vm_kernel::process_runtime::ProcessExit::Signaled {
                                    signal,
                                    core_dumped,
                                }),
                            ) => (
                            ...
                            (agentos_vm_kernel::kernel::WaitPidEvent::Stopped, _) => (
                                "stopped",
                            ...
                            (agentos_vm_kernel::kernel::WaitPidEvent::Continued, _) => {
                                ("continued", 0xffff, 0, 0, false)
                            }
                            (agentos_vm_kernel::kernel::WaitPidEvent::Exited, None) => {
                                return Err(VmError::InvalidState(String::from(
        ...
        "process.fd_open" => {
            ...
            let rights_base = javascript_sync_rpc_arg_str(&request.args, 3, "fd_open base rights")?
                .parse::<u64>()
                .map_err(|_| VmError::host("EINVAL", "fd_open base rights must be u64"))?;
            ...
            .parse::<u64>()
            .map_err(|_| VmError::host("EINVAL", "fd_open inheriting rights must be u64"))?;
        ...
        "process.path_open_at" => {
            ...
            .parse::<u64>()
            .map_err(|_| VmError::host("EINVAL", "path_open_at base rights must be u64"))?;
            let rights_inheriting = javascript_sync_rpc_arg_str(
                &request.args,
                5,
            ...
            .parse::<u64>()
            .map_err(|_| {
                VmError::host("EINVAL", "path_open_at inheriting rights must be u64")
            })?;
        ...
        "process.random_get" => {
            ...
            .map_err(|_| VmError::InvalidState("random_get length is too large".into()))?;
            ...
            getrandom::getrandom(&mut bytes).map_err(|error| {
                VmError::Io(format!("failed to read system random bytes: {error}"))
            })?;
        ...
        "process.fd_description_alias_count" => {
            ...
            .parse::<u64>()
            .map_err(|_| {
                VmError::host("EINVAL", "fd description id must be a u64 decimal string")
            })?;
            kernel
        ...
        "process.fd_pread" => {
            ...
            .map_err(|_| VmError::InvalidState("fd_pread length is too large".into()))?;
            let offset = javascript_sync_rpc_arg_str(&request.args, 2, "fd_pread offset")?
                .parse::<u64>()
                .map_err(|_| VmError::InvalidState("fd_pread offset must be u64".into()))?;
            kernel
                .fd_pread(
                    EXECUTION_DRIVER_NAME,
                    process.kernel_pid,
        ...
        "process.fd_pwrite" => {
            ...
            let offset = javascript_sync_rpc_arg_str(&request.args, 2, "fd_pwrite offset")?
                .parse::<u64>()
                .map_err(|_| VmError::InvalidState("fd_pwrite offset must be u64".into()))?;
            let written = kernel
        ...
        "process.fd_readdir" => {
            ...
            let cookie = javascript_sync_rpc_arg_str(&request.args, 1, "fd_readdir cookie")?
                .parse::<usize>()
                .map_err(|_| {
                    VmError::InvalidState("fd_readdir cookie must be usize".into())
                })?;
        ...
        "process.fd_truncate" => {
            ...
            let length = javascript_sync_rpc_arg_str(&request.args, 1, "fd_truncate length")?
                .parse::<u64>()
                .map_err(|_| VmError::InvalidState("fd_truncate length must be u64".into()))?;
            kernel
        ...
        "process.fd_getfd" => {
            ...
            kernel
                .fd_fcntl(
                    ...
                    process.kernel_pid,
                    fd,
                    agentos_vm_kernel::fd_table::F_GETFD,
                    0,
                )
                .map(Value::from)
                .map_err(kernel_error)
        ...
        "process.fd_setfd" => {
            ...
            kernel
                .fd_fcntl(
                    ...
                    process.kernel_pid,
                    fd,
                    agentos_vm_kernel::fd_table::F_SETFD,
                    flags,
                )
                .map(Value::from)
                .map_err(kernel_error)
        ...
        "process.fd_record_lock" => {
            ...
            let start = javascript_sync_rpc_arg_str(&request.args, 3, "fd_record_lock start")?
                .parse::<u64>()
                .map_err(|_| {
                    VmError::host("EINVAL", "fd_record_lock start must be u64")
                })?;
            let length = javascript_sync_rpc_arg_str(&request.args, 4, "fd_record_lock length")?
                .parse::<u64>()
                .map_err(|_| {
                    VmError::host("EINVAL", "fd_record_lock length must be u64")
                })?;
            let lock_type = match raw_lock_type {
                0 => agentos_vm_kernel::fd_table::RecordLockType::Read,
                1 => agentos_vm_kernel::fd_table::RecordLockType::Write,
                2 => agentos_vm_kernel::fd_table::RecordLockType::Unlock,
                _ => {
                    return Err(VmError::host("EINVAL", "fd_record_lock type must be F_RDLCK, F_WRLCK, or F_UNLCK",
                    ))
                }
            };
            let conflict = match command {
                ...
                14 => kernel
                    ...
                    .map(|()| None),
                _ => {
                    return Err(VmError::host("EINVAL", format!("unsupported fd_record_lock command {command}"
                    )))
                }
            }
            .map_err(kernel_error)?;
            let response = conflict.map_or_else(
                ...
                |lock| {
                    let lock_type = match lock.lock_type {
                        agentos_vm_kernel::fd_table::RecordLockType::Read => 0,
                        agentos_vm_kernel::fd_table::RecordLockType::Write => 1,
                        agentos_vm_kernel::fd_table::RecordLockType::Unlock => 2,
                    };
                    json!({
                        "type": lock_type,
                        "pid": lock.pid,
        ...
        "process.fd_dup_min" => {
            ...
            kernel
                .fd_fcntl(
                    ...
                    process.kernel_pid,
                    fd,
                    agentos_vm_kernel::fd_table::F_DUPFD,
                    min_fd,
                )
                .map(Value::from)
                .map_err(kernel_error)
        ...
        "process.fd_seek" => {
            ...
            let offset = javascript_sync_rpc_arg_str(&request.args, 1, "fd_seek offset")?
                .parse::<i64>()
                .map_err(|_| VmError::InvalidState("fd_seek offset must be i64".into()))?;
            let whence = u8::try_from(javascript_sync_rpc_arg_u32(
                &request.args,
                2,
                "fd_seek whence",
            )?)
            .map_err(|_| VmError::InvalidState("fd_seek whence is invalid".into()))?;
            kernel
                .fd_seek(
                    EXECUTION_DRIVER_NAME,
                    process.kernel_pid,
        ...
        "process.fd_chdir_path" => {
            ...
            let stat = kernel
                ...
                .map_err(kernel_error)?;
            if stat.filetype != agentos_vm_kernel::fd_table::FILETYPE_DIRECTORY {
                return Err(VmError::host("ENOTDIR", format!("file descriptor {fd} is not a directory"
                )));
            }
            kernel
                .fd_path(EXECUTION_DRIVER_NAME, process.kernel_pid, fd)
        ...
        "process.kill" => {
            ...
            let process_pid = i32::try_from(process.kernel_pid)
                .map_err(|_| VmError::InvalidState("process pid exceeds i32".into()))?;
        ...
        "process.setrlimit" => {
            ...
            kernel
                .set_resource_limit(
                    ...
                    agentos_vm_kernel::kernel::ProcessResourceLimit {
...
fn service_javascript_tls_deferred_rpc(
    ...
    request: &HostRpcRequest,
    capabilities: &CapabilityRegistry,
) -> Result<Option<HostServiceResponse>, VmError> {
    ...
    let deferred = |receiver| HostServiceResponse::Deferred {
        ...
        task_class: agentos_driver_tokio::TaskClass::Tls,
    };
    match request.method.as_str() {
        "net.socket_upgrade_tls" => {
            let socket_id =
            ...
            let socket = process.tcp_sockets.get(socket_id).ok_or_else(|| {
                VmError::InvalidState(format!("unknown TCP socket {socket_id} for TLS upgrade"))
            })?;
...
fn service_javascript_plain_socket_deferred_rpc(
    ...
    let deferred = |receiver| HostServiceResponse::Deferred {
        receiver,
        timeout: None,
        task_class: agentos_driver_tokio::TaskClass::Socket,
    };
    match request.method.as_str() {
        "net.write" => {
            ...
            let socket = process.unix_sockets.get(socket_id).ok_or_else(|| {
                VmError::InvalidState(format!("unknown net socket {socket_id} for net.write"))
            })?;
        ...
        "net.shutdown" => {
            ...
            let socket = process.unix_sockets.get(socket_id).ok_or_else(|| {
                VmError::InvalidState(format!("unknown net socket {socket_id} for net.shutdown"))
            })?;
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    if request.sync_request.method == "net.server_close" {
        ...
        let listener = request
            ...
            .unix_listeners
            .remove(&listener_id)
            .ok_or_else(|| VmError::InvalidState(format!("unknown net listener {listener_id}")))?;
    ...
    if request.sync_request.method == "net.connect" {
        let payload = request
            ...
            .cloned()
            .ok_or_else(|| {
                VmError::InvalidState(String::from("net.connect requires a request payload"))
            })
            .and_then(|value| {
                serde_json::from_value::<JavascriptNetConnectRequest>(value).map_err(|error| {
                    VmError::InvalidState(format!("invalid net.connect payload: {error}"))
                })
            })?;
        ...
        let port = payload.port.ok_or_else(|| {
            VmError::InvalidState(String::from("net.connect requires either a path or port"))
        })?;
    ...
    if request.sync_request.method == "net.http_wait" {
        ...
        let server = request
            ...
            .http_servers
            .get(&server_id)
            .ok_or_else(|| VmError::InvalidState(format!("unknown HTTP server {server_id}")))?;
    ...
    let event = if let Some(socket) = process.tcp_sockets.get_mut(socket_id) {
        ...
        let socket = process
            .unix_sockets
            .get_mut(socket_id)
            .ok_or_else(|| VmError::InvalidState(format!("unknown net socket {socket_id}")))?;
        socket.set_application_read_interest(true)?;
...
async fn service_javascript_dgram_poll_response(
    ...
    let event = process
        .udp_sockets
        .get(socket_id)
        .ok_or_else(|| VmError::InvalidState(format!("unknown UDP socket {socket_id}")))?
    ...
    match request.method.as_str() {
        ...
        "net.bind_unix" => {
            let payload = request
                ...
                .cloned()
                .ok_or_else(|| {
                    VmError::InvalidState(String::from("net.bind_unix requires a request payload"))
                })
                .and_then(|value| {
                    serde_json::from_value::<JavascriptNetListenRequest>(value).map_err(|error| {
                        VmError::InvalidState(format!("invalid net.bind_unix payload: {error}"))
                    })
                })?;
        ...
        "net.bind_connected_unix" => {
            ...
            let socket = process
                ...
                .ok_or_else(|| {
                    VmError::InvalidState(format!("unknown Unix socket {}", payload.socket_id))
                })?;
        ...
        "net.listen" => {
            let payload = request
                ...
                .cloned()
                .ok_or_else(|| {
                    VmError::InvalidState(String::from("net.listen requires a request payload"))
                })
                .and_then(|value| match value {
                    Value::String(json) => {
                        serde_json::from_str::<JavascriptNetListenRequest>(&json).map_err(|error| {
                            VmError::InvalidState(format!("invalid net.listen payload: {error}"))
                        })
                    }
        ...
        "net.socket_wait_connect" => {
            ...
            if let Some(socket) = process.tcp_sockets.get(socket_id) {
                ...
                let socket = process.unix_sockets.get(socket_id).ok_or_else(|| {
                    VmError::InvalidState(format!("unknown net socket {socket_id}"))
                })?;
                encode_net_json_string(socket.socket_info(), "net.socket_wait_connect")
            }
        }
        ...
        "net.socket_upgrade_tls" => Err(VmError::InvalidState(String::from(
        ...
        "net.socket_tls_query" => {
            ...
            let detailed = request
                ...
                .unwrap_or(false);
            let socket = process.tcp_sockets.get(socket_id).ok_or_else(|| {
                VmError::InvalidState(format!("unknown TCP socket {socket_id} for TLS query"))
            })?;
        ...
        "net.server_poll" => {
            ...
            let event = {
                let listener = process.unix_listeners.get_mut(listener_id).ok_or_else(|| {
                    VmError::InvalidState(format!("unknown net listener {listener_id}"))
                })?;
        ...
        "net.server_accept" => {
            ...
            if let Some(listener) = process.tcp_listeners.get_mut(listener_id) {
                ...
                return match listener.poll(
                    ...
                    Some(TcpListenerEvent::Error { code, message }) => {
                        Err(VmError::host(code.as_deref().unwrap_or("EIO"), message))
                    }
            ...
            match event {
                ...
                Some(UnixListenerEvent::Error { code, message }) => {
                    Err(VmError::host(code.as_deref().unwrap_or("EIO"), message))
                }
                None => Ok(net_timeout_value()),
            }
        ...
        "net.upgrade_socket_write" => {
            ...
            let socket = process
                .tcp_sockets
                .get(socket_id)
                .ok_or_else(|| VmError::InvalidState(format!("unknown TCP socket {socket_id}")))?;
            socket
...
pub(crate) fn error_code(error: &VmError) -> &str {
    match error {
        ...
        VmError::BridgeVersionMismatch(_) => "bridge_version_mismatch",
        VmError::Conflict(_) => "conflict",
        VmError::Unauthorized(_) => "unauthorized",
        VmError::Unsupported(_) => "unsupported",
        VmError::FrameTooLarge(_) => "frame_too_large",
        VmError::Kernel(_) => "kernel_error",
        VmError::Plugin(_) => "plugin_error",
        VmError::Execution(_) => "execution_error",
        VmError::ExecutionEventChannelClosed { .. } => "execution_event_channel_closed",
        VmError::Bridge(_) => "bridge_error",
        VmError::Io(_) => "io_error",
    }
}

pub(in crate::execution) fn guest_error_code(error: &VmError) -> Option<&str> {
    error.code()
}

pub(crate) fn host_service_error_code(error: &VmError) -> String {
    error
        .code()
...
pub(in crate::execution) fn host_service_error_message(error: &VmError) -> String {
    match error {
        VmError::ResourceLimit(limit) => crate::state::guest_limit_message(limit),
        VmError::Host(error) => error.message.clone(),
        _ => error.to_string(),
    }
}

pub(crate) fn host_service_error(error: &VmError) -> crate::executor::backend::HostServiceError {
    use crate::executor::backend::HostServiceError;

    match error {
        VmError::Host(error) => error.clone(),
        VmError::ResourceLimit(limit) => {
...
#[cfg(test)]
pub(crate) fn ignore_stale_javascript_sync_rpc_response(error: VmError) -> Result<(), VmError> {
    match error {
        VmError::Execution(message)
...
mod error_code_tests {
    use super::{
        ...
        javascript_sync_rpc_arg_u64, process_resource_limit_kind, VmError,
    };
    ...
    fn host_service_error_code_ignores_spoofed_errnos() {
        let error = VmError::Execution(String::from("user said 'EACCES: denied'"));
    ...
    fn host_service_error_code_preserves_real_sidecar_errnos() {
        let error = VmError::host("EACCES", "permission denied on /foo");
    ...
    fn host_service_error_code_preserves_dgram_state_errors() {
        for code in [
            ...
            let error = VmError::host(code, "dgram state error");
    ...
    fn host_service_error_code_does_not_parse_diagnostic_messages() {
        let error = VmError::Io(String::from(
    ...
    fn javascript_sync_rpc_error_hides_process_occupancy() {
        let error = VmError::ResourceLimit(LimitError {
    ...
    fn stale_bridge_filter_requires_registry_proof_of_cancellation() {
        let stale = VmError::Execution(String::from(
