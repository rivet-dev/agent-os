...
fn child_executor_capacity_error(
    snapshot: agentos_driver_tokio::VmExecutorAdmissionSnapshot,
) -> Option<VmError> {
...
mod child_executor_capacity_tests {
    ...
    fn saturated_child_spawn_reports_eagain_with_actionable_limit_details() {
        let error =
            child_executor_capacity_error(agentos_driver_tokio::VmExecutorAdmissionSnapshot {
                active: 6,
                maximum: 6,
            })
...
impl ManagedTransferredHostNetSocket {
    pub(super) fn clone_for_fd_transfer(&self) -> Result<Self, VmError> {
        Ok(Self {
...
pub(super) fn register_host_net_transfer_description(
    ...
    let mut descriptions = registry
        .lock()
        .map_err(|_| VmError::host("EIO", "host-network transfer registry lock poisoned"))?;
...
pub(super) fn validate_host_net_metadata_size(value: &Value, label: &str) -> Result<(), VmError> {
    let encoded_len = serde_json::to_vec(value)
        .map_err(|error| VmError::host("EINVAL", format!("invalid {label} metadata: {error}")))?
        .len();
    if encoded_len > HOST_NET_METADATA_MAX_BYTES {
        return Err(VmError::host("E2BIG", format!("{label} metadata is {encoded_len} bytes, exceeding the {HOST_NET_METADATA_MAX_BYTES}-byte limit"
        )));
    }
    fn validate_strings(value: &Value, label: &str) -> Result<(), VmError> {
        match value {
            Value::String(value) if value.len() > HOST_NET_METADATA_MAX_STRING_BYTES => {
                Err(VmError::host("ENAMETOOLONG", format!("{label} metadata string exceeds {HOST_NET_METADATA_MAX_STRING_BYTES} bytes"
                )))
            }
            Value::Array(values) => {
                for value in values {
            ...
            Value::Object(values) => {
                for (key, value) in values {
                    if key.len() > HOST_NET_METADATA_MAX_STRING_BYTES {
                        return Err(VmError::host("ENAMETOOLONG", format!("{label} metadata key exceeds {HOST_NET_METADATA_MAX_STRING_BYTES} bytes"
                        )));
                    }
                    validate_strings(value, label)?;
                }
...
pub(super) fn host_net_open_description_options(
    value: &Value,
    label: &str,
) -> Result<HostNetOpenDescriptionOptions, VmError> {
...
pub(super) fn validate_host_net_metadata(
    ...
    let object = value
        .as_object()
        .ok_or_else(|| VmError::host("EINVAL", format!("{label} metadata must be an object")))?;
    let domain = required_host_net_u32(object, "domain", label)?;
...
pub(super) fn host_net_metadata_mismatch(label: &str, field: &str) -> VmError {
    VmError::host(
        "EINVAL",
...
) -> Result<SpawnHostNetSource, VmError> {
...
pub(super) fn validate_host_net_resource_id(id: &str, label: &str) -> Result<(), VmError> {
    if id.len() > 256 {
        return Err(VmError::host(
            "ENAMETOOLONG",
...
pub(super) fn scm_rights_host_net_source(
    value: &Value,
) -> Result<Option<SpawnHostNetSource>, VmError> {
...
pub(super) fn transferred_hostnet_value(
    ...
    capability_identity: Option<(
        agentos_driver_tokio::capability::CapabilityId,
        agentos_driver_tokio::capability::CapabilityGeneration,
    )>,
...
pub(super) fn adopt_kernel_socket_transfer_guard(
    ...
    socket_id: SocketId,
    nonblocking: bool,
) -> Result<agentos_vm_kernel::fd_table::TransferredFd, VmError> {
...
pub(super) fn prepare_transferred_host_net_resource(
    ...
    value: &Value,
    label: &str,
) -> Result<TransferredHostNetSocket, VmError> {
...
pub(super) fn kernel_open_flags_from_wasi(oflag: i32) -> u32 {
    ...
    let mut flags = if oflag & 0x1000_0000 != 0 {
        if oflag & 0x0400_0000 != 0 {
            agentos_vm_kernel::fd_table::O_RDWR
        } else {
            agentos_vm_kernel::fd_table::O_WRONLY
        }
    } else {
        agentos_vm_kernel::fd_table::O_RDONLY
    };
    if oflag & 0x0000_0001 != 0 {
        flags |= agentos_vm_kernel::fd_table::O_APPEND;
    }
    if oflag & 0x0000_0004 != 0 {
        flags |= agentos_vm_kernel::fd_table::O_NONBLOCK;
    }
    if oflag & (1 << 12) != 0 {
        flags |= agentos_vm_kernel::fd_table::O_CREAT;
    }
    if oflag & (2 << 12) != 0 {
        flags |= agentos_vm_kernel::fd_table::O_DIRECTORY;
    }
    if oflag & (4 << 12) != 0 {
        flags |= agentos_vm_kernel::fd_table::O_EXCL;
    }
    if oflag & (8 << 12) != 0 {
        flags |= agentos_vm_kernel::fd_table::O_TRUNC;
    }
    if oflag & 0x0100_0000 != 0 {
        flags |= agentos_vm_kernel::fd_table::O_NOFOLLOW;
    }
    flags
}
...
mod direct_runtime_stdio_mapping_tests {
    ...
    fn posix_spawn_signal_attributes_are_committed_to_the_kernel_child() {
        ...
        parent
            .signal_action(
                libc::SIGUSR1,
                Some(agentos_vm_kernel::process_table::SignalAction {
                    disposition: agentos_vm_kernel::process_table::SignalDisposition::Ignore,
                    ..agentos_vm_kernel::process_table::SignalAction::DEFAULT
                }),
            )
            .unwrap();
        parent
...
) -> Result<PreparedSpawnHostNetFds, VmError> {
    const LINUX_GUEST_FD_LIMIT: u32 = 1 << 20;
    if let Some(limit) = kernel.resource_limits().max_open_fds {
        if inherited_fds.len() > limit {
            return Err(VmError::host("EMFILE", format!("inherited host-network fd list has {} entries, exceeding limits.resources.maxOpenFds ({limit}); raise limits.resources.maxOpenFds",
                inherited_fds.len()
            )));
        }
    }
        ...
        let managed_description = managed_description_id
            .map(|description_id| {
                managed_descriptions
                    ...
                    .cloned()
                    .ok_or_else(|| {
                        VmError::host("ENOTSOCK", "managed spawn description is unknown")
                    })
            })
            .transpose()?;
        let source = if let Some(description) = managed_description.as_ref() {
            match description.route_for(parent.kernel_pid).ok_or_else(|| {
                VmError::host("ESTALE", "managed spawn description has no parent route")
            })? {
                ManagedHostNetRoute::TcpSocket(id) | ManagedHostNetRoute::UnixSocket(id) => {
                    SpawnHostNetSource::Tcp(id.clone())
                }
...
pub(super) fn apply_spawn_host_net_file_actions(
    ...
    actions: &[ProcessSpawnFileAction],
) -> Result<Vec<ProcessSpawnFileAction>, VmError> {
...
pub(super) fn apply_posix_spawn_file_actions(
    ...
    actions: &[ProcessSpawnFileAction],
) -> Result<(AppliedPosixSpawnFileActions, String), VmError> {
            ...
            1 => {
                ...
                let fd = if let Some(fd) = mappings.remove(&guest_fd) {
                    ...
                    if mapped_kernel_fds.contains(&raw_fd) {
                        return Err(VmError::host("EBADF", format!("posix_spawn guest fd {guest_fd} collides with another mapped descriptor"
                        )));
                    }
                    Some(raw_fd)
                };
            ...
            2 => {
                ...
                if guest_source_fd == guest_fd {
                    let fd = if let Some(fd) = mappings.get(&guest_source_fd).copied() {
                        ...
                        return Err(VmError::host("EBADF", format!("posix_spawn dup2 source guest fd {guest_source_fd} is not kernel-backed"
                        )));
                    } else {
                ...
                let source_fd = if let Some(fd) = mappings.get(&guest_source_fd).copied() {
                    ...
                    if mapped_kernel_fds.contains(&raw_fd) {
                        return Err(VmError::host("EBADF", format!("posix_spawn dup2 source guest fd {guest_source_fd} collides with another mapped descriptor"
                        )));
                    }
                    raw_fd
                };
            ...
            6 => {
                ...
                if let Some(limit) = kernel.resource_limits().max_open_fds {
                    if action.close_from_guest_fds.len() > limit {
                        return Err(VmError::host("EMFILE", format!("posix_spawn closefrom guest fd list has {} entries, exceeding limits.resources.maxOpenFds ({limit}); raise limits.resources.maxOpenFds",
                ...
                for guest_fd in &action.close_from_guest_fds {
                    if *guest_fd < low_fd {
                        return Err(VmError::host("EINVAL", format!("posix_spawn closefrom guest fd {guest_fd} is below cutoff {low_fd}"
                        )));
                    }
                    closed_guest_fds.insert(*guest_fd);
                }
...
pub(super) fn apply_posix_spawn_file_actions_or_rollback(
    ...
    inherited_mappings: &[[u32; 2]],
    actions: &[ProcessSpawnFileAction],
) -> Result<AppliedPosixSpawnFileActions, VmError> {
        ...
        if options.spawn_attr_flags & POSIX_SPAWN_SETSIGDEF != 0 {
            for signal in &options.spawn_signal_defaults {
                ...
                process
                    .signal_action(
                        *signal as i32,
                        Some(agentos_vm_kernel::process_table::SignalAction::DEFAULT),
                    )
                    .map_err(kernel_error)?;
            }
        }
...
pub(super) fn preapply_posix_spawn_file_actions(
    ...
    inherited_mappings: &[[u32; 2]],
    actions: &[ProcessSpawnFileAction],
) -> Result<PreparedPosixSpawnFileActions, VmError> {
    let process = kernel
...
pub(super) fn javascript_spawn_attributes(
    options: &ProcessLaunchOptions,
) -> Result<JavascriptSpawnAttributes, VmError> {
    ...
    let process_group = u32::try_from(pgid)
        .map_err(|_| VmError::InvalidState(format!("invalid POSIX spawn process group {pgid}")))?;
...
fn reserve_child_process_sync_budget(
    ...
    max_buffer: usize,
    input_bytes: usize,
) -> Result<(VmPendingBudgetReservation, VmPendingBudgetReservation), VmError> {
...
fn parse_spawned_child_identity(spawned: &Value) -> Result<SpawnedChildIdentity, VmError> {
...
mod child_process_sync_budget_tests {
    ...
    fn budget(
        limit: usize,
        tracked: agentos_resource_accounting::queue_tracker::TrackedLimit,
    ) -> Arc<VmPendingByteBudget> {
        VmPendingByteBudget::new(limit, tracked)
    }

    fn assert_limit(error: VmError, name: &str, limit: u64, observed: u64) {
        assert_eq!(error.code(), Some("ERR_AGENTOS_RESOURCE_LIMIT"));
        let VmError::Host(host) = error else {
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    ...
    pub(crate) async fn pump_child_process_events(&mut self, vm_id: &str) -> Result<bool, VmError> {
        ...
        if delivery_backpressured {
            ...
            let runtime = self
                .vms
                .get(vm_id)
                .ok_or_else(|| VmError::InvalidState(format!("unknown VM {vm_id}")))?
                .runtime_context
                .clone();
            runtime
                .spawn(agentos_driver_tokio::TaskClass::Timer, async move {
            ...
            if envelope.vm_id == vm_id && envelope.process_id == target_process_id {
                if matches!(child_capacity, Some(0)) {
                    ...
                    return Err(process_event_queue_overflow_error(
                        self.config.protocol.max_process_events,
                    ));
                }
                if let Some(vm) = self.vms.get_mut(vm_id) {
    ...
    pub(crate) fn resolve_javascript_child_process_execution_with_mode(
        ...
        .ok_or_else(|| VmError::host("ENOENT", format!("command not found: {command}")))?;
    ...
    fn child_process_sync_max_buffer(
        ...
        if requested > limit {
            return Err(VmError::host("ERR_AGENTOS_CHILD_PROCESS_BUFFER_LIMIT", format!("child process maxBuffer {requested} exceeds {setting} ({limit}); raise {setting} for larger captured output"
            )));
        }
        Ok(requested)
    }
    ...
    pub(super) async fn begin_javascript_child_process_sync(
        ...
        let registration = (|| -> Result<_, VmError> {
            let vm = self
                .vms
                .get_mut(vm_id)
                .ok_or_else(|| missing_vm_error(vm_id))?;
    ...
    pub(crate) async fn defer_javascript_child_process_sync(
        ...
        Ok(HostServiceResponse::Deferred {
            receiver,
            timeout: None,
            task_class: agentos_driver_tokio::TaskClass::Vm,
        })
    }
    ...
    pub(crate) fn exec_process_image(
        ...
        {
            Some(VmError::Execution(String::from(
    ...
    #[cfg(test)]
    #[allow(dead_code)]
    pub(crate) async fn poll_descendant_process_for_test(
        &mut self,
        vm_id: &str,
        process_id: &str,
        current_process_path: &[&str],
    ...
    fn settle_descendant_managed_network_response(
        ...
        reply: DirectHostReplyHandle,
        operation: &str,
        response: Result<HostServiceResponse, VmError>,
    ) -> Result<(), VmError> {
        ...
        let result = match operation {
            ...
            ProcessOperation::Exec(request) => {
                ...
                match result {
                    ...
                    Ok(()) => {
                        reply.dismiss_claimed().map_err(VmError::from)?;
                        return Ok(());
                    }
                    Err(error) => Err(error),
                }
            ...
            match response {
                ...
                Err(error) if host_service_error_code(&error) == "EAGAIN" => {
                    if arm_deadline_wake {
                        ...
                        let timer =
                            runtime.spawn(agentos_driver_tokio::TaskClass::Timer, async move {
                                let mut deadline =
                        ...
                        match timer {
                            ...
                            Err(agentos_driver_tokio::TaskSpawnError::ResourceLimit(limit)) => {
                                child.clear_deferred_kernel_wait_rpc();
                                let error = VmError::ResourceLimit(limit);
                                request
                                    .reply
                                    .fail(host_service_error(&error))
                                    .map_err(|error| VmError::Execution(error.to_string()))?;
                            ...
                            Err(
                                error @ agentos_driver_tokio::TaskSpawnError::AdmissionClosed {
                                    ..
                                },
                            ) => {
                                child.clear_deferred_kernel_wait_rpc();
    ...
    ) -> Result<Option<InheritedOutputStream>, VmError> {
        let Some(vm) = self.vms.get(vm_id) else {
            return Ok(None);
        };
        ...
        if let Some(child_runtime_pid) = child_runtime_pid {
            match runtime_child_exit_status(child_runtime_pid)? {
                ...
                RuntimeChildStatusObservation::NotWaitable => {
                    return Err(VmError::host("ECHILD", format!("guest runtime process {child_runtime_pid} exited without an observable wait status"
                    )));
                }
            }
        }
