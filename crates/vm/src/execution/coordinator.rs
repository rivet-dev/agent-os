...
fn guest_kernel_core_error(error: crate::core::SidecarCoreError) -> VmError {
    match error.code() {
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
            ...
            let event = {
                ...
                if let Some(event) = process.lease_pending_execution_event() {
                    ...
                    match process.try_poll_execution_event() {
                        Ok(event) => event,
                        Err(VmError::ExecutionEventChannelClosed { .. }) => None,
    ...
    pub(crate) async fn vm_fetch(
        ...
        if matches!(stream_operation.as_deref(), Some("read" | "cancel")) {
            ...
            let (response_json, closed_stream) = if stream_operation.as_deref() == Some("read") {
                loop {
                    ...
                    let result = {
                        let vm = self.vms.get_mut(&vm_id).ok_or_else(|| {
                            VmError::InvalidState(String::from("unknown sidecar VM"))
                        })?;
                ...
                let (response_json, _target_process_id) = {
                    let vm = self
                        .vms
                        .get_mut(&vm_id)
                        .ok_or_else(|| VmError::InvalidState(String::from("unknown sidecar VM")))?;
        ...
        if let Some(target_process_id) = target_process_id {
            ...
            let fetch_result: Result<String, VmError> = async {
                loop {
