...
use agentos_vm_kernel::fd_table::{
...
fn filesystem_operation_is_read_only_mutation(operation: &FilesystemOperation) -> bool {
    use agentos_vm_kernel::fd_table::{O_CREAT, O_RDWR, O_TRUNC, O_WRONLY};
...
) -> Result<ActiveExecutionEvent, VmError> {
...
fn decode_host_operation(
    ...
    full_filesystem: bool,
    max_reply_bytes: usize,
) -> Result<Option<HostOperation>, VmError> {
    let operation = match request.method.as_str() {
        ...
        "process.fd_seek" => {
            ...
            let offset = javascript_sync_rpc_arg_str(&request.args, 1, "fd_seek offset")?
                .parse::<i64>()
                .map_err(|_| VmError::host("EINVAL", "fd_seek offset must be i64"))?;
        ...
        "process.hostnet_fd_open" => {
            ...
            let raw_kind = request
                .args
                .get(kind_index)
                .ok_or_else(|| VmError::host("EINVAL", "host-network socket kind is required"))?;
        ...
        "process.hostnet_recv" => {
            ...
            .map_err(|_| VmError::host("E2BIG", "receive length exceeds usize"))?;
            ...
            HostOperation::Network(NetworkOperation::Receive {
                ...
                max_bytes: BoundedUsize::try_new(requested, &limit).map_err(VmError::from)?,
        ...
        "process.hostnet_send" => {
            ...
            HostOperation::Network(NetworkOperation::Send {
                ...
                bytes: BoundedBytes::try_new(bytes, &limit).map_err(VmError::from)?,
        ...
        "process.hostnet_poll" => {
            let raw = request
                ...
                .ok_or_else(|| {
                    VmError::host("EINVAL", "host-network poll interests must be an array")
                })?;
            let interests = raw
                .iter()
                .map(|value| {
                    let entry = value.as_object().ok_or_else(|| {
                        VmError::host("EINVAL", "poll interest must be an object")
                    })?;
                    Ok(PollInterest {
                        fd: entry
                            .get("fd")
                            ...
                            .ok_or_else(|| VmError::host("EINVAL", "poll fd must be u32"))?,
                        readable: entry
                            .get("readable")
            ...
            HostOperation::Network(NetworkOperation::Poll {
                interests: BoundedVec::try_new(interests, &limit).map_err(VmError::from)?,
                deadline_ms: javascript_sync_rpc_arg_u64_optional(
                    &request.args,
                    1,
        ...
        "process.hostnet_tls_connect" => {
            let name_limit =
                PayloadLimit::new("runtime.network.maxHostBytes", 253).map_err(VmError::from)?;
            let alpn_limit =
                PayloadLimit::new("runtime.network.maxAlpnProtocols", 32).map_err(VmError::from)?;
            ...
            HostOperation::Network(NetworkOperation::TlsConnect {
                ...
                .map_err(VmError::from)?,
                alpn: BoundedVec::try_new(alpn, &alpn_limit).map_err(VmError::from)?,
                deadline_ms: javascript_sync_rpc_arg_u64_optional(
                    &request.args,
                    3,
        ...
        "process.fd_sendmsg_rights" => {
            let raw_rights = request
                ...
                .get(2)
                .and_then(Value::as_array)
                .ok_or_else(|| VmError::host("EINVAL", "sendmsg rights must be an array"))?;
            ...
            let rights = raw_rights
                ...
                .map(|value| {
                    value
                        .as_u64()
                        .and_then(|fd| u32::try_from(fd).ok())
                        .ok_or_else(|| VmError::host("EBADF", "sendmsg right must be u32"))
                })
                .collect::<Result<Vec<_>, _>>()?;
            let rights_limit =
                PayloadLimit::new("limits.network.maxScmRights", 253).map_err(VmError::from)?;
            let bytes_limit =
            ...
            HostOperation::Network(NetworkOperation::SendDescriptorRights {
                ...
                .map_err(VmError::from)?,
                rights: BoundedVec::try_new(rights, &rights_limit).map_err(VmError::from)?,
        ...
        "process.fd_recvmsg_rights" => {
            ...
            .map_err(|_| VmError::host("E2BIG", "recvmsg byte limit exceeds usize"))?;
            let max_rights = usize::try_from(javascript_sync_rpc_arg_u64(
                &request.args,
                2,
                "recvmsg maximum rights",
            )?)
            .map_err(|_| VmError::host("E2BIG", "recvmsg rights limit exceeds usize"))?;
            HostOperation::Network(NetworkOperation::ReceiveDescriptorRights {
                ...
                max_bytes: BoundedUsize::try_new(max_bytes, &byte_limit).map_err(VmError::from)?,
                ...
                signal_thread_id: request
                    ...
                    .map(|value| {
                        value
                            ...
                            .and_then(|value| u32::try_from(value).ok())
                            .ok_or_else(|| {
                                VmError::host("EINVAL", "POSIX poll signal thread id is invalid")
                            })
                    })
                    .transpose()?,
            })
        ...
        "dns.lookup" => {
            let payload = request
                ...
                .ok_or_else(|| VmError::host("EINVAL", "dns.lookup requires an object payload"))?;
            let hostname = payload
                .get("hostname")
                .and_then(Value::as_str)
                .ok_or_else(|| VmError::host("EINVAL", "dns.lookup hostname is required"))?;
        ...
        "dns.resolveRawRr" => {
            let payload = request
                ...
                .and_then(Value::as_object)
                .ok_or_else(|| {
                    VmError::host("EINVAL", "dns.resolveRawRr requires an object payload")
                })?;
            let hostname = payload
                .get("hostname")
                .and_then(Value::as_str)
                .ok_or_else(|| VmError::host("EINVAL", "DNS hostname is required"))?;
        ...
        "process.exec_image_read" => {
            let handle =
                javascript_sync_rpc_arg_str(&request.args, 0, "process.exec_image_read handle")?
                    .parse::<u64>()
                    .map_err(|_| VmError::host("EINVAL", "exec image handle must be u64"))?;
            let offset =
                javascript_sync_rpc_arg_str(&request.args, 1, "process.exec_image_read offset")?
                    .parse::<u64>()
                    .map_err(|_| VmError::host("EINVAL", "exec image offset must be u64"))?;
            let requested = usize::try_from(javascript_sync_rpc_arg_u64(
                &request.args,
                2,
                "process.exec_image_read byte length",
            )?)
            .map_err(|_| VmError::host("E2BIG", "exec image read length exceeds usize"))?;
            let encoded_reply = requested
                ...
                .ok_or_else(|| {
                    VmError::host("E2BIG", "exec image encoded reply size overflows usize")
                })?;
            PayloadLimit::new("limits.reactor.maxBridgeResponseBytes", max_reply_bytes)
                .map_err(VmError::from)?
                .admit(encoded_reply)
            ...
            HostOperation::Process(ProcessOperation::ReadExecutableImage {
                handle,
                offset,
                max_bytes: BoundedUsize::try_new(requested, &raw_limit).map_err(VmError::from)?,
            })
        }
        ...
        "process.setgroups" => {
            let values = request
                ...
                .first()
                .and_then(Value::as_array)
                .ok_or_else(|| VmError::host("EINVAL", "setgroups requires an array"))?;
            ...
            let groups = values
                ...
                .map(|value| {
                    value
                        .as_u64()
                        .and_then(|value| u32::try_from(value).ok())
                        .ok_or_else(|| VmError::host("EINVAL", "setgroups entries must be u32"))
                })
                .collect::<Result<Vec<_>, _>>()?;
            ...
            HostOperation::Identity(IdentityOperation::SetSupplementaryGroups {
                groups: BoundedVec::try_new(groups, &limit).map_err(VmError::from)?,
            })
        }
        "process.clock_time" => {
            ...
            let precision_ns = request
                ...
                .transpose()
                .map_err(|_| VmError::host("EINVAL", "clock precision must be u64"))?
                .unwrap_or_default();
            let deterministic_realtime_ns = if clock == GuestClockId::Realtime {
                request
                    ...
                    .transpose()
                    .map_err(|_| {
                        VmError::host("EINVAL", "deterministic realtime must be u64 nanoseconds")
                    })?
            } else {
                None
            };
        ...
        "process.signal_state" => {
            ...
            HostOperation::Signal(SignalOperation::SetAction {
                signal: i32::try_from(signal).map_err(|_| {
                    VmError::host("EINVAL", "process.signal_state signal exceeds i32")
                })?,
        ...
        "process.random_get" => {
            ...
            let requested = usize::try_from(requested).map_err(|_| {
                VmError::host("E2BIG", "process.random_get byte length exceeds usize")
            })?;
            let maximum = MAX_ENTROPY_CHUNK_BYTES.min(max_reply_bytes);
            let limit =
                PayloadLimit::new("maxEntropyChunkBytes", maximum).map_err(VmError::from)?;
            HostOperation::Entropy(EntropyOperation {
                length: BoundedUsize::try_new(requested, &limit).map_err(VmError::from)?,
            })
        }
        ...
        "__kernel_tty_set_size" => {
            ...
            .map_err(|_| VmError::host("EINVAL", "TTY columns exceed u16"))?;
            let rows = u16::try_from(javascript_sync_rpc_arg_u32(
                &request.args,
                2,
                "__kernel_tty_set_size rows",
            )?)
            .map_err(|_| VmError::host("EINVAL", "TTY rows exceed u16"))?;
        ...
        "__kernel_tcsetattr" => {
            ...
            let cc = request
                ...
                .and_then(Value::as_array)
                .ok_or_else(|| {
                    VmError::host("EINVAL", "TTY control characters must be an array")
                })?;
            if cc.len() != 7 {
                return Err(VmError::host(
                    "EINVAL",
            ...
            for (index, value) in cc.iter().enumerate() {
                control_characters[index] = value
                    ...
                    .ok_or_else(|| {
                        VmError::host("EINVAL", "TTY control character must be a byte")
                    })?;
            }
...
fn optional_identity_id(args: &[Value], index: usize, label: &str) -> Result<Option<u32>, VmError> {
...
fn account_record_limit(max_reply_bytes: usize) -> Result<BoundedUsize, VmError> {
    ...
    let limit = PayloadLimit::with_warning_hook("maxAccountRecordBytes", maximum, None)
        .map_err(VmError::from)?;
    BoundedUsize::try_new(maximum, &limit).map_err(VmError::from)
}

fn bounded_account_name(name: &str) -> Result<BoundedString, VmError> {
    let limit =
        PayloadLimit::new("maxAccountNameBytes", MAX_ACCOUNT_NAME_BYTES).map_err(VmError::from)?;
    BoundedString::try_new(name.to_owned(), &limit).map_err(VmError::from)
}

fn bounded_child_process_id(value: &str) -> Result<BoundedString, VmError> {
    let limit = PayloadLimit::new("maxChildProcessIdBytes", MAX_CHILD_PROCESS_ID_BYTES)
        .map_err(VmError::from)?;
    BoundedString::try_new(value.to_owned(), &limit).map_err(VmError::from)
}
...
) -> Result<BoundedProcessLaunchRequest, VmError> {
    let request_limit =
        PayloadLimit::new("limits.reactor.maxBridgeRequestBytes", max_request_bytes)
            .map_err(VmError::from)?;
    request_limit.admit_json(args).map_err(VmError::from)?;
...
fn decode_executable_image_resolution(
    ...
    argv_index: usize,
    max_request_bytes: usize,
) -> Result<Option<BoundedExecutableImageResolutionRequest>, VmError> {
...
fn bounded_dns_value(
    ...
    maximum: usize,
) -> Result<BoundedString, VmError> {
    let limit = PayloadLimit::new(limit_name, maximum).map_err(VmError::from)?;
    BoundedString::try_new(value.to_owned(), &limit).map_err(VmError::from)
}
...
fn decode_hostnet_socket_address(
    value: Option<&Value>,
    label: &str,
) -> Result<SocketAddress, VmError> {
    let value = value
        .and_then(Value::as_object)
        .ok_or_else(|| VmError::host("EINVAL", format!("{label} must be an object")))?;
    let path_limit =
        PayloadLimit::new("runtime.filesystem.maxPathBytes", 4096).map_err(VmError::from)?;
    let host_limit =
        PayloadLimit::new("runtime.network.maxHostBytes", 253).map_err(VmError::from)?;
    match value.get("type").and_then(Value::as_str) {
        Some("inet") => Ok(SocketAddress::Inet {
            host: BoundedString::try_new(
                value
                    .get("host")
                    .and_then(Value::as_str)
                    .ok_or_else(|| VmError::host("EINVAL", format!("{label} host is required")))?
                    .to_owned(),
                &host_limit,
            )
            .map_err(VmError::from)?,
            port: value
                ...
                .ok_or_else(|| VmError::host("EINVAL", format!("{label} port must be u16")))?,
        }),
        Some("unix-path") => Ok(SocketAddress::UnixPath(
            BoundedString::try_new(
                value
                    .get("path")
                    .and_then(Value::as_str)
                    .ok_or_else(|| VmError::host("EINVAL", format!("{label} path is required")))?
                    .to_owned(),
                &path_limit,
            )
            .map_err(VmError::from)?,
        ...
        Some("unix-abstract") => {
            let bytes = value
                .get("hex")
                .and_then(Value::as_str)
                .ok_or_else(|| VmError::host("EINVAL", format!("{label} hex is required")))?;
            if !bytes.len().is_multiple_of(2) {
                return Err(VmError::host("EINVAL", format!("{label} hex is invalid")));
            }
            let decoded = bytes
                .as_bytes()
                .chunks_exact(2)
            ...
            Ok(SocketAddress::UnixAbstract(
                BoundedBytes::try_new(decoded, &path_limit).map_err(VmError::from)?,
            ))
        }
...
fn decode_hostnet_socket_option(value: &str) -> Result<SocketOptionName, VmError> {
    match value {
...
fn decode_hostnet_socket_option_value(
    value: Option<&Value>,
    label: &str,
) -> Result<SocketOptionValue, VmError> {
    let value = value.ok_or_else(|| VmError::host("EINVAL", format!("{label} is required")))?;
    ...
    let object = value
        .as_object()
        .ok_or_else(|| VmError::host("EINVAL", format!("{label} is invalid")))?;
    if let Some(duration) = object.get("durationMs") {
        return Ok(SocketOptionValue::DurationMs(if duration.is_null() {
            ...
            Some(duration.as_u64().ok_or_else(|| {
                VmError::host("EINVAL", format!("{label} durationMs must be u64"))
            })?)
        }));
    }
    if let Some(enabled) = object.get("enabled").and_then(Value::as_bool) {
        let seconds = object
            ...
            .and_then(Value::as_u64)
            .and_then(|value| u32::try_from(value).ok())
            .ok_or_else(|| VmError::host("EINVAL", format!("{label} seconds must be u32")))?;
        return Ok(SocketOptionValue::Linger { enabled, seconds });
    }
    Err(VmError::host("EINVAL", format!("{label} is invalid")))
}

fn payload_limit_error(limit_name: &'static str, limit: usize, requested: usize) -> VmError {
...
fn decode_clock(raw: u32) -> Result<GuestClockId, VmError> {
    match raw {
...
fn decode_resource_limit(raw: u32) -> Result<ResourceLimitKind, VmError> {
    match raw {
...
fn decode_wait_target(selector: i32) -> Result<WaitTarget, VmError> {
    match selector {
        -1 => Ok(WaitTarget::Any),
        ...
        selector => selector
            ...
            .ok_or_else(|| VmError::host("EINVAL", "invalid waitpid selector")),
    }
}

fn decode_wait_options(options: u32) -> Result<u32, VmError> {
...
fn decode_rlim(args: &[Value], index: usize, label: &str) -> Result<Option<u64>, VmError> {
    let Some(value) = args.get(index) else {
        return Err(VmError::host("EINVAL", format!("{label} is required")));
    };
    let value = if let Some(text) = value.as_str() {
        text.parse::<u64>()
            .map_err(|_| VmError::host("EINVAL", format!("{label} must be u64")))?
    } else {
...
fn decode_signal_set(value: Option<&Value>) -> Result<SignalSetValue, VmError> {
    let signals = value
        .and_then(Value::as_array)
        .ok_or_else(|| VmError::host("EINVAL", "signal-mask set must be an array"))?;
...
fn signal_set_from_u32(signals: impl IntoIterator<Item = u32>) -> Result<SignalSetValue, VmError> {
    let mut bits = 0_u64;
...
pub(super) fn dispatch_host_operation(
...
) -> Result<HostOperationEffects, VmError> {
    ...
    match result {
        Ok(Some(response)) => {
            reply.succeed(response).map_err(VmError::from)?;
            Ok(effects)
        }
...
pub(super) fn service_deferred_guest_wait(
    ...
    wait_handle: agentos_vm_kernel::process_table::ProcessWaitHandle,
    ...
    if let Some(result) = result {
        return match result {
            Ok(value) => wait
                ...
                .map_err(VmError::from),
            Err(error) => wait.reply.fail(error).map_err(VmError::from),
        };
    }
    ...
    let task_class = if wait.kind == DeferredGuestWaitKind::Sleep {
        agentos_driver_tokio::TaskClass::Timer
    } else {
        agentos_driver_tokio::TaskClass::Vm
    };
...
fn claim_context_host_work(reply: &DirectHostReplyHandle) -> Result<bool, VmError> {
    match reply.claim() {
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    ...
    if let Err(error) = runtime.spawn(agentos_driver_tokio::TaskClass::Dns, async move {
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    ...
    let preflight = (|| {
        let url = Url::parse(url.as_str())
            .map_err(|error| VmError::host("ERR_INVALID_URL", error.to_string()))?;
        ...
        let host = url
            .host_str()
            .ok_or_else(|| VmError::host("ERR_INVALID_URL", "outbound HTTP URL has no host"))?
            .to_owned();
        let port = url
            .port_or_known_default()
            .ok_or_else(|| VmError::host("ERR_INVALID_URL", "outbound HTTP URL has no port"))?;
    ...
    if let Err(error) = runtime.spawn(agentos_driver_tokio::TaskClass::Socket, async move {
        let result = task_runtime
            .blocking()
            .run(reserved_bytes, move || issue_bounded_http_request(request))
            .await
            .map_err(|error| host_service_error(&VmError::from(error)))
...
pub(super) fn merge_process_internal_bootstrap_env<B>(
    ...
    let vm = sidecar
        .vms
        .get(vm_id)
        .ok_or_else(|| VmError::host("ESTALE", "host call VM no longer exists"))?;
...
fn settle_context_process_reply(
    reply: &DirectHostReplyHandle,
    result: Result<HostCallReply, VmError>,
) -> Result<(), VmError> {
    match result {
...
mod tests {
    ...
    fn context_preflight_failure_settles_the_original_typed_error() {
        ...
        assert!(settle_context_preflight::<()>(
            &reply,
            Err(VmError::host("ERR_INVALID_URL", "invalid URL")),
        )
    ...
    fn process_preflight_and_post_claim_failures_preserve_the_original_errno() {
        ...
        settle_context_process_reply(
            &preflight_reply,
            Err(VmError::host("EINVAL", "invalid launch options")),
        )
        ...
        settle_context_process_reply(
            &claimed_reply,
            Err(VmError::host("EPIPE", "child stdin closed")),
        )
