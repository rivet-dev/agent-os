        ...
        Err(error)
            if matches!(error.code(), "EAGAIN" | "EWOULDBLOCK")
                && kernel
                    ...
                    .map(|stat| stat.flags & agentos_vm_kernel::fd_table::O_NONBLOCK != 0)
...
pub(super) fn decode(
...
) -> Result<Option<FilesystemOperation>, VmError> {
    ...
    let output_count = |value: u64, label: &str| {
        let value = usize::try_from(value)
            .map_err(|_| VmError::host("E2BIG", format!("{label} exceeds usize")))?;
        response_limit
            ...
            .map_err(VmError::Host)?;
        BoundedUsize::try_new(value, &response_limit).map_err(VmError::Host)
    };
    ...
    let operation = match request.method.as_str() {
            ...
            exact_fds: request
                ...
                .map(|value| {
                    let values = value.as_array().ok_or_else(|| {
                        VmError::host("EINVAL", "fd_closefrom canonical targets must be an array")
                    })?;
                    let fds = values
                        .iter()
                        .map(|value| {
        ...
        "process.fd_record_lock" => FilesystemOperation::RecordLock {
            ...
            kind: match javascript_sync_rpc_arg_u32(&request.args, 2, "fd_record_lock type")? {
                ...
                _ => return Err(VmError::host("EINVAL", "invalid fd_record_lock type")),
            },
...
fn payload_limit(
    name: &'static str,
    maximum: usize,
) -> Result<crate::executor::backend::PayloadLimit, VmError> {
    crate::executor::backend::PayloadLimit::new(name, maximum).map_err(VmError::Host)
}
...
fn parse_u64_string(request: &HostRpcRequest, index: usize, label: &str) -> Result<u64, VmError> {
...
fn decode_path_operation(
    request: &HostRpcRequest,
    path: &impl Fn(usize, &str) -> Result<BoundedString, VmError>,
) -> Result<FilesystemOperation, VmError> {
    ...
    Ok(match request.method.as_str() {
        ...
        "process.path_readlink_at" => FilesystemOperation::ReadLinkAt {
            ...
            max_bytes: BoundedUsize::try_new(MAX_PATH_BYTES, &path_bound).map_err(VmError::Host)?,
        },
        ...
        _ => return Err(VmError::host("ENOSYS", "unknown path operation")),
    })
}
        ...
        let value = match operation {
            ...
            FilesystemOperation::Seek { fd, offset, whence } => {
                let whence = match whence {
                    DescriptorWhence::Set => agentos_vm_kernel::kernel::SEEK_SET,
                    DescriptorWhence::Current => agentos_vm_kernel::kernel::SEEK_CUR,
                    DescriptorWhence::End => agentos_vm_kernel::kernel::SEEK_END,
                    DescriptorWhence::Data => agentos_vm_kernel::kernel::SEEK_DATA,
                    DescriptorWhence::Hole => agentos_vm_kernel::kernel::SEEK_HOLE,
                };
                Value::String(
                    kernel
            ...
            FilesystemOperation::RecordLock {
                ...
                let kind = match kind {
                    FilesystemRecordLockKind::Read => {
                        agentos_vm_kernel::fd_table::RecordLockType::Read
                    }
                    FilesystemRecordLockKind::Write => {
                        agentos_vm_kernel::fd_table::RecordLockType::Write
                    }
                    FilesystemRecordLockKind::Unlock => {
                        agentos_vm_kernel::fd_table::RecordLockType::Unlock
                    }
                };
                ...
                if let (Some(atime_ms), Some(mtime_ms)) = (update.atime_ms, update.mtime_ms) {
                    kernel
                        .utimes_spec_for_process(
                            ...
                            pid,
                            &path,
                            agentos_vm_kernel::vfs::VirtualUtimeSpec::Set(
                                agentos_vm_kernel::vfs::VirtualTimeSpec::from_millis(atime_ms),
                            ),
                            agentos_vm_kernel::vfs::VirtualUtimeSpec::Set(
                                agentos_vm_kernel::vfs::VirtualTimeSpec::from_millis(mtime_ms),
                            ),
                            follow_symlinks,
                        )
                        .map_err(kernel_host_error)?;
...
fn sidecar_host_error(error: VmError) -> HostServiceError {
    match error {
        VmError::Host(error) => error,
...
fn resolve_path(
    ...
    if dir_fd == NODE_CWD_FD {
        ...
        if path
            .split('/')
            .any(agentos_vm_kernel::kernel::is_internal_unnamed_file_name)
        {
            return Err(HostServiceError::new(
                "ENOENT",
...
fn preopen_value(preopen: agentos_vm_kernel::kernel::ProcessWasiPreopen) -> Value {
    json!({
        "fd": preopen.fd,
...
fn node_stat_value(stat: agentos_vm_kernel::vfs::VirtualStat) -> Value {
    json!({
        "mode": stat.mode,
        "size": stat.size,
        "blocks": stat.blocks,
...
fn wasi_path_stat_value(stat: agentos_vm_kernel::vfs::VirtualStat) -> Value {
...
fn wasi_stat_value(stat: agentos_vm_kernel::vfs::VirtualStat, filetype: u8) -> Value {
    json!({
        "dev": stat.dev,
        "ino": stat.ino,
        "filetype": filetype,
...
fn time_spec(
    ...
    now: bool,
) -> Result<agentos_vm_kernel::vfs::VirtualUtimeSpec, HostServiceError> {
...
mod tests {
    ...
    fn test_runtime_context() -> agentos_driver_tokio::DriverHandle {
    ...
    fn real_wasi_dirfd_and_explicit_zero_rights_cannot_be_waived_by_absolute_paths() {
        ...
        let zero_dir = kernel
            .fd_open_with_rights(
                ...
                Some(root.fd),
                "/cap",
                agentos_vm_kernel::fd_table::O_DIRECTORY,
                None,
                Some((0, 0)),
            )
    ...
    fn sufficient_wasi_dirfd_rights_still_confine_absolute_paths() {
        ...
        assert_eq!(
            resolve_path(
                &mut kernel,
                &process,
                agentos_vm_kernel::fd_table::WASI_HIDDEN_PREOPEN_FD_TAG | cap_fd,
                "/outside"
            )
