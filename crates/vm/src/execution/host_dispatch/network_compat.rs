...
use agentos_vm_kernel::process_table::SignalSet;
use agentos_vm_kernel::socket_table::SocketType;
...
pub(in crate::execution) fn deferred_posix_poll_wake_lane<B>(
    ...
    vm_id: &str,
    process_id: &str,
) -> Result<DeferredPosixPollWakeLane, VmError> {
    let vm = sidecar
        .vms
        .get(vm_id)
...
fn posix_signal_set(set: SignalSetValue) -> Result<SignalSet, VmError> {
        ...
        if interest.events & POSIX_READ_EVENTS != 0 {
            let readable = match route {
                ...
                ManagedHostNetRoute::TcpListener(ref listener_id) => process
                    .tcp_listeners
                    .get_mut(listener_id)
                    .ok_or_else(|| VmError::host("EBADF", "managed TCP listener is stale"))?
                    .probe_readable(kernel, process.kernel_pid, trace_enabled)?,
                ManagedHostNetRoute::UnixListener(ref listener_id) => process
                    .unix_listeners
                    .get_mut(listener_id)
                    .ok_or_else(|| VmError::host("EBADF", "managed Unix listener is stale"))?
                    .probe_readable()?,
                ManagedHostNetRoute::UdpSocket(ref socket_id) => {
                    let socket = process
                        .udp_sockets
                        .get(socket_id)
                        .ok_or_else(|| VmError::host("EBADF", "managed UDP socket is stale"))?;
                    socket
                        .pending_datagram
                        .lock()
                        .map_err(|_| VmError::host("EIO", "UDP pending datagram lock poisoned"))?
                        .is_some()
                        || socket
...
fn native_tcp_listener_waiters(
    ...
    interests: &[KernelPollInterest],
) -> Result<Vec<tokio::io::unix::AsyncFd<std::net::TcpListener>>, VmError> {
        ...
        .filter(|interest| interest.events & POSIX_READ_EVENTS != 0)
    {
        ...
        waiters.push(
            tokio::io::unix::AsyncFd::new(listener.try_clone().map_err(|error| {
                VmError::host("EIO", format!("clone TCP listener for poll: {error}"))
            })?)
            .map_err(|error| {
                VmError::host("EIO", format!("register TCP listener poll waiter: {error}"))
            })?,
        );
    }
    Ok(waiters)
...
fn managed_posix_poll_read_notifies(
    ...
    process: &ActiveProcess,
    interests: &[KernelPollInterest],
) -> Result<Vec<Arc<tokio::sync::Notify>>, VmError> {
    let descriptions = managed_descriptions
        .lock()
        .map_err(|_| VmError::host("EIO", "managed description registry lock poisoned"))?;
...
fn fail_deferred_posix_poll(
    ...
    poll.reply.fail(failure).map_err(VmError::from)
}
...
pub(in crate::execution) fn service_deferred_posix_poll(
    ...
    wait_handle: agentos_vm_kernel::poll::PollWaitHandle,
    notify: Arc<tokio::sync::Notify>,
    socket_paths: &SocketPathContext,
    kernel_readiness: KernelSocketReadinessRegistry,
    capabilities: CapabilityRegistry,
    ...
    if ready || poll.deadline.is_some_and(|deadline| now >= deadline) {
        if let Err(error) = restore_deferred_posix_mask(process, &mut poll) {
            return poll.reply.fail(error).map_err(VmError::from);
        }
        if poll.combined {
    ...
    match wake_task {
        ...
        Err(error) => {
            fail_deferred_posix_poll(process, poll, host_service_error(&VmError::from(error)))
        }
    }
}
...
) -> Result<Option<NetworkOperation>, VmError> {
    let id_limit = PayloadLimit::new("runtime.network.maxCapabilityIdBytes", MANAGED_ID_BYTES)
        .map_err(VmError::Host)?;
    let host_limit =
        PayloadLimit::new("runtime.network.maxHostBytes", HOST_BYTES).map_err(VmError::Host)?;
    ...
    let operation = match request.method.as_str() {
        ...
        "net.socket_upgrade_tls" => {
            ...
            serde_json::from_str::<TlsBridgeOptions>(options).map_err(|error| {
                VmError::host("EINVAL", format!("invalid TLS options: {error}"))
            })?;
...
fn bounded_optional(
    value: Option<String>,
    limit: &PayloadLimit,
) -> Result<Option<BoundedString>, VmError> {
    value
        .map(|value| BoundedString::try_new(value, limit).map_err(VmError::Host))
        .transpose()
}
...
fn optional_u64(request: &HostRpcRequest, index: usize, label: &str) -> Result<u64, VmError> {
    Ok(javascript_sync_rpc_arg_u64_optional(&request.args, index, label)?.unwrap_or_default())
}
fn optional_bool(request: &HostRpcRequest, index: usize, label: &str) -> Result<bool, VmError> {
    match request.args.get(index) {
        None | Some(Value::Null) => Ok(false),
        Some(Value::Bool(value)) => Ok(*value),
        Some(_) => Err(VmError::host(
            "EINVAL",
...
fn decode_value_arg<T: serde::de::DeserializeOwned>(
    ...
    serde_json::from_value(
        request
            ...
            .get(index)
            .cloned()
            .ok_or_else(|| VmError::host("EINVAL", format!("{label} payload is required")))?,
    )
    .map_err(|error| VmError::host("EINVAL", format!("invalid {label} payload: {error}")))
}
...
fn decode_optional_unix(
    ...
    autobind: bool,
    limit: &PayloadLimit,
) -> Result<Option<ManagedUnixAddress>, VmError> {
...
fn decode_unix_address(
    ...
    autobind: bool,
    limit: &PayloadLimit,
) -> Result<ManagedUnixAddress, VmError> {
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    ...
    let (runtime, connection_id, session_id, notify, response) = {
        ...
        let process = VmManager::<B>::active_process_by_owned_path_mut(root, &pending.process_path)
            .ok_or_else(|| {
                VmError::host("ESTALE", "managed stream read target process disappeared")
            })?;
...
fn managed_description<B>(
    context: &ManagedFdNetworkServiceContext<'_, B>,
    description_id: u64,
) -> Result<ManagedHostNetDescription, VmError> {
    context
        .managed_descriptions
        .lock()
        .map_err(|_| VmError::host("EIO", "managed description registry lock poisoned"))?
        .get(&description_id)
        .cloned()
        .ok_or_else(|| VmError::host("ENOTSOCK", "managed socket description disappeared"))
}
...
fn managed_route<B>(
    context: &ManagedFdNetworkServiceContext<'_, B>,
    description_id: u64,
) -> Result<ManagedHostNetRoute, VmError> {
...
fn managed_unix_address(address: &SocketAddress) -> Result<ManagedUnixAddress, VmError> {
    match endpoint_for_address(address).unix {
        Some(address) => Ok(address),
        None => Err(VmError::host("EAFNOSUPPORT", "expected an AF_UNIX address")),
    }
}
...
pub(in crate::execution) fn managed_socket_address_from_info(
    info: &Value,
    peer: bool,
) -> Result<Option<SocketAddress>, VmError> {
    ...
    if let (Some(host), Some(port)) = (
        ...
        let port = u16::try_from(port)
            .map_err(|_| VmError::host("EINVAL", "socket endpoint port exceeds u16"))?;
    ...
    match operation {
        ...
        NetworkOperation::Bind { fd, address } => {
            ...
            let description = descriptions.get(&description_id).cloned().ok_or_else(|| {
                VmError::host("ENOTSOCK", "managed socket description disappeared")
            })?;
            let current_route = description
                .route_for(context.process.kernel_pid)
                .cloned()
                .ok_or_else(|| VmError::host("ESTALE", "managed socket route is missing"))?;
            ...
            {
                return Err(VmError::host("EINVAL", "socket is already bound"));
            }
            let response = match (description.domain, description.kind) {
                ...
                (HostSocketDomain::Inet4 | HostSocketDomain::Inet6, SocketKind::Datagram) => {
                    let ManagedHostNetRoute::UdpSocket(socket_id) = &current_route else {
                        return Err(VmError::host("EIO", "UDP socket route is missing"));
                    };
                ...
                _ => return Err(VmError::host("EOPNOTSUPP", "unsupported socket bind")),
            };
            let route = match (description.domain, description.kind) {
                (HostSocketDomain::Unix, SocketKind::Stream) => ManagedHostNetRoute::UnixBound {
                    listener_id: response_string_field(&response, "serverId")
                        .ok_or_else(|| VmError::host("EIO", "Unix bind omitted listener id"))?,
                },
                (HostSocketDomain::Inet4 | HostSocketDomain::Inet6, SocketKind::Stream) => {
                    ManagedHostNetRoute::TcpBound {
                        reservation_id: response_string_field(&response, "reservationId")
                            .ok_or_else(|| {
                                VmError::host("EIO", "TCP bind omitted reservation id")
                            })?,
                    }
                }
        ...
        NetworkOperation::Listen { fd, backlog } => {
            ...
            let mut endpoint = match description.bound_address.as_ref() {
                ...
                None => return Err(VmError::host("EINVAL", "Unix listener must be bound")),
            };
            ...
            let listener_id = response_string_field(&response, "serverId")
                .ok_or_else(|| VmError::host("EIO", "listen omitted listener id"))?;
            let route = if description.domain == HostSocketDomain::Unix {
                ManagedHostNetRoute::UnixListener(listener_id)
            } else {
                ManagedHostNetRoute::TcpListener(listener_id)
        ...
        NetworkOperation::Accept {
            ...
            let parent = descriptions.get(&description_id).cloned().ok_or_else(|| {
                VmError::host("ENOTSOCK", "managed listener description disappeared")
            })?;
            let parent_route = parent
                .route_for(context.process.kernel_pid)
                .cloned()
                .ok_or_else(|| VmError::host("ESTALE", "managed listener route is missing"))?;
            ...
            let mut value = match response_value(&response) {
                ...
                _ => {
                    ...
                    return Err(VmError::host("EIO", "accept returned an invalid response"));
                }
            };
            ...
            if !expected_socket_present {
                rollback_managed_stream_socket(&mut context, &socket_id);
                return Err(VmError::host("EIO", "accept returned an unknown socket id"));
            }
        ...
        NetworkOperation::Receive {
            ...
            match route {
                ...
                ManagedHostNetRoute::UdpSocket(_) => Err(VmError::host(
                    "ERR_AGENTOS_CONTEXT_DISPATCH_REQUIRED",
                    "UDP receive requires the asynchronous fd poll dispatcher",
                )),
                _ => Err(VmError::host("ENOTCONN", "socket is not connected")),
            }
        }
        ...
        NetworkOperation::LocalAddress { fd } | NetworkOperation::PeerAddress { fd } => {
            ...
            if peer {
                return Err(VmError::host("ENOTCONN", "socket has no peer address"));
            }
            ...
            Err(VmError::host("EINVAL", "socket is not bound"))
        }
            ...
            match (name, value) {
                ...
                (SocketOptionName::NoDelay, SocketOptionValue::Bool(value)) => {
                    let route = descriptions
                        ...
                        .ok_or_else(|| {
                            VmError::host("ESTALE", "managed socket route is missing")
                        })?;
                    let ManagedHostNetRoute::TcpSocket(id) = &route else {
                        return Err(VmError::host("ENOPROTOOPT", "TCP_NODELAY requires TCP"));
                    };
                    context
                        .process
                        .tcp_sockets
                        .get_mut(id)
                        .ok_or_else(|| VmError::host("EBADF", "managed TCP route is stale"))?
                        .set_no_delay(value)?;
                    descriptions
                        .get_mut(&description_id)
                ...
                (SocketOptionName::KeepAlive, SocketOptionValue::Bool(value)) => {
                    ...
                    let ManagedHostNetRoute::TcpSocket(id) = &route else {
                        return Err(VmError::host("ENOPROTOOPT", "SO_KEEPALIVE requires TCP"));
                    };
                    context
                        .process
                        .tcp_sockets
...
fn bounded_managed_id(value: String) -> Result<BoundedString, VmError> {
    BoundedString::try_new(
        value,
...
fn bounded_managed_payload(value: String) -> Result<BoundedString, VmError> {
    BoundedString::try_new(
        value,
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    ...
    let path = pending
        ...
        .map(String::as_str)
        .collect::<Vec<_>>();
    VmManager::<B>::active_process_by_path(root, &path)
}
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    ...
    let path = pending
        ...
        .map(String::as_str)
        .collect::<Vec<_>>();
    VmManager::<B>::active_process_by_path_mut(root, &path)
}
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    ...
    if let Err(error) = runtime.spawn(agentos_driver_tokio::TaskClass::Udp, async move {
...
fn udp_reentry_context<B>(
    sidecar: &VmManager<B>,
    vm_id: &str,
) -> Result<(agentos_driver_tokio::DriverHandle, String, String), VmError> {
    let vm = sidecar
        .vms
        .get(vm_id)
        .ok_or_else(|| VmError::host("ESTALE", "UDP-poll VM no longer exists"))?;
    Ok((
...
fn buffered_udp_poll_event(
    poll_handle: &ActiveUdpPollHandle,
    peek: bool,
) -> Result<Option<DatagramEvent>, VmError> {
...
mod managed_tests {
    ...
    fn closed_udp_reentry_lane_cancels_the_claimed_reply() {
        ...
        drop(receiver);
        runtime.handle().tokio_handle().block_on(send_udp_reentry(
            sender,
    ...
    fn deferred_posix_poll_wake_is_durable_with_a_competing_broker_waiter() {
        ...
        let envelope = runtime.handle().tokio_handle().block_on(async move {
        ...
        runtime.handle().tokio_handle().block_on(async move {
    ...
    fn managed_posix_poll_waits_on_socket_readiness_after_broker_wake_is_consumed() {
        ...
        runtime.handle().tokio_handle().block_on(async {
