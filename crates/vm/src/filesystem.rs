...
use crate::core::handle_guest_filesystem_call as core_guest_filesystem_call;
...
use agentos_vm_kernel::kernel::is_internal_unnamed_file_name;
use agentos_vm_kernel::vfs::{VirtualStat, VirtualTimeSpec, VirtualUtimeSpec};
use nix::libc;
...
fn kernel_path_error(
    operation: &str,
    path: &str,
    error: impl Into<agentos_vm_kernel::kernel::KernelError>,
) -> VmError {
    let error = error.into();
    let base = VmError::host(
        error.code(),
...
fn parse_timespec_seconds(value: f64, label: &str) -> Result<VirtualTimeSpec, VmError> {
    if !value.is_finite() {
    ...
    VirtualTimeSpec::new(sec, nanos as u32)
        .map_err(|error| VmError::InvalidState(format!("{label}: {error}")))
}

fn parse_timespec_integer(value: &Value, label: &str) -> Result<i64, VmError> {
    value
        .as_i64()
        .or_else(|| value.as_u64().and_then(|value| i64::try_from(value).ok()))
        .ok_or_else(|| VmError::InvalidState(format!("{label} must be an integer")))
}

fn parse_utime_spec_value(value: &Value, label: &str) -> Result<VirtualUtimeSpec, VmError> {
    ...
    let sec_value = object
        .get("sec")
        .ok_or_else(|| VmError::InvalidState(format!("{label} timespec requires sec")))?;
    let sec = parse_timespec_integer(sec_value, &format!("{label}.sec"))?;
    let nsec = u32::try_from(parse_timespec_integer(
        nsec_value,
        &format!("{label}.nsec"),
    )?)
    .map_err(|_| VmError::InvalidState(format!("{label}.nsec must fit within u32")))?;
    VirtualTimeSpec::new(sec, nsec)
        .map(VirtualUtimeSpec::Set)
        .map_err(|error| VmError::InvalidState(format!("{label}: {error}")))
}

fn parse_utime_arg(args: &[Value], index: usize, label: &str) -> Result<VirtualUtimeSpec, VmError> {
    let value = args
        .get(index)
        .ok_or_else(|| VmError::InvalidState(format!("{label} is required")))?;
...
fn native_guest_filesystem_core_error(error: crate::core::SidecarCoreError) -> VmError {
    match error.code() {
        Some(code) => VmError::Host(crate::executor::backend::HostServiceError::new(
            code,
            error.message(),
        )),
...
fn module_kernel_optional<T>(
    result: Result<T, agentos_vm_kernel::kernel::KernelError>,
...
pub(crate) fn service_javascript_module_sync_rpc(
    ...
    let value = (|| -> Result<Value, VmError> {
...
pub(crate) fn service_javascript_fs_read_sync_rpc(
    ...
    .map_err(|_| {
        VmError::InvalidState("filesystem read length must fit within usize".to_string())
    })?;
    let position =
    ...
    match request.method.as_str() {
        ...
        "fs.writevSync" => {
            ...
            let contents = request.raw_bytes_args.get(&1).ok_or_else(|| {
                VmError::InvalidState(String::from("filesystem writev requires raw byte payload"))
            })?;
            let position = javascript_sync_rpc_arg_u64_optional(
                &request.args,
                2,
        ...
        "fs.remountSync" => {
            ...
            let options = request.args.get(1).and_then(Value::as_str).ok_or_else(|| {
                VmError::InvalidState(String::from("filesystem remount options must be a string"))
            })?;
            kernel
...
fn read_le_u32(payload: &[u8], offset: &mut usize, label: &str) -> Result<u32, VmError> {
    let end = offset
        .checked_add(4)
        .ok_or_else(|| VmError::InvalidState(format!("filesystem {label} offset overflow")))?;
    let bytes = payload
        .get(*offset..end)
        .ok_or_else(|| VmError::InvalidState(format!("truncated filesystem {label} payload")))?;
    *offset = end;
...
fn decode_javascript_writev_raw_payload(payload: &[u8]) -> Result<Vec<&[u8]>, VmError> {
    let mut offset = 0usize;
    ...
    for _ in 0..count {
        ...
        let end = offset.checked_add(len).ok_or_else(|| {
            VmError::InvalidState(String::from("filesystem writev payload length overflow"))
        })?;
        let buffer = payload.get(offset..end).ok_or_else(|| {
            VmError::InvalidState(String::from("truncated filesystem writev payload"))
        })?;
        buffers.push(buffer);
        offset = end;
    }
...
pub(crate) fn service_javascript_fs_readdir_entries(
    ...
    kernel_pid: u32,
    path: &str,
) -> Result<BTreeMap<String, bool>, VmError> {
    let entries = kernel
...
fn encode_javascript_readdir_raw_payload(
        ...
        .filter(|(name, _)| name != "." && name != "..")
    {
        ...
        let name_len = u32::try_from(name.len()).map_err(|_| {
            VmError::InvalidState(String::from("filesystem readdir entry name too long"))
        })?;
...
mod tests {
    ...
    //   cargo test -p agentos-vm --lib module_resolution_vfs_vs_host_cold_start_perf -- --nocapture --ignored
    #[cfg(any(
        feature = "node-v8",
        feature = "python-v8-pyodide",
        feature = "wasm-v8"
