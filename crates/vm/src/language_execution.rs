...
fn lower_operation(payload: RequestPayload) -> Result<LoweredOperation, VmError> {
        ...
        RequestPayload::TypeScriptCheck(payload) => {
            ...
            let request = serde_json::json!({
                ...
                "compilerOptions": payload
                    ...
                    .transpose()
                    .map_err(|error| VmError::InvalidState(format!("invalid TypeScript compiler options: {error}")))?,
            });
...
fn typed_rejection(request: &RequestFrame, code: &str, message: impl AsRef<str>) -> DispatchResult {
    DispatchResult {
        response: crate::core::reject(request, code, message.as_ref()),
        events: Vec::new(),
    }
}
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    pub(crate) async fn execute_language_operation(
        ...
        if operation.command == "__agentos_typescript_file" {
            let requested_path = operation.args.first().cloned().ok_or_else(|| {
                VmError::InvalidState(String::from("executeTypeScriptFile requires a file path"))
            })?;
            let vm = self
                .vms
                .get_mut(&vm_id)
        ...
        let (process_id, generation, descriptor, reused_resident) = {
            ...
            let execution = vm
                ...
                .or_insert_with(|| ManagedLanguageExecution {
                    ...
                    event_limit: self.config.protocol.max_process_events.max(1),
    ...
    pub(crate) fn expire_public_execution_deadlines(&mut self) -> Result<(), VmError> {
        let now = now_ms();
...
fn active_process_id<B: VmManagerHost>(
    sidecar: &VmManager<B>,
    vm_id: &str,
    execution_id: &str,
