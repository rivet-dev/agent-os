...
pub(crate) use crate::core::bindings::{
...
use crate::core::permissions::{allow_all_policy, deny_all_policy, evaluate_permissions_policy};
use crate::protocol::{
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    ...
    let registration_result = (|| -> Result<_, VmError> {
        ...
        Ok::<_, VmError>(binding_command_names(vm).len() as u32)
    })();
...
fn refresh_binding_registry(vm: &mut VmState) -> Result<(), VmError> {
...
pub(crate) fn resolve_binding_command(
    ...
    args: &[String],
    cwd: Option<&str>,
) -> Result<Option<BindingCommandResolution>, VmError> {
...
fn resolve_registry_command(
    ...
    args: &[String],
    guest_cwd: &str,
) -> Result<BindingCommandResolution, VmError> {
    let timeout_ms =
...
fn validate_bindings_registration(payload: &RegisterHostCallbacksRequest) -> Result<(), VmError> {
    core_validate_bindings_registration(payload).map_err(binding_registration_error)
}

fn binding_registration_error(error: BindingRegistrationError) -> VmError {
    match error {
        BindingRegistrationError::InvalidState(message) => VmError::InvalidState(message),
        BindingRegistrationError::Conflict(message) => VmError::Conflict(message),
    }
}
