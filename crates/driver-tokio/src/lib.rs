...
//!
//! Production sidecars construct exactly one [`TokioDriver`] at their
//! process entrypoint. Subsystems receive a clone of [`DriverHandle`]; they
...
use metrics::{
    DriverMetrics, ExecutorMetricClass, TelemetryFallback, TelemetryFallbackCode,
...
thread_local! {
    static IS_AGENTOS_DRIVER_WORKER: Cell<bool> = const { Cell::new(false) };
}
...
pub fn is_driver_worker_thread() -> bool {
    IS_AGENTOS_DRIVER_WORKER.with(Cell::get)
}

#[derive(Clone, Debug, PartialEq, Eq)]
pub struct DriverFairnessConfig {
...
impl Default for DriverFairnessConfig {
    fn default() -> Self {
        Self {
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct DriverResourceConfig {
...
impl Default for DriverResourceConfig {
    fn default() -> Self {
        Self {
...
pub struct DriverConfig {
    ...
    pub resources: DriverResourceConfig,
    pub fairness: DriverFairnessConfig,
}
...
impl Default for DriverConfig {
    fn default() -> Self {
        ...
        Self {
            ...
            resources: DriverResourceConfig::default(),
            fairness: DriverFairnessConfig::default(),
        }
    }
}

impl DriverConfig {
    pub fn validate(&self) -> Result<(), DriverBuildError> {
        for (field, value) in [
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct DriverBuildError(String);
...
/// A generation-scoped view of the process driver.
///
/// Scoping changes task ownership and resource admission while preserving the
/// process's single Tokio runtime.
...
impl DriverHandle {
    ...
    pub fn allocate_vm_generation(&self) -> Result<u64, DriverBuildError> {
    ...
    pub fn scoped_for_vm(&self, resources: Arc<ResourceLedger>, generation: u64) -> VmDriverHandle {
...
impl<F> WatchdogFuture<F> {
    fn new(future: F, class: TaskClass, threshold: Duration, metrics: DriverMetrics) -> Self {
        Self {
...
static PROCESS_RUNTIME: OnceLock<Result<TokioDriver, DriverBuildError>> = OnceLock::new();

impl TokioDriver {
    fn build(config: DriverConfig) -> Result<Self, DriverBuildError> {
        config.validate()?;
        ...
        let runtime = tokio::runtime::Builder::new_multi_thread()
            ...
            .on_thread_start(|| IS_AGENTOS_DRIVER_WORKER.with(|marker| marker.set(true)))
            .on_thread_stop(|| IS_AGENTOS_DRIVER_WORKER.with(|marker| marker.set(false)))
            .enable_all()
            .build()
            .map_err(|error| {
                DriverBuildError(format!(
    ...
    pub fn process(config: &DriverConfig) -> Result<&'static Self, DriverBuildError> {
        match PROCESS_RUNTIME.get_or_init(|| Self::build(config.clone())) {
            Ok(runtime) if &runtime.config == config => Ok(runtime),
            Ok(runtime) => Err(DriverBuildError(format!(
    ...
    /// process scheduler.
    pub fn process_handle() -> Result<DriverHandle, DriverBuildError> {
        match PROCESS_RUNTIME.get() {
            Some(Ok(runtime)) => Ok(runtime.handle()),
            Some(Err(error)) => Err(error.clone()),
            #[cfg(test)]
            None => Self::process(&DriverConfig::default()).map(Self::handle),
            #[cfg(not(test))]
            None => Err(DriverBuildError(String::from(
                "ERR_AGENTOS_RUNTIME_NOT_INITIALIZED: the process entrypoint must construct TokioDriver before starting subsystems",
            ))),
        }
    }
...
mod tests {
    ...
    fn process_runtime_bounds_every_resource_class_by_default() {
        ...
        for resource in ResourceClass::ALL {
            let usage = runtime.handle().resources().usage(resource);
    ...
    fn blocking_executor_enforces_byte_reservations() {
        let runtime = TokioDriver::build(DriverConfig {
            worker_threads: 1,
        ...
        let blocking = runtime.handle().blocking().clone();
        let error = runtime
    ...
    fn blocking_executor_runs_on_fixed_workers_and_releases_bytes() {
        ...
        assert_eq!(blocking.reserved_bytes(), 0);
        let metrics = runtime.handle().metrics().snapshot();
        assert_eq!(
    ...
    fn vm_scoped_context_shares_workers_and_charges_both_ledgers() {
        ...
        .expect("build runtime");
        let process = Arc::clone(runtime.handle().resources());
        ...
        let scoped = runtime.handle().scoped(Arc::clone(&vm_ledger));
    ...
    fn assert_vm_generation_churn_reconciles(generation_count: u64, logical_vm_count: u64) {
        ...
        assert_eq!(runtime.handle().tasks().active_total(), 0);
    }
    ...
    fn closing_vm_context_rejects_stale_task_and_blocking_clones() {
        ...
        let scoped = runtime.handle().scoped_for_vm(Arc::clone(&vm_ledger), 77);
    ...
    fn close_admission_linearizes_task_and_blocking_rejection() {
        ...
        let scoped = runtime.handle().scoped_for_vm(Arc::clone(&vm_ledger), 88);
    ...
    fn closing_vm_context_wakes_admitted_readiness_waiters() {
        ...
        let scoped = runtime.handle().scoped_for_vm(resources, 88);
