...
impl ExecutionBackend for WasmV8Execution {
    ...
    fn native_process_id(&self) -> Option<u32> {
        WasmV8Execution::native_process_id(self)
    }
    ...
    fn is_prepared_for_start(&self) -> bool {
        WasmV8Execution::is_prepared_for_start(self)
    }

    fn start_prepared(&mut self) -> Result<(), HostServiceError> {
        WasmV8Execution::start_prepared(self).map_err(|error| {
    ...
    fn set_paused(&self, paused: bool) -> Result<(), HostServiceError> {
        let result = if paused {
            WasmV8Execution::pause(self)
        } else {
            WasmV8Execution::resume(self)
        };
    ...
    fn close_stdin(&mut self) -> Result<(), HostServiceError> {
        WasmV8Execution::close_stdin(self).map_err(|error| {
...
fn no_native_host_consumer() -> WasmExecutionError {
    WasmExecutionError::Host(HostServiceError::new(
        "ENOTCONN",
        "native WebAssembly host calls require the sidecar host-event consumer",
    ))
}
...
impl Default for WasmV8ExecutionEngine {
    fn default() -> Self {
...
mod tests {
    use super::{
        ...
        WasmExecutionError, WasmExecutionEvent, WasmExecutionLimits, WasmInternalSyncRpc,
        WasmPendingEventBudget, WasmPermissionTier, WasmSignalCheckpointInbox,
        WasmV8ExecutionEngine, DEFAULT_WASM_PREWARM_TIMEOUT_MS, DEFAULT_WASM_RUNNER_HEAP_LIMIT_MB,
    ...
    use agentos_executor_contract::backend::{ExecutionWakeIdentity, PublishedSignalCheckpoint};
...
"#
    ...
    fn dispose_context_reclaims_wasm_and_nested_javascript_metadata() {
        let mut engine = WasmV8ExecutionEngine::default();
        let baseline = (
    ...
    fn managed_signal_dispatch_drains_the_published_delivery_without_double_claiming() {
        ...
        let end = helper
            .find("Object.defineProperty(globalThis, '__agentOsWasmSignalDispatch'")
