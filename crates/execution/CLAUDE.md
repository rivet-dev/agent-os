...
- For builtins that guest CommonJS should `require("node:...")`, update both `Module.builtinModules` and `loadBuiltinModule()` in `crates/execution/assets/v8-bridge.source.js`; adding only one side leaves `_requireFrom()` stuck resolving the builtin as a normal module and can deadlock or recurse.
