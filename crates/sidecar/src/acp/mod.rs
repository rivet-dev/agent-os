...
use agentos_vm::extension::ExtensionSnapshot;
use agentos_vm::limits::AcpLimits;
#[cfg(test)]
use agentos_vm::limits::DEFAULT_ACP_MAX_READ_LINE_BYTES;
use agentos_vm::wire::{
...
use agentos_vm::{
    ...
    ExtensionInterruptResponse, ExtensionResponse, VmError,
};
...
impl AcpExtension {
    ...
    async fn handle_payload(
        ...
        payload: &[u8],
    ) -> Result<ExtensionResponse, VmError> {
    ...
    async fn session_store(&self, ctx: &mut ExtensionContext<'_>) -> Result<SessionStore, VmError> {
    ...
    async fn open_session(
        ...
        match store.get(&session_id).await {
            Ok(Some(existing)) => {
                ...
                {
                    return AcpHandlerOutput::response(Err(VmError::InvalidState(format!(
    ...
    async fn read_history(
        ...
        let session = store
            ...
            .await
            .map_err(session_store_error)?
            .ok_or_else(|| VmError::InvalidState(format!("session_not_found: {session_id}")))?;
            ...
            .map(|event| {
                Ok(AcpDurableHistoryEntry {
                    ...
                    sequence: u64::try_from(event.sequence).map_err(|_| {
                        VmError::InvalidState(String::from("invalid stored history sequence"))
                    })?,
...
impl Extension for AcpExtension {
    ...
    fn bootstrap_vm_database<'a>(
        &'a self,
        database: agentos_vm::vm_sqlite::SharedVmSqliteDatabase,
    ) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
...
struct AcpHandlerOutput {
    response: Result<AcpResponse, VmError>,
    events: Vec<agentos_vm::wire::EventFrame>,
}

impl AcpHandlerOutput {
    fn response(response: Result<AcpResponse, VmError>) -> Self {
        Self {
            response,
            events: Vec::new(),
        }
...
fn session_store_error(error: agentos_vm::vm_sqlite::VmSqliteError) -> VmError {
    match error {
        error @ (agentos_vm::vm_sqlite::VmSqliteError::ResultTooLarge { .. }
        | agentos_vm::vm_sqlite::VmSqliteError::HistoryEventBatchTooLarge { .. }
        | agentos_vm::vm_sqlite::VmSqliteError::HistoryByteBatchTooLarge { .. }
        | agentos_vm::vm_sqlite::VmSqliteError::DurableCollectionLimit { .. }) => {
            VmError::InvalidState(error.to_string())
        }
        error => VmError::InvalidState(format!("session_storage_error: {error}")),
    }
}

fn validate_user_session_id(session_id: &str) -> Result<(), VmError> {
...
fn default_session_id(session_id: Option<String>) -> Result<String, VmError> {
    let session_id = session_id.unwrap_or_else(|| String::from("main"));
    validate_user_session_id(&session_id)?;
    Ok(session_id)
}

fn safe_sequence(sequence: u64) -> Result<i64, VmError> {
...
fn parse_json_array(text: &str, field: &str) -> Result<Vec<Value>, VmError> {
    serde_json::from_str::<Vec<Value>>(text)
        .map_err(|error| VmError::InvalidState(format!("invalid {field} JSON array: {error}")))
}

fn parse_mcp_servers(text: &str) -> Result<Vec<McpServer>, VmError> {
...
fn parse_string_map(text: &str, field: &str) -> Result<HashMap<String, String>, VmError> {
    serde_json::from_str::<HashMap<String, String>>(text)
        .map_err(|error| VmError::InvalidState(format!("invalid {field} JSON object: {error}")))
}
...
fn stored_session_info(session: StoredSession) -> Result<AcpDurableSessionInfo, VmError> {
    Ok(AcpDurableSessionInfo {
        ...
        latest_sequence: u64::try_from(session.latest_sequence)
            .map_err(|_| VmError::InvalidState(String::from("invalid stored latest sequence")))?,
        title: session.title,
...
) -> Result<AcpDurableSessionInfo, VmError> {
    Ok(AcpDurableSessionInfo {
        session_id: session.session_id,
        agent: session.agent,
        cwd: session.cwd,
...
fn encode_list_cursor(session: &StoredSessionSummary) -> Result<String, VmError> {
...
fn decode_list_cursor(cursor: &str) -> Result<(i64, String), VmError> {
    let payload = base64::engine::general_purpose::URL_SAFE_NO_PAD
        .decode(cursor)
        .map_err(|_| {
            VmError::InvalidState(String::from("invalid_session_cursor: malformed cursor"))
        })?;
...
fn json_strings_to_array_text(values: &[String]) -> Result<String, VmError> {
    let values = values
        .iter()
        .map(|value| serde_json::from_str::<Value>(value))
        .collect::<Result<Vec<_>, _>>()
        .map_err(|error| {
            VmError::InvalidState(format!("invalid ACP configuration option: {error}"))
        })?;
    serde_json::to_string(&values).map_err(|error| VmError::InvalidState(error.to_string()))
}

fn decode_request(payload: &[u8]) -> Result<AcpRequest, VmError> {
    serde_bare::from_slice(payload)
        .map_err(|error| VmError::InvalidState(format!("invalid ACP request: {error}")))
}

fn encode_response(response: AcpResponse) -> Result<Vec<u8>, VmError> {
    serde_bare::to_vec(&response)
        .map_err(|error| VmError::InvalidState(format!("invalid ACP response: {error}")))
}

fn encode_event(event: AcpEvent) -> Result<Vec<u8>, VmError> {
    serde_bare::to_vec(&event)
        .map_err(|error| VmError::InvalidState(format!("invalid ACP event: {error}")))
}

fn encode_callback(callback: AcpCallback) -> Result<Vec<u8>, VmError> {
    serde_bare::to_vec(&callback)
        .map_err(|error| VmError::InvalidState(format!("invalid ACP callback: {error}")))
}

fn error_response(error: VmError) -> AcpResponse {
...
fn error_code(error: &VmError) -> String {
    if let VmError::Host(error) = error {
        return error.code.clone();
    }
    let code = match error {
        VmError::ResourceLimit(_) => "resource_limit",
        VmError::InvalidState(message) => message
            .split_once(':')
            .map(|(prefix, _)| prefix)
            .filter(|prefix| {
                !prefix.is_empty()
        ...
        VmError::Kernel(_) => "kernel",
        VmError::Plugin(_) => "plugin",
        VmError::Execution(_) => "execution",
        VmError::ExecutionEventChannelClosed { .. } => "execution_event_channel_closed",
        VmError::Bridge(_) => "bridge",
        VmError::Io(_) => "io",
        VmError::Host(_) => unreachable!("handled above"),
    };
    String::from(code)
}
...
mod tests {
    use super::*;
    use agentos_acp_protocol::PROTOCOL_VERSION;
    ...
    fn obsolete_permission_protocol_and_backend_surfaces_stay_removed() {
        let backend_sources = [
            ...
            include_str!("../../../acp-protocol/protocol/agent_os_acp_v1.bare"),
        ];
        for removed in [
    ...
    fn configured_acp_limit_errors_preserve_stable_wire_codes() {
        ...
        assert_eq!(
            error_code(&VmError::InvalidState(String::from(
        ...
        assert_eq!(
            error_code(&session_store_error(
                agentos_vm::vm_sqlite::VmSqliteError::ResultTooLarge { used: 2, limit: 1 }
            )),
            "sqlite_result_limit"
        );
    }
    ...
    fn adapter_gone_classifier_matches_both_observation_paths() {
        ...
        let exited = VmError::InvalidState(format!(
        ...
        let gone = VmError::InvalidState(String::from("VM vm-5 has no active process agent-6"));
        ...
        let transient = VmError::InvalidState(String::from(
    ...
    fn durable_route_keys_preserve_full_vm_ownership_identity() {
        use agentos_vm::wire::VmOwnership;
    ...
    fn adapter_exit_error_is_recognized_for_eviction() {
        ...
        let timed_out =
            VmError::InvalidState(String::from("timed out waiting for ACP response id=3"));
        assert!(!is_adapter_exited_error(&timed_out));
        let broken_pipe = VmError::InvalidState(String::from(
