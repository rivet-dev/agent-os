...
impl AcpExtension {
    pub(super) async fn prompt_durable_session(
        ...
        let input_json = match serde_json::to_string(&json!({
            ...
            Ok(value) => value,
            Err(error) => {
                return AcpHandlerOutput::response(Err(VmError::InvalidState(error.to_string())));
            }
        };
        ...
        if let Ok(mut cancellations) = self.prompt_cancellations.lock() {
            ...
            let error = finish_prompt_failure(
                ...
                VmError::InvalidState(String::from("prompt cancellation registry is poisoned")),
            )
            .await;
            return AcpHandlerOutput {
                response: Err(error),
        ...
        let rpc = match raw.response {
            ...
            Ok(other) => {
                let error =
                    VmError::InvalidState(format!("invalid prompt response variant: {other:?}"));
                let error = finish_prompt_failure(
                    &store,
                    &session_id,
                    &prompt_id,
        ...
        let response: Value = match serde_json::from_str(&rpc.response) {
            ...
            Err(error) => {
                let error = finish_prompt_failure(
                    ...
                    sink.last_output_sequence,
                    "invalid_prompt_response",
                    VmError::InvalidState(format!("invalid ACP prompt response JSON: {error}")),
                )
                .await;
                return AcpHandlerOutput {
                    response: Err(error),
        ...
        let response = match &output.response {
            Ok(AcpResponse::AcpSessionRpcResponse(response)) => {
                match serde_json::from_str::<Value>(&response.response) {
                    ...
                    Err(error) => {
                        return AcpHandlerOutput {
                            response: Err(VmError::InvalidState(format!(
            ...
            Ok(_) => {
                return AcpHandlerOutput {
                    response: Err(VmError::InvalidState(String::from(
...
pub(super) struct DurableUpdateSink {
    ...
    limits: agentos_vm::limits::AcpLimits,
...
impl DurableUpdateSink {
    ...
    pub(super) async fn handle_permission_request(
        ...
        let acp_request_id = rpc_id.cloned().ok_or_else(|| {
            VmError::InvalidState(String::from("ACP inbound request missing private id"))
        })?;
        ...
        let offered_option_ids = permission_option_ids(params);
        {
            let mut pending = self.pending_permission_responses.lock().map_err(|_| {
                VmError::InvalidState(String::from("permission response registry is poisoned"))
            })?;
            if pending
                .insert(
                    key.clone(),
        ...
        for event in stored {
            let payload = encode_event(AcpEvent::AcpDurableSessionEvent(AcpDurableSessionEvent {
                ...
                sequence: u64::try_from(event.sequence)
                    .map_err(|_| VmError::InvalidState(String::from("invalid stored sequence")))?,
    ...
    pub(super) fn message_json(&self) -> Result<Option<String>, VmError> {
...
pub(super) fn decode_durable_event(event_json: &str) -> Result<AcpDurableEvent, VmError> {
    let event: Value = serde_json::from_str(event_json)
        .map_err(|error| VmError::InvalidState(format!("invalid durable event JSON: {error}")))?;
    let kind = event
        .get("type")
        .and_then(Value::as_str)
        .ok_or_else(|| VmError::InvalidState(String::from("durable event is missing type")))?;
    match kind {
        "session_update" => Ok(AcpDurableEvent::AcpDurableSessionUpdate(
            AcpDurableSessionUpdate {
                update: serde_json::to_string(event.get("update").ok_or_else(|| {
                    VmError::InvalidState(String::from("durable session update is missing update"))
                })?)
                .map_err(|error| VmError::InvalidState(error.to_string()))?,
            },
        )),
...
async fn wait_for_permission_signal(
    ...
    events: &mut Vec<agentos_vm::wire::EventFrame>,
) -> Result<PendingPermissionSignal, VmError> {
        ...
        let signal = if let Some(cancellation) = cancellation.as_deref_mut() {
            tokio::select! {
                ...
                response = &mut receiver => Some(response.map_err(|_| {
                    VmError::InvalidState(format!("permission request {request_id} lost its waiter"))
                })?),
                changed = cancellation.changed() => {
                    if changed.is_err() || *cancellation.borrow() {
                        pending.lock().map_err(|_| VmError::InvalidState(String::from(
...
fn handle_permission_wait_event(
    ...
    pending: &Arc<StdMutex<BTreeMap<String, PendingPermissionResponse>>>,
    events: &mut Vec<agentos_vm::wire::EventFrame>,
    event: Option<agentos_vm::wire::EventFrame>,
) -> Result<Option<PendingPermissionSignal>, VmError> {
...
pub(super) fn public_permission_request(
    ...
    request
        .as_object_mut()
        .ok_or_else(|| {
            VmError::InvalidState(String::from("ACP permission params must be an object"))
        })?
        .insert(
...
pub(super) fn parse_content_blocks(
    ...
    .map_err(|error| VmError::InvalidState(format!("invalid_content_block: {error}")))?;
    Ok(blocks)
}
...
pub(super) async fn finish_prompt_failure(
    ...
    {
        Ok(_) => error,
        Err(commit_error) => VmError::InvalidState(format!(
            "{message}; additionally failed to commit terminal prompt state: {commit_error}"
        )),
    }
}

pub(super) fn prompt_response_from_json(text: &str) -> Result<AcpResponse, VmError> {
    let value: Value = serde_json::from_str(text)
        .map_err(|error| VmError::InvalidState(format!("invalid stored prompt result: {error}")))?;
    Ok(AcpResponse::AcpPromptResponse(AcpPromptResponse {
        session_id: value
            .get("sessionId")
            .and_then(Value::as_str)
            .ok_or_else(|| VmError::InvalidState(String::from("stored result missing sessionId")))?
            .to_owned(),
        message: value
            .get("message")
        ...
        stop_reason: value
            .get("stopReason")
            .and_then(Value::as_str)
            .ok_or_else(|| VmError::InvalidState(String::from("stored result missing stopReason")))?
            .to_owned(),
    }))
}

pub(super) fn coalesce_completed_message(updates: Vec<Value>) -> Result<Vec<Value>, VmError> {
...
pub(super) fn checked_acp_bytes(
    ...
    let next = used.checked_add(requested).ok_or_else(|| {
        VmError::ResourceLimit(LimitError {
    ...
    if next > limit {
        return Err(VmError::ResourceLimit(LimitError {
            scope: format!("session={session_id}"),
            resource: ResourceClass::BufferedBytes,
            used,
            requested,
