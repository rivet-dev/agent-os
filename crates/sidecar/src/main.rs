...
fn main() {
    ...
    if env_flag("AGENTOS_SIDECAR_COMBINED_STDIO") {
        if let Err(error) = agentos_sidecar::transport::run_combined_with_extensions_and_executors(
    ...
    if let Err(error) = agentos_sidecar::transport::run_with_extensions_and_executors(
        agentos_sidecar::extensions(),
        agentos_sidecar::executor_registry(),
        control_fd,
    ) {
