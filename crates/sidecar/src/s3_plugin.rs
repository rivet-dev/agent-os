...
mod tests {
    ...
    fn s3_plugin_persists_files_across_reopen_and_preserves_links() {
        ...
        let mut filesystem =
            S3BackedFilesystem::from_config(test_config(&server, "persist")).expect("open s3 fs");
        filesystem
        ...
        let mut reopened =
            S3BackedFilesystem::from_config(test_config(&server, "persist")).expect("reopen s3 fs");
    ...
    fn s3_plugin_cleans_up_stale_chunk_objects_after_truncate() {
        ...
        let mut filesystem =
            S3BackedFilesystem::from_config(test_config(&server, "truncate")).expect("open s3 fs");
        filesystem
    ...
    fn s3_plugin_metadata_only_flush_reuses_existing_chunks() {
        ...
        let mut reopened =
            S3BackedFilesystem::from_config(test_config(&server, "chmod")).expect("reopen s3 fs");
        assert_eq!(
            reopened
                .stat("/large.txt")
    ...
    fn s3_plugin_rejects_oversized_manifest_entries() {
        ...
        let manifest = PersistedFilesystemManifest {
            format: String::from(MANIFEST_FORMAT),
            path_index: BTreeMap::from([(String::from("/"), 1), (String::from("/huge.bin"), 2)]),
            inodes: BTreeMap::from([
                (
                    1,
                    PersistedFilesystemInode {
