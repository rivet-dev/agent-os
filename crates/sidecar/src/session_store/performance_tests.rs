...
use agentos_vm::limits::{DEFAULT_ACP_MAX_SESSION_LIST_ENTRIES, DEFAULT_SQLITE_MAX_RESULT_BYTES};
use agentos_vm::vm_sqlite::{VmSqliteDatabase, VmSqliteError};
...
impl ActorUdsFixture {
    ...
    async fn database(&self) -> SharedVmSqliteDatabase {
        agentos_vm::vm_sqlite::resolve_vm_sqlite(
...
fn append_work_is_constant_near_history_limit_on_local_file_and_actor_uds() {
    runtime().block_on(async {
        let dir = tempfile::tempdir().expect("tempdir");
        let local = agentos_vm::vm_sqlite::resolve_vm_sqlite(
