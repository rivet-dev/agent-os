use agentos_resource_accounting::queue_tracker::TrackedLimit;
use agentos_vm::core::{generated_wire_blocking_extension_interrupt, BlockingExtensionInterrupt};
use agentos_vm::wire::{
...
use agentos_vm::{
    EventSinkTransport, Extension, ExtensionInterruptRequest, SidecarRequestTransport, VmError,
...
struct ProtocolBudgetConfig {
    ...
    label: &'static str,
    metric: agentos_driver_tokio::metrics::ChannelMetricClass,
}
...
struct ProtocolBudget {
    ...
    changed: Arc<Condvar>,
    metrics: agentos_driver_tokio::metrics::DriverMetrics,
}
...
fn wire_protocol_error(error: ProtocolCodecError) -> VmError {
...
pub fn run_with_extensions(
    ...
    run_with_extensions_and_executors(extensions, crate::executor_registry(), control_fd)
}

pub fn run_combined_with_extensions(
    extensions: Vec<Box<dyn Extension>>,
) -> Result<(), Box<dyn Error>> {
    run_combined_with_extensions_and_executors(extensions, crate::executor_registry())
}

pub fn run_with_extensions_and_executors(
    extensions: Vec<Box<dyn Extension>>,
    executors: agentos_vm::ExecutorRegistry,
    control_fd: OwnedFd,
) -> Result<(), Box<dyn Error>> {
    run_with_optional_control(extensions, executors, Some(control_fd))
}

pub fn run_combined_with_extensions_and_executors(
    extensions: Vec<Box<dyn Extension>>,
    executors: agentos_vm::ExecutorRegistry,
) -> Result<(), Box<dyn Error>> {
    run_with_optional_control(extensions, executors, None)
}
...
fn run_with_optional_control(
    ...
    let config = VmManagerConfig {
        instance_id: String::from("agentos-sidecar"),
    ...
    let runtime_context = runtime.handle();
    ...
    {
        if let Err(error) =
            agentos_vm::executor::v8_host::ensure_runtime_initialized(&runtime_context)
        {
...
async fn run_async(
    ...
    let ingress_budget = ProtocolBudget::new(
        ProtocolBudgetConfig {
            ...
            metric: agentos_driver_tokio::metrics::ChannelMetricClass::StdioIngress,
        },
        metrics.clone(),
    );
    ...
    let stdin_gauge = agentos_resource_accounting::queue_tracker::register_queue(
    ...
    let ordinary_egress_budget = ProtocolBudget::new(
        ProtocolBudgetConfig {
            ...
            metric: agentos_driver_tokio::metrics::ChannelMetricClass::StdioEgress,
        },
    ...
    let (limit_warning_tx, mut limit_warning_rx) = channel::<
        agentos_resource_accounting::queue_tracker::LimitWarning,
    >(MAX_LIMIT_WARNING_QUEUE);
    agentos_resource_accounting::queue_tracker::set_limit_warning_handler(Box::new(
        move |warning| {
    ...
    // needed.
    let process_event_notify = sidecar.process_event_notify();
    ...
    if let Some(mut control_writer) = control_writer.take() {
        ...
        runtime_context.spawn(agentos_driver_tokio::TaskClass::Runtime, async move {
                ...
                match maybe_warning {
                    Some(warning) => {
                        ...
                        if let Some(connection_id) = active_connections.iter().next() {
                            ...
                            let frame = agentos_vm::service::structured_event_frame(
                                connection_id,
                                "limit_warning",
                                detail,
                            )?;
...
fn blocking_extension_request(
    ...
    let extension = sidecar.extension(&envelope.namespace)?;
...
fn spawn_heartbeat_thread(
    ...
    thread::spawn(move || {
        loop {
            ...
            let frame = match agentos_vm::service::structured_event_frame(
                ...
                Err(error) => {
                    ...
                    tracing::error!(
                        target: "agentos_sidecar::transport",
                        %error,
...
fn default_compile_cache_root() -> PathBuf {
    ...
    std::env::temp_dir().join("agentos-vm-compile-cache")
}

#[cfg(test)]
mod tests {
    use super::*;
    use agentos_vm::wire::{AuthenticateRequest, KillProcessRequest};
    use agentos_vm::{
        ExtensionContext, ExtensionFuture, ExtensionInterruptResponse, ExtensionResponse,
    };
    use std::io::Cursor;
    ...
    fn test_protocol_budget(
        ...
        ProtocolBudget::new(
            ...
            agentos_driver_tokio::metrics::DriverMetrics::new(),
        )
    }
    ...
    async fn heartbeat_task_emits_periodic_structured_heartbeat_frames() {
        ...
        for beat in 0..2 {
            ...
            let event =
                agentos_vm::wire::event_frame_to_compat(event).expect("decode heartbeat frame");
            let agentos_vm::protocol::EventPayload::Structured(structured) = event.payload else {
    ...
    fn stdio_work_queues_are_bounded() {
        let capacity = agentos_sidecar_protocol::config::DEFAULT_MAX_INGRESS_FRAMES;
    ...
    async fn protocol_output_queue_physically_separates_control_and_events() {
        ...
        let event = agentos_vm::service::structured_event_frame(
            "conn-priority",
            "ordinary",
            std::collections::HashMap::new(),
        )
    ...
    fn stdout_frame_queue_applies_backpressure_instead_of_crashing() {
        let queue_frame = |request_id: RequestId| {
            ...
            ProtocolFrame::EventFrame(
                agentos_vm::service::structured_event_frame("conn-queue", "queue-test", detail)
                    .expect("queue event"),
            )
        };
...
impl SessionScope {
    ...
    fn compat_ownership_scope(&self) -> agentos_vm::protocol::OwnershipScope {
...
impl EventSinkTransport for FrameEventTransport {
    fn emit_event(&self, event: agentos_vm::wire::EventFrame) -> Result<(), VmError> {
        send_output_frame(&self.writer, ProtocolFrame::EventFrame(event))
            .map_err(|error| VmError::Bridge(error.to_string()))
    }
}
...
impl FrameSidecarRequestLimits {
    fn from_config(config: &VmManagerConfig) -> Self {
        Self {
            max_pending_responses: config.protocol.max_pending_responses,
            max_pending_response_bytes: config.protocol.max_pending_response_bytes,
...
type PendingSidecarResponseResult = Result<PendingSidecarResponse, VmError>;
...
impl FrameSidecarRequestTransport {
    ...
    fn register_waiter(
        &self,
        request_id: RequestId,
    ) -> Result<mpsc::Receiver<PendingSidecarResponseResult>, VmError> {
        let mut pending = self.pending.lock().map_err(|_| {
            VmError::Bridge(String::from("sidecar callback waiter map lock poisoned"))
        })?;
    ...
    fn cancel_waiter(&self, request_id: RequestId) -> Result<(), VmError> {
        self.pending
            .lock()
            .map_err(|_| {
                VmError::Bridge(String::from("sidecar callback waiter map lock poisoned"))
...
impl SidecarRequestTransport for FrameSidecarRequestTransport {
    fn send_request(
        &self,
        request: agentos_vm::protocol::SidecarRequestFrame,
        timeout: Duration,
    ) -> Result<agentos_vm::protocol::SidecarResponseFrame, VmError> {
        let request =
        ...
        match receiver.recv_timeout(response_timeout) {
            ...
            Err(mpsc::RecvTimeoutError::Disconnected) => Err(VmError::Io(String::from(
