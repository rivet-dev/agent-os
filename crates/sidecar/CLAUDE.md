...
- Author ACP behavior as an `Ext` extension over `agentos-acp-protocol`; do not add new top-level sidecar request/response variants for agent-session RPCs.
- Keep `agentos-acp-protocol` as the only ACP payload schema source; extension requests, responses, events, and callbacks must use the generated BARE types.
