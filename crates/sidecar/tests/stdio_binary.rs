#[path = "../../vm/tests/support/mod.rs"]
mod support;

use agentos_vm::wire::{self, *};
use base64::Engine;
...
fn spawn_sidecar_binary() -> (Child, ChildStdin, ChildStdout, UnixStream) {
    ...
    let mut command = Command::new(env!("CARGO_BIN_EXE_agentos-sidecar"));
    command
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
    ...
    let mut child = command.spawn().expect("spawn sidecar binary");
...
fn native_sidecar_binary_supports_js_bridge_host_filesystem_access() {
    let host_root = temp_dir("stdio-binary-vm-host-interface");
    fs::write(host_root.join("existing.txt"), "vm-host-interface-ok").expect("seed host file");
    ...
    match read.payload {
        ResponsePayload::GuestFilesystemResultResponse(response) => {
            assert_eq!(response.content.as_deref(), Some("vm-host-interface-ok"));
        }
        other => panic!("unexpected read response: {other:?}"),
    }
