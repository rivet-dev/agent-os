...
//! loop in `crates/sidecar/src/acp/runtime.rs` now (a) forwards agent
//! stderr as an Agent OS ACP extension event and (b) observes the adapter
//! `ProcessExitedEvent` and returns
//! `VmError::InvalidState("ACP adapter process {id} exited with code {} ...")`.
//!
...
fn adapter_stderr_and_exit_surface_to_caller() {
    ...
    // `VmError::InvalidState` into an `AcpErrorResponse` that carries the
...
fn new_sidecar(name: &str) -> VmManager<RecordingBridge> {
    ...
    let driver = agentos_driver_tokio::TokioDriver::process(&config.runtime)
        .expect("create process driver")
        .handle();
    VmManager::with_config_extensions_driver_and_executors(
...
fn authenticate(sidecar: &mut VmManager<RecordingBridge>) -> String {
    let result = sidecar
        .dispatch_wire_blocking(RequestFrame {
            schema: agentos_vm::wire::protocol_schema(),
            request_id: 1,
...
fn open_session(sidecar: &mut VmManager<RecordingBridge>, connection_id: &str) -> String {
    let result = sidecar
        .dispatch_wire_blocking(RequestFrame {
            schema: agentos_vm::wire::protocol_schema(),
            request_id: 2,
