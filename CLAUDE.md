...
- Rust execution tests that instantiate `JavascriptExecutionEngine`, `PythonExecutionEngine`, or `WasmExecutionEngine` should call `set_import_cache_base_dir(...)` with a unique temp root per test VM before starting work; otherwise parallel `cargo test` runs can race through the shared `/tmp` stale-cache sweep and delete another test's runner assets.
- Rust execution tests that override process-wide env such as `AGENT_OS_NODE_BINARY` must serialize that override with a shared lock for the full test body; the env var is global to the test process, so parallel tests will otherwise spawn the wrong fake node binary or delete another test's temp fixture mid-run.
- Rust sidecar integration tests that swap `AGENT_OS_NODE_BINARY` for a fake shim must take the same shared lock across the full test body, including sibling tests in the same test binary that run the real Node runtime; otherwise the shim leaks into unrelated guest-execution assertions and produces misleading empty stdout or invocation-count failures.
- Sidecar Node isolation tests that assert guest-visible `process.pid` / `process.ppid` should compare against `SnapshotProcesses`, not `ProcessStartedResponse.pid`; the execute response reports the host runtime child PID, while the snapshot exposes the kernel PID surfaced inside the guest.
...
- For projected guest packages that still hang or fail on heavy bare-package imports (for example `@mariozechner/pi-ai`, `@mariozechner/pi-ai/oauth`, `yaml`, `ajv`, or interactive-theme dependencies inside `@mariozechner/pi-coding-agent`), fix the issue in `crates/execution/src/node_import_cache.rs` `rewritePiCodingAgentImports(...)` by rewriting the specific source files to direct sibling `dist/*.js` modules or tiny guest-owned stubs; do not fall back to host package resolution.
- In `crates/execution/src/node_import_cache.rs`, `guestVisiblePathFromHostPath(...)` is an error-scrubbing helper, not a reachability check; it returns `/unknown` for unmapped host paths, so module-resolution guards must reject that sentinel instead of treating any non-empty guest string as loadable.
- The generated Node runner in `crates/execution/src/node_import_cache.rs` must treat absolute host entrypoints that are already inside a visible guest-path mapping as host paths, not guest paths; remapping them through `hostPathFromGuestPath(...)` prefixes the shadow root twice and breaks `vm.writeFile(...); vm.spawn("node", ["/guest/path"])` flows.
- Top-level `await import(...)` failures in the generated Node runner should be rendered explicitly to stderr after guest-path translation; on newer Node releases, mutating `error.stack` and rethrowing still lets the default uncaught formatter print the original host file URL.
...
- In `packages/core/src/sidecar/native-kernel-proxy.ts`, `process_output` delivery must stay independent from `getSignalState(...)` cache refreshes; warming the signal-state cache is fine, but awaiting that extra RPC on the stdout/stderr path makes interactive shell tests and terminal flows flaky under parallel load.
